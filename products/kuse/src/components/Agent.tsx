...
import { type AgentEvent, isTauri, runAgent } from "../lib/tauri-api";
			...
			case "tool_end":
				setToolExecutions((prev) => {
					...
					const last = updated.findLast(
						(t: any) => t.tool === event.tool && t.status === "running",
					);
					if (last) {
			...
			>
				...
				>
					<div class="agent-output">
						...
						<Show
							when={!toolExecutions().length && !currentText() && !isRunning()}
						>
							<div class="empty-agent">
								<h2>Agent Mode</h2>
								<p>
									The agent can execute tools to help you with tasks like
									reading files, running commands, and searching code.
								</p>
								<p class="tools-list">
									Available tools: file_read, file_write, file_edit, bash, glob,
									grep, list_dir
								</p>
							</div>
						</Show>
