...
const ModelSelector: Component<ModelSelectorProps> = (props) => {
	...
	const [providerType, setProviderType] = createSignal<ProviderType>(
		getCurrentProviderType(),
	);
	const [ollamaStatus, setOllamaStatus] =
		createSignal<OllamaStatus>("checking");
	const [ollamaModels, setOllamaModels] = createSignal<OllamaModel[]>([]);
	const [ollamaBaseUrl, _setOllamaBaseUrl] = createSignal(
					...
					<Show when={ollamaStatus() === "running"}>
						...
						<Show when={ollamaModels().length === 0}>
							<div class="no-models">
								...
								<p class="hint">
									Run <code>ollama pull llama3.2</code> to install a model
								</p>
							</div>
						</Show>
