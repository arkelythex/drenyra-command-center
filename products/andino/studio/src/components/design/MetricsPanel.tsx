...
export default function MetricsPanel({ design }: { design: DroneDesign }) {
	return (
		<div className="bg-bg-surface border border-border-subtle rounded-[10px] p-4">
			...
			<MetricRow
				label="Arm Length"
				value={design.armLength.toString()}
				unit="mm"
			/>
			<MetricRow label="Motor Count" value={design.motorCount.toString()} />
			<MetricRow
				label="TWR"
				value={design.twr.toFixed(1)}
				unit=":1"
				valueClassName={twrColor(design.twr)}
			/>
			<MetricRow
				label="Payload"
				value={design.payloadMass.toString()}
				unit="g"
			/>
			<MetricRow
				label="Flight Time"
				value={design.flightTimeMin.toFixed(1)}
				unit="min"
			/>
			<MetricRow label="Cost" value={`$${design.costUsd.toLocaleString()}`} />
			<MetricRow
				label="Battery"
				value={`${design.batteryCells}S ${design.batteryCapacity}mAh`}
			/>
			<MetricRow
				label="Propeller"
				value={`${design.propellerDiameter}"x${design.propellerPitch}"`}
			/>
			<MetricRow
				label="Material"
				value={
					design.frameMaterial.charAt(0).toUpperCase() +
					design.frameMaterial.slice(1)
				}
			/>
		</div>
	);
