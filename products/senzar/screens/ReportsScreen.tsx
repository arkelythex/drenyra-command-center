    ...
    <div className="flex flex-1 flex-col overflow-hidden bg-gray-50 h-full p-6 md:p-8">
      <div className="max-w-6xl mx-auto w-full flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-3xl font-bold text-gray-900 tracking-tight">Reportes SENASA</h2>
            <p className="text-gray-500 mt-1">Gestión de certificados fitosanitarios y documentos de exportación.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer group">
            ...
            <h3 className="text-lg font-bold text-gray-900">Certificado Fitosanitario</h3>
            <p className="text-sm text-gray-500 mt-2 mb-4">Para exportación de productos vegetales frescos. Requiere inspección.</p>
            <span className="text-xs font-bold text-primary flex items-center group-hover:underline">
              Iniciar Trámite <Icon name="arrow_forward" className="text-sm ml-1" />
            </span>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer group">
            ...
            <h3 className="text-lg font-bold text-gray-900">Certificado de Origen</h3>
            <p className="text-sm text-gray-500 mt-2 mb-4">Acredita la procedencia de la mercancía para beneficios arancelarios.</p>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer group">
            ...
            <h3 className="text-lg font-bold text-gray-900">Reporte de Inspección</h3>
            <p className="text-sm text-gray-500 mt-2 mb-4">Registro interno de hallazgos de plagas y tratamientos aplicados.</p>
            <span className="text-xs font-bold text-orange-600 flex items-center group-hover:underline">
              Ver Historial <Icon name="arrow_forward" className="text-sm ml-1" />
            </span>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {[
                  { id: 'EXP-2024-001', type: 'Certificado Fitosanitario', date: '14 Oct 2024', status: 'approved' },
                  { id: 'EXP-2024-002', type: 'Certificado de Origen', date: '13 Oct 2024', status: 'pending' },
                  { id: 'INT-2024-089', type: 'Reporte Inspección', date: '12 Oct 2024', status: 'approved' },
                  { id: 'EXP-2024-003', type: 'Certificado Fitosanitario', date: '10 Oct 2024', status: 'rejected' },
                ].map((doc) => (
                    ...
                    <td className="px-6 py-4">
                      {doc.status === 'approved' && (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700">
                          <span className="w-1.5 h-1.5 rounded-full bg-green-600"></span> Aprobado
                        </span>
                      )}
                      {doc.status === 'pending' && (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-yellow-100 text-yellow-700">
                          <span className="w-1.5 h-1.5 rounded-full bg-yellow-600 animate-pulse"></span> En Trámite
                        </span>
                      )}
                      {doc.status === 'rejected' && (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700">
                          <span className="w-1.5 h-1.5 rounded-full bg-red-600"></span> Observado
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button className="text-primary font-bold hover:underline">Ver PDF</button>
                    </td>
                  </tr>
                ))}
              </tbody>
