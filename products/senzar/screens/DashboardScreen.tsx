...
const MOCK_LOTS: Lot[] = [
	...
	},
	{
		...
		status: "en-proceso",
		progress: 85,
		details: { responsible: "Ing. R. Torres", startTime: "06:30 AM" },
	},
	{
		id: "A-03",
		name: "Lote A-03",
...
export const DashboardScreen: React.FC = () => {
	...
	const selectedLot =
		MOCK_LOTS.find((l) => l.id === selectedLotId) || MOCK_LOTS[0];
				...
				<div className="flex-1 bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col min-h-[500px]">
					<div className="p-5 border-b border-gray-200 flex flex-wrap items-center justify-between gap-4">
						<div className="flex items-center gap-3">
							...
							<div className="flex rounded-lg bg-gray-100 p-1">
								<button className="px-3 py-1 text-xs font-bold rounded-md bg-white text-gray-900 shadow-sm transition-all">
									Estado
								</button>
								<button className="px-3 py-1 text-xs font-medium rounded-md text-gray-500 hover:text-gray-700 transition-all">
									Rendimiento
								</button>
							</div>
						</div>
					...
					<div className="flex-1 relative bg-gray-100 p-6 overflow-hidden">
						<div
							...
							style={{
								backgroundImage:
									"radial-gradient(#6f9b69 1px, transparent 1px)",
                    ...
                    `}
									>
										<div
											className={`absolute top-0 right-0 text-white text-[10px] font-bold px-2 py-0.5 rounded-bl-lg uppercase ${tagColors[lot.status]}`}
										>
											{lot.status.replace("-", " ")}
										</div>
										...
										<div className="mt-2">
											<h4
												className={`font-bold ${isSelected ? "text-accent" : "text-gray-900"}`}
											>
												{lot.name}
											</h4>
										...
										{lot.status === "alerta" ? (
											<div className="flex items-center gap-1 text-xs text-red-600 font-bold mt-2">
												<Icon name="water_drop" className="text-sm" filled />{" "}
												{lot.alertType === "riego" ? "Riego" : "Plaga"}
											</div>
											...
											<div className="flex justify-between items-end mt-2">
												...
												<Icon
													name="agriculture"
													className="text-accent animate-pulse text-lg"
												/>
											</div>
										) : (
			...
			<div className="w-full lg:w-96 flex-none bg-white border-l border-gray-200 flex flex-col overflow-y-auto">
				<div className="p-6 border-b border-gray-200 bg-gray-50/50">
					<div className="flex items-center justify-between mb-4">
						<span className="inline-flex items-center rounded-md bg-accent/10 px-2 py-1 text-xs font-bold text-accent ring-1 ring-inset ring-accent/20">
							LOTE SELECCIONADO
						</span>
						...
						<div className="mt-4 grid grid-cols-2 gap-3">
							<div className="bg-white p-2 rounded border border-gray-200">
								...
								<span className="text-sm font-bold text-gray-800">
									{selectedLot.details.responsible}
								</span>
							</div>
							<div className="bg-white p-2 rounded border border-gray-200">
								<span className="text-[10px] uppercase text-gray-400 font-bold block">
						...
						<div className="relative flex gap-4 pb-8 group">
							...
							<div className="flex-1 -mt-1 p-3 bg-green-50 rounded-lg border border-green-100">
								<div className="flex justify-between items-start">
									...
									<span className="text-[10px] font-bold bg-white px-1.5 py-0.5 rounded text-green-700 shadow-sm">
										100%
									</span>
								</div>
								<p className="text-xs text-gray-600 mt-1">
									450 jabas registradas
								</p>
								<div className="mt-2 text-xs text-gray-500 flex items-center gap-1">
									<Icon name="schedule" className="text-[14px]" /> Finalizado:
									09:15 AM
								</div>
							</div>
						</div>
						...
						<div className="relative flex gap-4 pb-8">
							...
							<div className="flex-1 -mt-1 p-3 bg-white rounded-lg border-l-4 border-yellow-500 shadow-md">
								<div className="flex justify-between items-start">
									...
									<span className="text-[10px] font-bold bg-yellow-100 px-1.5 py-0.5 rounded text-yellow-800">
										EN RUTA
									</span>
								</div>
