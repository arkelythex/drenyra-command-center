...
The web app SHALL define itself as an agentic fiscal command center focused on
supervised accounting operations.

#### Requirement 1 acceptance criteria

- The web philosophy names the primary workspace model: navigation, central work
  area, evidence/agent/approval context.
...
Every agentic accounting recommendation in the web app SHALL expose evidence,
confidence, scope, and approval state.
...
The web app SHALL prioritize fiscal/accounting workflows over disconnected
feature pages.

#### Requirement 3 acceptance criteria

- Monthly close, SIRE, reconciliation, and invoice review are treated as
  flagship workflows.
- Navigation and command surfaces support outcome-first entry points.
- Existing feature routes can remain, but should be subordinated to workflow
  goals.

### Requirement 4: Cognitive load reduction

The web app SHALL apply progressive disclosure and review empathy to complex
fiscal workflows.
...
- Web and CLI use compatible concepts for company, RUC, period, evidence,
  approvals, and reversals.
- A workflow started in one surface can be explained from the other surface
