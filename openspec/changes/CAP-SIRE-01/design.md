...
async ({ body, query, set }) => {
  ...
  const fiscalPeriodId = await resolveFiscalPeriodId(query.companyId, query.period);
...
export const evidenceEdges = pgTable("evidence_edges", {
  id: uuid("id").primaryKey().defaultRandom(),
  fromNodeId: uuid("from_node_id").notNull()  // FK to evidence_nodes.id
    .references(() => evidenceNodes.id),
  toNodeId: uuid("to_node_id").notNull()      // FK to evidence_nodes.id
    .references(() => evidenceNodes.id),
...
export function buildSummary(
  ...
  const critical = threshold != null
    ? rows.filter(r => r.status !== "MATCH" && Math.abs(r.difference) >= threshold).length
    : mismatched + missingOnLedger + missingOnSunat;

  return { matched, mismatched, missingOnLedger, missingOnSunat, critical, totalDifference };
}
```
...
export function EvidenceBadge({ source, status, confidence }: EvidenceBadgeProps) {
...
export class SireReconcilerService {
   ...
   */
  static async reconcileUnknown(submissionId: string): Promise<ReconcilerState> {
...
if (isTimeout(error)) {
  await db.update(sireSubmissions)
