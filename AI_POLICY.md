...
> [!IMPORTANT]
> **Fiscal correctness is a product safety requirement.** The Command Center projects the verifiable accounting core into the professional experience: tenants, documents, close, reconciliation, approvals, SUNAT. A contribution that breaks money invariants (BigInt cents, never floats), RUC/period isolation, receipts, the fiscal lifecycle (captura → clasificacion → conciliacion → cierre → declaracion → auditoria), or the human-approval boundary is a product defect regardless of who or what authored it. The contributor owns the submission; AI assistance does not transfer that ownership.
...
- Reviewing and validating every change, claim, and test result — including conformance and migration evidence.
...
- Verifying that fiscal invariants hold: no floats for money, RUC/period scope enforced, receipts issued for every material action, and the Command Center never claims fiscal authority that lives in the consumed `drenyra-ai` contracts.
...
- Using floats for money, skipping RUC scope checks, touching contracts without conformance-vector updates, or bypassing the human-approval boundary.
