...
const testTargets = vitestTargets.length > 0 ? vitestTargets : DEFAULT_TEST_TARGETS;
...
if (!resolvedDatabaseUrl) {
  ...
  console.error(
    `Tried: ${unique([process.env.DATABASE_URL, ...configuredUrls, ...DEFAULT_DB_URLS]).map(redactDatabaseUrl).join(', ')}`
  );
  process.exit(1);
}

console.log(`Using DATABASE_URL=${redactDatabaseUrl(resolvedDatabaseUrl)} for DB tests.`);
