...
interface AccountingJobRunControlPlaneSnapshot {
	traceId: string;
	surfaceId: string;
...
const CONTROL_PLANE_PAYLOAD_KEY = "contextControlPlane";
const TOP_LEVEL_TRACE_ID_KEY = "traceId";
...
function isKnowledgeSourceReference(
	value: unknown,
...
function readControlPlaneSnapshot(
...
function writeControlPlaneSnapshot(
	payload: Record<string, unknown>,
...
function toApprovalState(run: AccountingJobRunRecord): ContextApprovalState {
...
const SUPPORTED_EXECUTABLE_JOBS = ["prepare-sire"] as const;
type SupportedExecutableJob = (typeof SUPPORTED_EXECUTABLE_JOBS)[number];

const ACCOUNTING_JOB_ERRORS = {
...
const TERMINAL_ACCOUNTING_JOB_RUN_STATUSES: AccountingJobRunStatus[] = [
	"COMPLETED",
	"FAILED",
	"CANCELLED",
];

const ACCOUNTING_JOB_RUN_TRANSITIONS: Record<
...
function isTerminalAccountingJobRunStatus(
	status: AccountingJobRunStatus,
): boolean {
