...
export function generateInvoiceXML(data: InvoiceXMLData): string {
	const doc = create({ version: "1.0", encoding: "UTF-8" })
		...
		.up()
		.ele("cbc:IssueDate")
		.txt(data.issueDate.toISOString().split("T")[0])
		.up()
		.ele("cbc:DueDate")
		.txt(data.dueDate.toISOString().split("T")[0])
		.up()
		...
		.txt(data.customer.taxId)
		.up()
		.up()
		.ele("cac:PartyLegalEntity")
		.ele("cbc:RegistrationName")
		.txt(data.customer.legalName)
		.up()
		.up()
		.up()
		.up()
	...
	data.items.forEach((item, index) => {
		doc
			...
			.ele("cbc:PriceAmount", { currencyID: data.currency })
			.txt(item.totalAmount.toFixed(2))
			.up()
