...
import type { ExchangeRateResult, RucValidationResult } from "./sunat-types";

// Simple in-memory cache (can be replaced with Redis in production)
const rucCache = new Map<
	string,
	{ data: RucValidationResult; timestamp: number }
>();
const exchangeRateCache = new Map<
	string,
	{ data: ExchangeRateResult; timestamp: number }
>();
 ...
 */
export async function validateRucWithAPI(
	ruc: string,
	...
	try {
		...
		logger.error(
			...
			"Error validating RUC with external API",
		);
...
export async function getExchangeRateFromAPI(
	...
	try {
		...
		logger.error(
			{ error, queryDate },
			"Error fetching exchange rate from external API",
		);
