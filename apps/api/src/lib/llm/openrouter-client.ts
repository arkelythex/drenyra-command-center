...
async function fetchWithRetries(input: RequestInfo, init: RequestInit, maxAttempts = 4): Promise<Response> {
  let attempt = 0;
  let backoffMs = 400;
...
export async function openRouterChatJson<T>(
  ...
  if (!model) throw new Error("OPENROUTER_DEFAULT_MODEL is required (or pass { model })");
  ...
  if (!res.ok) {
    throw new Error(`OpenRouter error (HTTP ${res.status}): ${text.slice(0, 800)}`);
  }
