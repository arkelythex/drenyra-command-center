...
function checkScalar(
	...
	if (schema.type === "number") {
		...
		if (typeof schema.minimum === "number" && numericValue < schema.minimum) {
			errors.push(
				`${path}: expected number >= ${schema.minimum}, got ${numericValue}`,
			);
		}
		if (typeof schema.maximum === "number" && numericValue > schema.maximum) {
			errors.push(
				`${path}: expected number <= ${schema.maximum}, got ${numericValue}`,
			);
		}
	}
	...
	if (schema.enum && !schema.enum.includes(String(value))) {
		errors.push(
			`${path}: value "${String(value)}" not in enum ${JSON.stringify(schema.enum)}`,
		);
	}
}
