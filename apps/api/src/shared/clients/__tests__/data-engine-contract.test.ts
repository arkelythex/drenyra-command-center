...
const CONTRACT_SCOPE =
	process.env.DATA_ENGINE_CONTRACT_SCOPE === "smoke" ? "smoke" : "full";
...
describe("Data Engine Contract Tests", () => {
	...
	it("validates health contract", async (ctx) => {
		try {
			...
			expect(response.headers.get("x-contract-version")).toBe(
	...
	it("validates cashflow analyze contract", async (ctx) => {
		try {
			...
			const { response, data } =
				await getJson<DataEngineCashflowAnalyzeResponse>(
