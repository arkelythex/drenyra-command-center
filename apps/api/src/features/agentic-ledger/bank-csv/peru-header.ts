 ...
 * ```
 */
export function parsePeruHeaderCsv(
	csvText: string,
	label: string,
): BankCsvParseResult {
	...
	if (lines.length <= 1)
		return { transactions: [], warnings: ["CSV has no data rows"] };
	...
	const idxDesc = findHeaderIndex(header, [
		"descripcion",
		"detalle",
		"concepto",
		"glosa",
	]);
	const idxCargo = findHeaderIndex(header, [
		"cargo",
		"debito",
		"egreso",
		"retiro",
	...
	const idxAbono = findHeaderIndex(header, [
		"abono",
		"credito",
		"ingreso",
		"deposito",
	...
	const idxMonto = findHeaderIndex(header, [
		"monto",
		"importe",
		"importeoperacion",
		"importeoperaciones",
	]);
	const idxTipo = findHeaderIndex(header, [
		"tipo",
		"tipomovimiento",
		"naturaleza",
	]);
	const idxRef = findHeaderIndex(header, [
		"nrooperacion",
		"numerooperacion",
		"operacion",
		"referencia",
	...
	if (idxDesc === -1)
		warnings.push(`${label} CSV: description column not detected`);
	...
	for (let i = 1; i < lines.length; i++) {
		...
		const cargo =
			idxCargo >= 0 ? parseFlexibleAmount(row[idxCargo] ?? "") : null;
		const abono =
			idxAbono >= 0 ? parseFlexibleAmount(row[idxAbono] ?? "") : null;
		const monto =
			idxMonto >= 0 ? parseFlexibleAmount(row[idxMonto] ?? "") : null;
		...
		if (cargo !== null || abono !== null) {
			...
			const typeHint =
				idxTipo >= 0 ? (row[idxTipo] ?? "").trim().toLowerCase() : "";
			if (typeHint) {
				if (/(cargo|debito|egreso|retiro|debe|d)/i.test(typeHint))
					type = "DEBIT";
				if (/(abono|credito|ingreso|deposito|haber|c)/i.test(typeHint))
					type = "CREDIT";
			}
