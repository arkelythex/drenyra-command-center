...
const wasmSkillService = new AgenticLedgerWasmSkillService(new TransactionService());
...
export const agenticLedgerModule = new Elysia({ prefix: "/api/agentic-ledger" })
  .onError(({ code, set }) => {
    if (code === "VALIDATION") {
      set.status = 422;
      return fail("Invalid agentic ledger request", "VALIDATION_ERROR");
    }
    return;
  })
  .post(
    ...
    async ({ body, set }) => {
      try {
        const { imported, duplicates, warnings } = await service.ingestBank(body);
        return { imported, duplicates, warnings };
      } catch (error) {
        set.status = 400;
        return {
          message: error instanceof Error ? error.message : "Invalid ingest request",
        };
      }
    },
    {
      ...
      detail: {
        ...
        description: "Importa transacciones bancarias (CSV/normalizadas) para alimentar el Agentic Ledger.",
      },
    }
  )
  .post(
    ...
    async ({ body, set }) => {
      try {
        ...
        return {
          success: false,
          error: error instanceof Error ? error.message : "Invalid WASM skill request",
        };
      }
    },
    {
