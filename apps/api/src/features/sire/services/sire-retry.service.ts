    ...
    for (const submission of failedSubmissions) {
      try {
        ...
        const retryResult = await SireSubmissionService.submit({
          ...
          payloadFormat: submission.payloadFormat as 'txt' | 'csv' | 'json' | 'xml',
        ...
        await sireSubmissionRepository.update(submission.id, {
          status:
            retryResult.status === 'ACCEPTED' || retryResult.status === 'SIMULATED'
              ? 'ACCEPTED'
              : 'FAILED',
        ...
        const nextRetryMinutes = Math.pow(
          2,
          (submission.attemptNumber ?? 1) + 1
        ); // Exponential backoff
