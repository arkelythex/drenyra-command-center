...
export class SireRegisterExportService {
   ...
   */
  static async generateSalesRegister(options: SIREExportOptions): Promise<string | Buffer> {
    ...
    const records: SIRESalesRecord[] = invoicesList.map((invoice, index) => {
      ...
      const exchangeRate = invoice.currency === 'USD' ? parseFloat(invoice.exchangeRate) : 1;
      ...
      return {
        ...
        tipoComprobante: SireRegisterExportService.getDocumentType(invoice.invoiceNumber),
    ...
    if (options.format === 'EXCEL') {
      return SireRegisterExportService.exportSalesToExcel(records, { year, month });
    }
   ...
   * @returns TXT string or Excel Buffer
   */
  static async generatePurchasesRegister(options: SIREExportOptions): Promise<string | Buffer> {
    const { year, month, companyId } = options;
    ...
    const records: SIREPurchasesRecord[] = billsList.map((bill, index) => {
      ...
      const subtotal = Money.fromAmount(parseFloat(bill.subtotalAmount), currency);
      ...
      const exchangeRate = bill.currency === 'USD' ? parseFloat(bill.exchangeRate) : 1;
      ...
      return {
        ...
        tipoComprobante: SireRegisterExportService.getDocumentType(bill.billNumber),
    ...
    if (options.format === 'EXCEL') {
      return SireRegisterExportService.exportPurchasesToExcel(records, { year, month });
    }
   ...
   */
  static validateSIREFormat(content: string, type: 'sales' | 'purchases'): SIREValidationResult {
    ...
    return { isValid: errors.length === 0, errors, warnings, recordCount: lines.length };
  }
  ...
  private static exportSalesToTXT(records: SIRESalesRecord[]): string {
    return records.map(r => [
      r.periodo, r.correlativo, r.fechaEmision, r.tipoComprobante,
      r.serie, r.numero, r.numeroFinal, r.tipoDocumentoCliente,
      r.numeroDocumentoCliente, r.razonSocialCliente,
      ...
      r.fechaEmisionModificado, r.tipoComprobanteModificado,
      r.serieModificado, r.numeroModificado, r.estado,
    ].join('|')).join('\n');
  }

  private static exportPurchasesToTXT(records: SIREPurchasesRecord[]): string {
    return records.map(r => [
      r.periodo, r.correlativo, r.fechaEmision, r.fechaVencimiento,
      r.tipoComprobante, r.serie, r.anoDUA, r.numeroComprobante, r.numeroFinal,
      r.tipoDocumentoProveedor, r.numeroDocumentoProveedor, r.razonSocialProveedor,
      ...
      SireRegisterExportService.formatAmount(r.tipoCambio, 3),
      r.fechaEmisionModificado, r.tipoComprobanteModificado,
      r.serieModificado, r.numeroDUAModificado, r.numeroModificado,
      r.fechaDetraccion, r.numeroDetraccion, r.retencion,
      r.estado, r.clasificacionBienes,
    ].join('|')).join('\n');
  }
  ...
  private static async exportPurchasesToExcel(
    ...
    worksheet.columns = [
      ...
      { header: 'Tipo Doc Proveedor', key: 'tipoDocumentoProveedor', width: 18 },
