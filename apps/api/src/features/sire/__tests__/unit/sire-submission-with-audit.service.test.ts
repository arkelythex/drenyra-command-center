...
vi.mock('../../services/tenant-sunat-context.service', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../services/tenant-sunat-context.service')>();
  return {
    ...actual,
...
describe('SIRE submission audit idempotency scope', () => {
  ...
  it('does not update another company submission when logging a blocked attempt', async () => {
    ...
    ).rejects.toThrow('Forbidden SIRE idempotency key belongs to another company');
  ...
  it('resolves tenant SUNAT context before API submission and passes it to submission service', async () => {
    ...
    vi.mocked(sireSubmissionRepository.findByIdempotencyKey).mockResolvedValue(null);
    vi.mocked(sireSubmissionRepository.create).mockResolvedValue(auditRecord as never);
  ...
  it('fails closed and records safe audit metadata when tenant context cannot be resolved', async () => {
    ...
    expect(JSON.stringify(vi.mocked(sireSubmissionRepository.update).mock.calls)).not.toContain(
      'api-token',
    );
  });
  ...
  it('fails closed on submitted payload RUC mismatch before submission service call', async () => {
    ...
    ).rejects.toThrow('SUNAT supplied RUC does not match authenticated company');
