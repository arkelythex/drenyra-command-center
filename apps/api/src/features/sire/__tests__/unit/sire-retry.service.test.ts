...
describe('SireRetryService', () => {
  ...
  it('retries eligible failed submissions and marks them as accepted when retry succeeds', async () => {
    vi.mocked(sireSubmissionRepository.getFailedSubmissionsForRetry).mockResolvedValue([
      {
        id: 'sub-1',
        companyId: 'cmp-1',
        period: '2026-02',
    ...
    expect(sireSubmissionRepository.incrementAttempt).toHaveBeenCalledWith('sub-1');
