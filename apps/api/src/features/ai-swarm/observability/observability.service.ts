...
export class AiObservabilityService {
	...
	static async listRuns(
		...
		return await db.query.agentRunStates
			.findMany({
			...
			.then((rows) =>
				rows.map((row) => ({
					...row,
					context: row.context
						? (sanitizeAiObservationPayload(row.context) as Record<
								string,
								unknown
							>)
						: row.context,
	...
	static async getRunEvents(
		...
		return await db.query.agentRunEvents
			.findMany({
			...
			.then((rows) =>
				rows.map((row) => ({
					...row,
					payload: row.payload
						? (sanitizeAiObservationPayload(row.payload) as Record<
								string,
								unknown
							>)
						: row.payload,
	 ...
	 */
	static async getCompanyMemory(companyId: string): Promise<{
		summary: string | null;
		recentRuns: number;
		companyId: string;
	}> {
	 ...
	 */
	static async getMemoryHistory(companyId: string): Promise<
		Array<{
			runId: string;
	...
	> {
		...
		return rows.map((row) => ({
			runId: row.runId,
			memorySummary:
				((row.context as Record<string, unknown>)?.memorySummary as string) ??
				"",
	...
	static async getBatch(
		...
		const row = await db.query.batchRuns.findFirst({
			where: and(eq(batchRuns.id, batchId), eq(batchRuns.companyId, companyId)),
			with: { items: true },
		});
		if (!row) return null;
		return {
