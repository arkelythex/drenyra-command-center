...
export const aiObservabilityModule = new Elysia({
	...
	.get(
		...
		async ({ headers, set }) => {
			...
			try {
				const data = await AiObservabilityService.getSummary(
	...
	.post(
		...
		{
			body: CreateBatchBodyOpenApi,
			detail: { tags: ["Observability"], summary: "Submit batch" },
		},
	)
	...
	.get(
		...
		{
			query: BatchListQueryOpenApi,
			detail: { tags: ["Observability"], summary: "List batches" },
		},
	)
	...
	.get(
		...
		{
			params: BatchParamsOpenApi,
			detail: { tags: ["Observability"], summary: "Get batch detail" },
		},
	)
	...
	.post(
		...
		async ({ params, headers, set }) => {
			...
			try {
				await AiObservabilityService.cancelBatch(
					parsed.data.batchId,
					authz.actor.companyId,
				);
