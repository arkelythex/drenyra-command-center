...
import type { CompleteProcessingInput, CompleteProcessingOutput } from './complete-invoice-processing.workflow';
...
export class MultiRucProcessingWorkflow {
  ...
  async execute(input: MultiRucInput): Promise<MultiRucOutput> {
    ...
    logger.info({ totalDocuments }, 'Calculated total documents for multi-RUC workflow');
  ...
  generateReport(output: MultiRucOutput): {
    ...
    const averageCostPerInvoice = output.execution.totalCostUsd / totalInvoicesProcessed;
    const processingTime = `${(output.execution.totalDurationMs / 1000).toFixed(2)}s`;
    const successRate =
      output.successfulCompanies / output.totalCompanies;
