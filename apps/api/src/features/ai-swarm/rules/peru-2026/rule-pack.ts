...
export const PERU_FISCAL_THRESHOLDS_2026: PeruFiscalThresholds2026 = {
	...
	defaultBancarizacionMinimumPen: thresholdFromEnv(
		"SUNAT_BANCARIZACION_MIN_PEN",
		2000,
	),
...
export function getPeruRulePack2026(): {
	...
	const activeRuleCount = PERU_RULE_PACK_2026.filter(
 ...
 * ```
 */
export function buildPolicyWarnings2026(
	invoice: InvoiceData,
): ValidationWarning[] {
	...
	const likelyServiceInvoice = invoice.items.some((item) =>
		/servicio|consultor/i.test(item.descripcion),
	);
	if (
		likelyServiceInvoice &&
		...
		warnings.push({
			...
			suggestion:
				"Corroborar tipo de operación y tasa de detracción aplicable.",
		});
	}
	...
	if (invoice.fecha >= thresholds.sireMandatoryFrom) {
		warnings.push({
			...
			message:
				"Periodo alcanzó ventana SIRE junio 2026; confirmar propuesta RVIE/RCE antes del cierre.",
			suggestion:
				"Ejecutar conciliación contra propuesta SIRE y revisar diferencias.",
		});
	}
