...
function isLikelyServiceInvoice(invoice: InvoiceData): boolean {
	return invoice.items.some((item) =>
		/servicio|consultor|asesori|honorario|mantenimiento/i.test(
			item.descripcion,
		),
	);
}
 ...
 * ```
 */
export function runSunatAiParitySubagent(
	invoice: InvoiceData,
): SunatParityResult {
	...
	const expectedIgv =
		Math.round(invoice.subtotal * thresholds.igvRate * 100) / 100;
	...
	if (
		isLikelyServiceInvoice(invoice) &&
		invoice.total >= thresholds.detraccionMinimumPen
	) {
	...
	if (issueDate <= adaptationEnd) {
		alerts.push({
			...
			suggestion:
				"Regularizar diferencias antes del cierre para evitar contingencias futuras.",
			relatedRuleIds: [],
		});
	}
	...
	const ruleIdsByDomain = new Map<PeruRuleDomain, string[]>(
		[...neededDomains].map((domain) => [
			domain,
			buildRuleIdsByDomain(relevantRules, domain),
		]),
	);

	const alertsWithRules = alerts.map((alert) => {
		const domain = alert.code.includes("IGV")
			? "igv"
...
export async function buildSunatRagContextAsync(
	...
	try {
		const { sunatKnowledgeService } = await import(
			"@drenyra/ai/services/sunat-knowledge"
		);
		...
		if (
			invoice.items.some((i) =>
				i.descripcion.toLowerCase().includes("servicio"),
			)
		) {
 ...
 * ```
 */
export function mapParityAlertsToValidation(alerts: SunatParityAlert[]): {
