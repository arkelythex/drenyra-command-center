  ...
  .get(
    ...
    async ({ query, set }) => {
      ...
      return {
        ...
        data: {
          ...
          budget: {
            daily: dbSummary.totalEvents > 0 ? dbSummary.daily : memoryUsage.daily,
            monthly: dbSummary.totalEvents > 0 ? dbSummary.monthly : memoryUsage.monthly,
          },
          meta: {
    ...
    },
    {
      ...
      detail: {
        ...
        description: 'Retorna métricas de gasto consolidadas (DB histórico + memoria sesión).',
      },
    }
  )
