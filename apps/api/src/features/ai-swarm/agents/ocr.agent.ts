...
import type { InvoiceData, AgentResult } from '../config/types';
  ...
  async extractInvoice(
    ...
    if (!hasOpenRouterKey()) {
      return {
        ...
        error: {
          code: 'OCR_NO_API_KEY',
          message: 'OPENROUTER_API_KEY not configured. OCR agent requires API key.',
        },
        metadata: {
          agentType: 'ocr',
          modelUsed: 'none',
...
`;
  ...
  async extractBatch(
    ...
    return Promise.all(images.map((img) => this.extractInvoice(img.url, img.id)));
  }
}
