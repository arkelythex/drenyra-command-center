...
export class SUNATAgent {
   ...
   */
  async validateInvoice(invoice: InvoiceData): Promise<AgentResult<ValidationResult>> {
    const startTime = Date.now();

    try {
      // Step 1: Rule-based validation (fast, deterministic)
      const { errors: ruleErrors, warnings: ruleWarnings } = this.validateWithRules(invoice);
  ...
  private validateWithRules(invoice: InvoiceData): {
    ...
    const expectedIGV = Math.round(invoice.subtotal * PERU_RULE_PACK.thresholds.igvRate * 100) / 100;
    if (Math.abs(invoice.igv - expectedIGV) > PERU_RULE_PACK.thresholds.amountTolerancePen) {
      errors.push({
        field: 'igv',
        code: 'INVALID_IGV',
    ...
    if (Math.abs(invoice.total - expectedTotal) > PERU_RULE_PACK.thresholds.amountTolerancePen) {
      errors.push({
        field: 'total',
        code: 'INVALID_TOTAL',
