	...
	asientos: z.array(
		z.object({
			...
			evidence: z
				.string()
				.optional()
				.describe("Justificación de la clasificación"),
		}),
	),
		...
		if (!hasOpenRouterKey()) {
			return {
				...
				error: {
					...
					message:
						"OPENROUTER_API_KEY not configured. PCGE agent requires API key.",
				},
				metadata: {
					agentType: "pcge",
					modelUsed: "none",
...
`;
	...
	async classifyBatch(
		...
		return Promise.all(
			invoices.map((invoice) => this.classifyInvoice(invoice)),
		);
	}
