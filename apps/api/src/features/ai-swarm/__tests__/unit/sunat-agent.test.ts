...
type EdgeCase = {
  name: string;
  invoice: InvoiceData;
  expectedErrorCodes: Array<'INVALID_RUC' | 'INVALID_IGV' | 'INVALID_TOTAL' | 'INVALID_SERIE_FORMAT'>;
};
...
describe('SUNATAgent', () => {
  ...
  it('covers 100 edge cases for fiscal validator reliability', async () => {
    ...
    const edgeCases = [...igvCases, ...totalCases, ...invalidRucCases, ...invalidSerieCases];
  ...
  it('keeps false positives under 1% for 100 valid invoices', async () => {
    const validCases: InvoiceData[] = Array.from({ length: 100 }, (_, index) => {
    ...
    const results = await Promise.all(validCases.map((invoice) => agent.validateInvoice(invoice)));
    const falsePositives = results.filter((result) => !result.data?.isValid).length;
