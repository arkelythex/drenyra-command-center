...
describe('SUNATAgent AI path', () => {
  ...
  it('merges deterministic rule result with AI warnings and cost metadata', async () => {
    ...
    expect(result.data?.warnings.some((warning) => warning.code === 'BANKING_REQUIRED')).toBe(true);
    expect(result.data?.warnings.some((warning) => warning.code === 'SUNAT_AI_SIRE_ADAPTATION')).toBe(true);
