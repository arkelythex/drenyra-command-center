...
import { and, batchRunItems, batchRuns, db, eq } from "@drenyra/infrastructure";
...
describeDb("Batch API (DB integration)", () => {
	...
	afterEach(async () => {
		...
		for (const batchId of batchIds) {
			await db.delete(batchRunItems).where(eq(batchRunItems.batchId, batchId));
			await db.delete(batchRuns).where(eq(batchRuns.id, batchId));
		}
		batchIds.length = 0;
	});
