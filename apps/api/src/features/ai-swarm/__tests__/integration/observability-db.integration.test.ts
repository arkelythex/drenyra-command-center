...
describeDb("Observability API (DB integration)", () => {
	...
	afterEach(async () => {
		for (const runId of runIds) {
			await db.delete(agentRunEvents).where(eq(agentRunEvents.runId, runId));
			await db.delete(agentRunStates).where(eq(agentRunStates.runId, runId));
		}
		runIds.length = 0;
	});
	...
	it("should filter runs by status", async () => {
		...
		const failed = await db
			.select()
			.from(agentRunStates)
			.where(eq(agentRunStates.status, "failed"));
		...
		const running = await db
			.select()
			.from(agentRunStates)
			.where(eq(agentRunStates.status, "running"));
