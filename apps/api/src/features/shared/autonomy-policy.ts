...
function hasValidApproval(approval?: AutonomyApproval): boolean {
  ...
  return approval.approvedBy.trim().length > 0 && approval.reason.trim().length > 0;
}
...
export class AutonomyPolicyService {
  static evaluate(input: AutonomyEvaluationInput): AutonomyEvaluationResult {
    ...
    if (config.globalKillSwitch) {
      return AutonomyPolicyService.block({
        ...
        message: 'Autonomous execution is globally disabled (kill switch active).',
    ...
    if (input.approval && !approvalValid) {
      rules.push({
        ...
        passed: false,
        reason: 'Approval payload is malformed (approvedBy and reason are required)',
      });
    ...
    if (config.requireApprovalForCritical) {
      ...
      if (!criticalAllowed) breaches.push('critical execution requires approval');
    }
    ...
    if (typeof input.estimatedAmountPen === 'number') {
      ...
      if (!amountAllowed) breaches.push('amount exceeds auto-execution threshold');
    }

    if (typeof input.riskScore === 'number') {
      const riskAllowed = input.riskScore <= config.maxRiskScore || approvalValid;
      rules.push({
    ...
    return AutonomyPolicyService.allow({
      ...
      config,
      rules,
      reason: approvalValid ? 'Allowed with human approval' : 'Allowed by policy',
    });
  }
  ...
  private static readConfig(): AutonomyConfig {
    return {
      ...
      globalKillSwitch: parseBoolean(process.env.AUTONOMY_GLOBAL_KILL_SWITCH, false),
