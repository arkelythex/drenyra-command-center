...
export class PrometeoService {
	...
	async login(credentials: BankCredentials): Promise<string> {
		...
		if (!response.ok) {
			throw new Error(
				await this.extractHttpError(response, "Prometeo login failed"),
			);
		}
	...
	async getAccounts(
		...
		if (!response.ok) {
			throw new Error(
				await this.extractHttpError(response, "Failed to fetch accounts"),
			);
		}
	...
	async getMovements(
		...
		if (!response.ok) {
			throw new Error(
				await this.extractHttpError(response, "Failed to fetch movements"),
			);
		}
	...
	async getPersonalInfo(
		...
		if (!response.ok) {
			throw new Error(
				await this.extractHttpError(response, "Failed to fetch personal info"),
			);
		}
		...
		return {
			name: data.name,
			documentType: data.document_type as
				| "dni"
				| "pasaporte"
				| "carne_extranjeria",
	...
	private async extractHttpError(
		response: Response,
		fallback: string,
	): Promise<string> {
		try {
