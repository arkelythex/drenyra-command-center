...
function validateProviderSpecificFields(input: {
  ...
  if ((input.documentType && !input.documentNumber) || (!input.documentType && input.documentNumber)) {
...
export const bankingProvidersRoutes = new Elysia({ prefix: '/api/banking-providers' })
  .onError(({ code, set }) => {
    if (code === 'VALIDATION') {
      set.status = 422;
      return fail('Invalid banking providers request', 'VALIDATION_ERROR');
    }
    return;
  })
  /**
  ...
  .post(
    ...
    async ({ body, set }) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : 'Failed to connect to bank';
