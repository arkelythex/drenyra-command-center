...
describe("CashflowProjection", () => {
	...
	const createInflow = (
		amount: number,
		dueDate: Date,
		status: CashflowItem["status"] = "pending",
	): CashflowItem => ({
		id: `inflow-${amount}`,
		type: "inflow",
	...
	describe("weekly breakdown", () => {
		it("should group items by week", () => {
			...
			const inflows = [createInflow(1000, week1), createInflow(2000, week2)];

			const outflows = [createOutflow(500, week1), createOutflow(700, week2)];
