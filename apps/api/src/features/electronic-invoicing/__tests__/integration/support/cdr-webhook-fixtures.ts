...
export async function cleanupCdrWebhookFixtures(
  ...
  for (const fixture of fixtures.splice(0)) {
    ...
    await db.delete(transactions).where(eq(transactions.id, fixture.transactionId));
    await db.delete(businessPartners).where(eq(businessPartners.id, fixture.customerId));
