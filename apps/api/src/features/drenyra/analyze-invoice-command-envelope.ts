...
export function createAnalyzeInvoiceCommandEnvelope(
	...
	const evidenceId = input.invoiceId
		? `invoice-${input.invoiceId}`
		: "invoice-analysis-draft";
	return createDrenyraCommandEnvelope({
		...
		deterministicChecks: [
			...
			},
			{
				...
				summary:
					"Invoice requires deterministic CPE/UBL validation before material use",
				evidenceIds: [evidenceId],
			},
		],
		approval: {
			...
			status: "not_required",
			summary:
				"Invoice analysis is advisory and does not create ledger entries",
		},
		diff: {
			kind: "risk_profile",
			summary:
				"Draft invoice risk profile prepared without changing fiscal state",
			after: { analyzedInvoiceId: evidenceId },
		},
		trace: {
