...
export function createReviewSunatCommandEnvelope(
	...
	const evidenceId = input.caseId ? `sunat-${input.caseId}` : "sunat-review-snapshot";
	return createDrenyraCommandEnvelope({
		...
		riskLevel: "MEDIUM",
		evidence: [{ id: evidenceId, type: "SUNAT_RECORD", title: "SUNAT scoped review snapshot", sourceRef: input.sourceRef }],
		deterministicChecks: [
			{
				id: "ruc-checksum",
				label: "RUC checksum",
		...
		approval: { required: false, status: "not_required", summary: "Review envelope is advisory and does not submit to SUNAT" },
		diff: { kind: "none", summary: "No fiscal state mutation proposed by review-sunat" },
