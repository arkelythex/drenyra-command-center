...
function auditEvent(
	input: Partial<AuditEvent> & Pick<AuditEvent, "id" | "eventType">,
): AuditEvent {
	return {
		caseId: input.caseId,
...
async function buildApp() {
	...
	await repository.createAuditEvent(
		auditEvent({ id: "audit-allowed", eventType: "CAPABILITY_ALLOWED" }),
	);
	await repository.createAuditEvent(
		auditEvent({ id: "audit-denied", eventType: "CAPABILITY_DENIED" }),
	);
	await repository.createAuditEvent(
		auditEvent({
			id: "audit-case",
	...
	return new Elysia({ prefix: "/api/drenyra" }).use(
		createCommandEnvelopeAuditRoutes({ commandCenter }),
	);
}
...
describe("Drenyra command envelope audit routes", () => {
	it("lists scoped case-less capability audit events", async () => {
		...
		const response = await app.handle(
			new Request("http://localhost/api/drenyra/command-envelope/audit", {
		...
		expect(
			payload.data.events.map((event: AuditEvent) => event.id),
		).not.toContain("audit-other-scope");
	});
