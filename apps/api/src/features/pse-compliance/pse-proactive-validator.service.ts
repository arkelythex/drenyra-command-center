...
): Promise<SubagentCheck> {
	const expectedIgv = Number((input.ple.salesTotalPen * 0.18).toFixed(2));
	const gap = Number(
		Math.abs(input.pdt.declaredIgvPen - expectedIgv).toFixed(2),
	);
...
async function runPdtSubagent(
	...
	const hasSalesGap =
		Math.abs(input.ple.salesTotalPen - input.pdt.declaredNetSalesPen) > 5;
	...
	return {
		...
		evidence: {
			...
			isFuturePeriod,
			salesGapPen: Number(
				(input.ple.salesTotalPen - input.pdt.declaredNetSalesPen).toFixed(2),
			),
			form: input.pdt.form,
		},
	};
...
function buildFallbackAiAlerts(
	checks: SubagentCheck[],
	reason: string,
): AiAlert[] {
