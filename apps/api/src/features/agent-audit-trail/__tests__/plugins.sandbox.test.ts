...
import {
	...
	createSandboxContextData,
} from "../plugins/plugin-sandbox";
...
describe("audit plugin sandbox", () => {
	it("blocks plugin paths outside approved roots", () => {
		expect(() =>
			assertPluginSandboxConstraints({
				...
				conditions: [
					{ kind: "path", path: "secrets.apiKey", operator: "exists" },
				],
				finding: {
					code: "SECRET_LEAK",
	...
	it("builds isolated context with allowed paths only", () => {
		...
		expect(
			(sandbox as { inputs?: Record<string, unknown> }).inputs?.hiddenField,
		).toBeUndefined();
	});
