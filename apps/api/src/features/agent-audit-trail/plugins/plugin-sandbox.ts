...
function isSafePath(path: string): boolean {
  return !['__proto__', 'prototype', 'constructor'].some((segment) => path.includes(segment));
}
...
function resolvePath(contextData: Record<string, unknown>, path: string): unknown {
 ...
 * ```
 */
export function assertPluginSandboxConstraints(plugin: AuditPluginDefinition): void {
  if (plugin.conditions.length > MAX_CONDITIONS) {
    throw new Error(`Plugin "${plugin.id}" exceeds sandbox condition limit (${MAX_CONDITIONS})`);
  }

  if (plugin.allowedPaths.length > MAX_ALLOWED_PATHS) {
    throw new Error(`Plugin "${plugin.id}" exceeds sandbox allowedPaths limit (${MAX_ALLOWED_PATHS})`);
  }
  const wildcardPaths = plugin.allowedPaths.filter((path) => path.trim().endsWith('.*'));
  if (wildcardPaths.length > MAX_WILDCARD_PATHS) {
    throw new Error(`Plugin "${plugin.id}" exceeds wildcard path limit (${MAX_WILDCARD_PATHS})`);
  }

  for (const rawPath of plugin.allowedPaths) {
    const path = normalizePath(rawPath);
    if (!isPathPatternValid(path)) {
      throw new Error(`Plugin "${plugin.id}" contains invalid path pattern: "${rawPath}"`);
    }

    const root = getRoot(path);
    if (!ALLOWED_ROOTS.has(root)) {
      throw new Error(`Plugin "${plugin.id}" cannot access root path "${root}"`);
    }
  }
}
