...
const toDomain = (row: typeof evidence.$inferSelect): Evidence =>
	Evidence.fromPrimitives({
		...
		hash: row.hash,
		...(row.hashChain !== null ? { hashChain: row.hashChain } : {}),
		evidenceType: row.evidenceType,
		source: row.source,
		status: row.status,
		...(row.metadata !== null ? { metadata: row.metadata } : {}),
		...(row.extractedData !== null ? { extractedData: row.extractedData } : {}),
		...(row.classifierResult !== null
			? { classifierResult: row.classifierResult }
			: {}),
		...(row.validatedAt !== null ? { validatedAt: row.validatedAt.toISOString() } : {}),
		...(row.validatedBy !== null ? { validatedBy: row.validatedBy } : {}),
