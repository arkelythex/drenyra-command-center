...
export class TransactionReference {
  ...
  static normalize(reference: string): string {
    ...
    for (const [type, pattern] of Object.entries(TransactionReference.PATTERNS)) {
      const match = cleaned.match(pattern);
      if (match) {
        const prefix = type === 'INVOICE' ? 'F' : type === 'BOLETA' ? 'BV' : 'B';
  ...
  static detectType(reference: string): 'INVOICE' | 'BILL' | 'BOLETA' | 'OTHER' {
  ...
  containsIn(text: string): boolean {
    ...
    return upperText.includes(this.normalized) || 
