...
export class BankTransaction {
  ...
  static create(props: BankTransactionProps, currency: Currency): BankTransaction {
   ...
   * ```
   */
  linkToDocument(documentId: string, documentType: DocumentType, matchScore: MatchScore): BankTransaction {
  ...
  isWithinDateRange(targetDate: Date, windowDays: number): boolean {
    const diff = Math.abs(this.transactionDate.getTime() - targetDate.getTime());
