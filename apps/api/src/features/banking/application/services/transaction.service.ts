...
export class TransactionService {
  ...
  async createTransaction(companyId: string, data: CreateTransactionDTO) {
    ...
    const currentBalance = Money.fromAmount(Number(account.currentBalance), currency);
    const delta = Money.fromAmount(data.amount, currency);
    const newBalance = data.type === 'CREDIT' ? currentBalance.add(delta) : currentBalance.subtract(delta);
    ...
    const transaction = await this.repository.createTransaction(transactionInput);

    await this.repository.updateAccount(account.id, { currentBalance: balanceValue });
  ...
  async reconcileTransaction(
    ...
    await this.repository.reconcileTransaction(transactionId, userId, documentId, documentType);
  }

  async importTransactions(companyId: string, accountId: string, transactions: ImportTransactionInput[]) {
    let imported = 0;
