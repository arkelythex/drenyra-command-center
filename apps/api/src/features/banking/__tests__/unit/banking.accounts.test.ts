...
const { BankingApplicationService } = await import('../../application/services/banking.application-service');
...
describe('BankingApplicationService / Account Management', () => {
  ...
  it('createAccount delegates to AccountService', async () => {
    ...
    expect(result).toEqual({ id: 'acc-1' });
    expect(mocks.accountService.createAccount).toHaveBeenCalledWith('comp-1', { accountName: 'Main' });
  });
  ...
  it('updateAccount delegates to AccountService', async () => {
    ...
    expect(mocks.accountService.updateAccount).toHaveBeenCalledWith('acc-1', { accountName: 'Updated' });
  });
  ...
  it('getBalance delegates to AccountService', async () => {
    mocks.accountService.getBalance.mockResolvedValue({ current: '10.00', available: '8.00' });
