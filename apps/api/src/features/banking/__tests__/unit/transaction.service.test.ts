...
const { TransactionService } = await import('../../application/services/transaction.service');
type CreateTransactionInput = Parameters<TransactionService['createTransaction']>[1];
type CreateTransactionResult = Awaited<ReturnType<TransactionService['createTransaction']>>;
...
describe('TransactionService', () => {
  ...
  it('lists transactions with date filters', async () => {
    ...
    expect(mockRepository.findTransactions).toHaveBeenCalledWith('acc-1', { startDate: start, endDate: end });
  });
  ...
  it('reconciles a transaction', async () => {
    ...
    expect(mockRepository.reconcileTransaction).toHaveBeenCalledWith('tx-1', 'user-1', undefined, undefined);
  });
  ...
  it('imports transactions and continues on error', async () => {
    ...
    const imported = await service.importTransactions('comp-1', 'acc-1', [
      { date: new Date(2025, 0, 1), description: 'A', amount: 10, type: 'CREDIT' },
      { date: new Date(2025, 0, 2), description: 'B', amount: 20, type: 'DEBIT' },
    ]);
