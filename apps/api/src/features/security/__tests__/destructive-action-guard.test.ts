...
describe('destructive-action-guard', () => {
  it('blocks 15 destructive attack prompts for non-admin roles', () => {
    ...
    const blocked = attackPrompts.map((prompt) =>
      guardDestructivePrompt([{ role: 'user', content: prompt }], 'analyst', false),
    );
  ...
  it('allows fiscal override only with privileged role + explicit override header', () => {
    ...
    const denied = guardDestructivePrompt([{ role: 'user', content: prompt }], 'analyst', false);
    const allowed = guardDestructivePrompt([{ role: 'user', content: prompt }], 'admin', true);
  ...
  it('allows non-destructive prompts', () => {
    const result = guardDestructivePrompt(
      [{ role: 'user', content: 'calcula detraccion de una factura de S/ 1180' }],
      'analyst',
      false,
    );
