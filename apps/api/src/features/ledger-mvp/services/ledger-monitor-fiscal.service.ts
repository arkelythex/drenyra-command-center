...
>;
type ProactiveValidationAlert = ProactiveValidationResult['proactiveAlerts'][number];
...
function resolveAlertCategory(alert: ProactiveValidationAlert): LedgerMonitorFiscalAlertCategory {
  const normalizedText = `${alert.message} ${alert.action}`.toLowerCase();
  if (normalizedText.includes('igv')) return 'igv';
  if (normalizedText.includes('rvie') || normalizedText.includes('rce')) return 'rce';
  if (normalizedText.includes('pdt') || normalizedText.includes('ruc')) return 'pdt';
  if (
...
export class LedgerMonitorFiscalService {
  ...
  async run(input: LedgerMonitorFiscalInput): Promise<LedgerMonitorFiscalResult> {
