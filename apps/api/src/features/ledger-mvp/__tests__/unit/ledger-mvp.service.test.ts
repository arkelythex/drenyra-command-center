...
function createProfitLoss(overrides?: Partial<ProfitLossReport>): ProfitLossReport {
  return {
    period: {
...
function createDashboard(overrides?: Partial<ComplianceDashboard>): ComplianceDashboard {
  return {
    score: 95,
    totalIssues: 0,
    criticalIssues: 0,
...
function createService() {
  const verifySireReproducibility = vi.fn<
    (input: {
      companyId: string;
      year: number;
      month: number;
  ...
  const getIgvSummary = vi.fn<
    (companyId: string, year: number, month: number) => Promise<IGVSummary>
  >();
  const getSireSummary = vi.fn<
    (companyId: string, year: number, month: number) => Promise<SIRESummary>
  >();
  const getSunatLiveSummary = vi.fn<
    (input: { companyId: string; period: string; ruc: string }) => Promise<SIRESunatLiveSummary>
  >();
  ...
  const getBalanceSheet = vi.fn<
    (companyId: string, asOfDate: Date) => Promise<BalanceSheetReport>
  >();
  ...
  const getComplianceDashboard = vi.fn<
    (companyId: string) => Promise<ComplianceDashboard>
  >();

  const getComplianceIssues = vi.fn<
    (companyId: string) => Promise<ComplianceIssue[]>
  >();

  const validatePseCompliance = vi.fn<
    (
...
describe('LedgerMvpService', () => {
  ...
  it('returns ready for SIRE autopilot when reconciliation is reproducible', async () => {
    ...
    ctx.verifySireReproducibility.mockResolvedValue(createReproducibilityReport());
    ...
    ctx.getSunatLiveSummary.mockResolvedValue({
      ...
      ledgers: [
        { ledgerType: 'ventas', recordCount: 10, totalAmount: 1000, totalIGV: 180 },
  ...
  it('returns manual_review when SUNAT live summary diverges from local totals', async () => {
    ...
    ctx.getSunatLiveSummary.mockResolvedValue({
      ...
      message: 'Resumen SUNAT API consultado en tiempo real.',
      ledgers: [
        { ledgerType: 'ventas', recordCount: 9, totalAmount: 980, totalIGV: 176.4 },
        { ledgerType: 'compras', recordCount: 0, totalAmount: 0, totalIGV: 0 },
      ],
    });
    ctx.pdtBuilder.mockReturnValue({
  ...
  it('returns manual_review for NPIF basic when period has no accounting movement', async () => {
    ...
    ctx.getProfitLoss.mockResolvedValue(
      createProfitLoss({ revenue: '0.00', expenses: '0.00', netIncome: '0.00' }),
    );
  ...
  it('keeps mandatory NPIF warning even when warning-disable flags are enabled', async () => {
    ...
    const originalLegacyDisableWarning = process.env.FLUX_MVP_DISABLE_NPIF_WARNING;
    ...
    try {
      ...
      if (originalLegacyDisableWarning === undefined) {
        ...
        process.env.FLUX_MVP_DISABLE_NPIF_WARNING = originalLegacyDisableWarning;
      }
    }
  });
  ...
  it('returns blocked for monitor fiscal when there is at least one critical issue', async () => {
    ...
    ctx.validatePseCompliance.mockResolvedValue({
      ...
      proactiveAlerts: [
        ...
        {
          level: 'info',
          message: 'OPENROUTER credentials missing; using deterministic fallback alerts.',
  ...
  it('returns runtime-normalized AI alerts for monitor fiscal when proactive alerts do not include heuristic openrouter markers', async () => {
      ...
      proactiveAlerts: [
        {
          ...
          action: 'Revisar libro ventas y recalcular base imponible para PDT 621.',
        },
        {
          level: 'info',
          message: 'Inconsistencia leve entre RVIE y resumen interno de compras.',
    ...
    expect(result.alerts[0]).toMatchObject({
      ...
      confidence: 0.92,
      source: 'ai',
      recommendedAction: 'Revisar libro ventas y recalcular base imponible para PDT 621.',
    });
