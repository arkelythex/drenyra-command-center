...
describe("ledger-mvp-rollout.service", () => {
	...
	const originalSireRolesLegacy =
		process.env.FLUX_MVP_ALLOWED_ROLES_SIRE_AUTOPILOT;
	...
	const originalMonitorRoles =
		process.env.LEDGER_MVP_ALLOWED_ROLES_MONITOR_FISCAL;
	const originalMonitorRolesLegacy =
		process.env.FLUX_MVP_ALLOWED_ROLES_MONITOR_FISCAL;
	...
	it("uses default Ledger MVP allowed roles and supports custom role allowlist", () => {
		...
		expect(getLedgerMvpAllowedRoles()).toEqual([
			"owner",
			"senior",
			"admin",
			"superadmin",
		...
		expect(isLedgerMvpRoleAllowed("sire_autopilot_run", "accountant")).toBe(
			true,
		);
		...
		expect(getLedgerMvpAllowedRolesForEndpoint("sire_autopilot_run")).toEqual([
			"tax-analyst",
		]);
		expect(isLedgerMvpRoleAllowed("sire_autopilot_run", "tax-analyst")).toBe(
			true,
		);
		expect(isLedgerMvpRoleAllowed("sire_autopilot_run", "accountant")).toBe(
			false,
