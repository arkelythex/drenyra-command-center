...
export class GovernanceAuditService {
  static async listDecisions(query: GovernanceAuditQuery): Promise<GovernanceAuditResult> {
    ...
    if (!query.feature || query.feature === 'all' || query.feature === 'sire') {
      ...
      for (const row of sireRows) {
        ...
        const decision = normalizeDecision(readString(trace?.decision)) ??
...
function resolveTimestamp(candidates: Array<string | Date | undefined | null>): string {
...
function extractSireTrace(warnings: unknown, errors: unknown): Record<string, unknown> | null {
