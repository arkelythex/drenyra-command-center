 ...
 * ```
 */
export function generateCreditNoteXml(data: CreditNoteData): XmlGenerationResult {
  ...
  if (!validatePartyDocument(data.customer.ruc, data.customer.documentType)) {
    throw new Error(`Invalid document number for customer: ${data.customer.ruc}`);
  }

  if (data.customer.documentType === '1' && !originalDocumentId.startsWith('B')) {
  ...
  // UBL Version
  doc.ele('cbc:UBLVersionID').txt(data.ublVersionId || UBL_VERSION).up();
  doc.ele('cbc:CustomizationID').txt(data.customizationId || CUSTOMIZATION_ID).up();
  ...
  discrepancy.ele('cbc:ResponseCode').txt(data.discrepancyResponse.responseCode).up();
  discrepancy.ele('cbc:Description').txt(data.discrepancyResponse.description).up();
  discrepancy.up();
  ...
  data.invoiceLines.forEach((line) => {
    ...
    createAmountElement(priceElement, 'cbc', 'PriceAmount', line.unitPrice, data.documentCurrencyCode);
    priceElement.up();
  ...
  const fileName = generateXmlFileName(data.supplier.ruc, '07', series, correlative);
