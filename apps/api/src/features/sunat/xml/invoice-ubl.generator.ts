...
export function generateInvoiceXml(data: InvoiceData): XmlGenerationResult {
  ...
  if (data.paymentTerms) {
    ...
    paymentTermsElement.ele('cbc:ID').txt(data.paymentTerms.paymentMeansCode).up();
  ...
  data.taxTotals.forEach((taxTotal) => {
    ...
    taxTotal.taxSubtotal.forEach((subtotal) => {
      ...
      categoryElement.ele('cbc:Percent').txt(formatAmount(subtotal.taxRate)).up();
  ...
  data.invoiceLines.forEach((line) => {
    ...
    if (line.taxAmount) {
      ...
      createAmountElement(lineTaxTotal, 'cbc', 'TaxAmount', line.taxAmount, data.documentCurrencyCode);
      lineTaxTotal.up();
    }
