...
export function signXml(xml: string, certificate: Certificate): string {
	try {
		if (!xml.includes("ExtensionContent")) {
			throw new Error(
				"ExtensionContent not found in XML. Cannot add signature.",
			);
		}
		...
		if (!extensionContent) {
			extensionContent =
				(doc.getElementsByTagName("ExtensionContent")[0] as
					| XmlDomElement
					| undefined) ??
				(doc.getElementsByTagName("ext:ExtensionContent")[0] as
					| XmlDomElement
					| undefined) ??
				null;
		}
		...
		signedXmlString = insertCertificateInSignature(
			signedXmlString,
 ...
 */
function insertCertificateInSignature(
	signedXml: string,
	publicCert: string,
): string {
	...
	let keyInfo = doc.getElementsByTagNameNS(
		"http://www.w3.org/2000/09/xmldsig#",
		"KeyInfo",
	)[0];
	if (!keyInfo) {
		...
		const signedInfo = signature.getElementsByTagNameNS(
			"http://www.w3.org/2000/09/xmldsig#",
			"SignedInfo",
		)[0];
	...
	let x509Certificate = keyInfo.getElementsByTagNameNS(
		"http://www.w3.org/2000/09/xmldsig#",
		"X509Certificate",
	)[0];
	if (!x509Certificate) {
