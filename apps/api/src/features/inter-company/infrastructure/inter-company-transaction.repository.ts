...
class InterCompanyTransactionRepository
	implements IInterCompanyTransactionRepository
{
	private async enrichWithCompanyMetadata(
		...
		const ids = Array.from(
			new Set(rows.flatMap((r) => [r.fromCompanyId, r.toCompanyId])),
		);
	...
	async validateSameGroup(
		...
		if (fromCompanyId === toCompanyId) {
			throw new Error(
				"Cannot create inter-company transaction with same company",
			);
		}
	}
	...
	async findManyByGroup(
		...
		return this.enrichWithCompanyMetadata(
			rows as unknown as InterCompanyTransactionRow[],
		);
	}
	...
	async findAuditPage(
		...
		const conditions = [
			eq(interCompanyTransactions.economicGroupId, filters.economicGroupId),
		];
		if (filters.detractionProfile) {
			conditions.push(
				eq(
					interCompanyTransactions.detractionProfile,
					filters.detractionProfile,
				),
			);
		}
		if (filters.detractionRuleCode) {
			conditions.push(
				eq(
					interCompanyTransactions.detractionRuleCode,
					filters.detractionRuleCode,
				),
			);
		}
		if (filters.dateFrom)
			conditions.push(
				gte(interCompanyTransactions.createdAt, filters.dateFrom),
			);
		if (filters.dateTo)
			conditions.push(lte(interCompanyTransactions.createdAt, filters.dateTo));

		const where = and(...conditions);
		const [rows, countRows] = await Promise.all([
			db.query.interCompanyTransactions.findMany({
				where,
				orderBy,
				limit,
				offset,
		...
		const items = await this.enrichWithCompanyMetadata(
			rows as unknown as InterCompanyTransactionRow[],
		);
...
export const interCompanyTransactionRepository =
	new InterCompanyTransactionRepository();
