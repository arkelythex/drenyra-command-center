...
const createHandler = () => new CreateInterCompanyTransactionHandler(repository);
...
describe('CreateInterCompanyTransactionHandler', () => {
  ...
  it('calculates taxes when taxType is GRAVADO and persists the atomic transaction', async () => {
    ...
    (repository.createAtomic as ReturnType<typeof vi.fn>).mockResolvedValue(baseResult);
    (repository.validateSameGroup as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
  ...
  it('returns calculations without tax when taxType is EXONERADO', async () => {
    ...
    const spotConfigSpy = vi.spyOn(taxRateProviderService, 'getSpotDetractionConfig');
