...
export interface InterCompanyStats {
  ...
  detractionByProfile: Record<string, { count: number; totalDetraction: number }>;
...
export class GetStatsHandler {
  ...
  async execute(economicGroupId: string): Promise<InterCompanyStats> {
    ...
    const totalIGV = txs.reduce((s, tx) => s + parseFloat(tx.igvAmount ?? '0'), 0);
    const totalDetraction = txs.reduce((s, tx) => s + parseFloat(tx.detractionAmount ?? '0'), 0);
    ...
    return {
      ...
      detractionByProfile,
      detractionByRule,
      timeSaved: { minutes: timeSavedMinutes, hours: Math.round(timeSavedMinutes / 60) },
