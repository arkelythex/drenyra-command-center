...
export class CreateInterCompanyTransactionHandler {
  ...
  async execute(
    ...
    const detractionProfile = normalizeSpotDetractionProfile(input.detractionProfile);
    ...
    logger.info('Inter-company transaction completed', { id: result.interCompany.id });
    return result;
  }
  ...
  private async calculateTaxes(
    ...
    const config = await taxRateProviderService.getSpotDetractionConfig(atDate, detractionProfile);
    const operationCents = Math.round(amount * 100);
    const hasDetraction = config.thresholdCents <= 0 || operationCents > config.thresholdCents;
    ...
    return {
      ...
      detractionRate: hasDetraction ? Number((config.rate * 100).toFixed(4)) : null,
