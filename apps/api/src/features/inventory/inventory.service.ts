...
function multiplyScaledDecimals(a: bigint, b: bigint): bigint {
  const product = a * b;
  const shouldRoundUp = product >= 0n ? product % SCALE_FACTOR >= SCALE_FACTOR / 2n : product % SCALE_FACTOR <= -(SCALE_FACTOR / 2n);
...
function divideScaledDecimals(dividend: bigint, divisor: bigint): bigint {
  ...
  const sameSign = (dividend >= 0n && divisor > 0n) || (dividend < 0n && divisor < 0n);
  return sameSign ? quotient + 1n : quotient - 1n;
}

function buildInventoryScope(companyId: string, productId: string, warehouseId?: string) {
  if (warehouseId) {
    return and(
...
export class InventoryService {
   ...
   */
  static async list(companyId: string, warehouseId?: string): Promise<InventoryListRow[]> {
    const conditions = warehouseId
      ? and(eq(inventory.companyId, companyId), eq(inventory.warehouseId, warehouseId))
      : eq(inventory.companyId, companyId);
   ...
   */
  static async getByProduct(productId: string, warehouseId?: string): Promise<InventoryByProductRow | null> {
    const conditions = warehouseId
   ...
   */
  static async recordMovement(companyId: string, data: InventoryMovementInput): Promise<InventoryMovementRow> {
    const { productId, warehouseId, type, quantity, unitCost } = data;

    return await db.transaction(async (tx) => {
      const quantityScaled = parseScaledDecimal(quantity);
      const unitCostScaled = unitCost !== undefined ? parseScaledDecimal(unitCost) : undefined;
      const totalCostScaled =
        unitCostScaled !== undefined ? multiplyScaledDecimals(quantityScaled, unitCostScaled) : undefined;
      ...
      const [movement] = await tx
        ...
        .values({
          ...
          unitCost: unitCostScaled !== undefined ? formatScaledDecimal(unitCostScaled) : undefined,
          totalCost: totalCostScaled !== undefined ? formatScaledDecimal(totalCostScaled) : undefined,
      ...
      if (type === 'IN' || type === 'TRANSFER') newQuantityScaled += quantityScaled;
      ...
      if (type === 'IN' && unitCostScaled !== undefined) {
        const existingValue = multiplyScaledDecimals(currentQtyScaled, currentCostScaled);
        const incomingValue = multiplyScaledDecimals(quantityScaled, unitCostScaled);
        const weightedValue = existingValue + incomingValue;
        newUnitCostScaled =
          newQuantityScaled > 0n ? divideScaledDecimals(weightedValue, newQuantityScaled) : unitCostScaled;
      } else if (type === 'ADJUSTMENT' && unitCostScaled !== undefined) {
        newUnitCostScaled = unitCostScaled;
      }

      const totalValueScaled = multiplyScaledDecimals(newQuantityScaled, newUnitCostScaled);
   ...
   */
  static async getKardex(companyId: string, productId: string, startDate?: Date, endDate?: Date): Promise<KardexRow[]> {
    const conditions = [
    ...
    return movements.map((movement) => {
      ...
      return {
        ...
        totalValue: formatScaledDecimal(multiplyScaledDecimals(balanceScaled, unitCostScaled)),
      };
    });
  }
  ...
  static async getSummary(companyId: string): Promise<InventorySummary> {
    ...
    return {
      ...
      totalQuantity: formatScaledDecimal(
        items.reduce((sum, item) => sum + parseScaledDecimal(item.quantity), 0n),
      ),
      totalValue: formatScaledDecimal(
        items.reduce((sum, item) => sum + parseScaledDecimal(item.totalValue), 0n),
      ),
      lowStockItems: items.filter((item) => {
        if (!item.minStock) return false;
        return parseScaledDecimal(item.quantity) < parseScaledDecimal(item.minStock);
      }).length,
      warehouseCount:
        (
          await db
   ...
   * Crea almacén.
   */
  static async createWarehouse(companyId: string, data: WarehouseCreateInput): Promise<WarehouseRow> {
