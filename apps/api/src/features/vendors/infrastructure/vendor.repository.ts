...
export class VendorRepository implements IVendorRepository {
	async create(input: CreateVendorInput): Promise<Vendor> {
		...
		const preferredPaymentMethod: PreferredPaymentMethod =
			input.preferredPaymentMethod ?? "TRANSFER";
				...
				const [profile] = await tx
					...
					.set({
						...
						preferredPaymentMethod: (input.preferredPaymentMethod ??
							existing.preferredPaymentMethod) as PreferredPaymentMethod,
	...
	async list(filters: VendorListFilters): Promise<Vendor[]> {
		const rows = await withCompanyRlsTransaction(
			...
			async (tx) => {
				return await tx
					...
					.innerJoin(
						businessPartners,
						eq(vendorProfiles.id, businessPartners.id),
					)
		...
		return vendors.filter((vendor) => {
			...
			if (filters.category) {
				...
				return vendor.purchaseCategories.some(
