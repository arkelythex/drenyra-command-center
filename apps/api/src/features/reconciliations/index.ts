 ...
 * ```
 */
export const reconciliationsModule = new Elysia({ prefix: '/api/reconciliations' })
  .use(companyScopeGuard())
  .get('/pending', async ({ query, set }) => {
    try {
      const limit = query.limit ?? 10;
      const pendingReconciliations = await getPending(
        query.companyId,
        limit
      );
  ...
  .get('/reconciled', async ({ query, set }) => {
    try {
      const limit = query.limit ?? 50;
      const reconciled = await getReconciled(
        query.companyId,
        limit
      );
  ...
  .post('/:id/reconcile', async ({ params, body, set }) => {
    try {
  ...
  .post('/:id/unreconcile', async ({ params, body, set }) => {
    try {
      const unreconciled = await unreconcile(
        body.companyId,
        params.id
      );
      if (!unreconciled) {
  ...
  .get('/:id/matches', async ({ params, query, set }) => {
    try {
  ...
  .post('/external/reconcile', async ({ body, set }) => {
    try {
