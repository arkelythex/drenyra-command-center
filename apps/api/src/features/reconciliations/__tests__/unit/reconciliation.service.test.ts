...
describe('ReconciliationService', () => {
  ...
  it('delegates reconciliation operations', async () => {
    const repository = {
      ...
      markReconciled: vi.fn().mockResolvedValue({ id: 'tx-1', reconciledAt: new Date() }),
      markUnreconciled: vi.fn().mockResolvedValue({ id: 'tx-1', reconciledAt: null }),
      findAllByCompany: vi.fn(),
      findById: vi.fn(),
    };
    const service = new ReconciliationService(repository);

    const reconciled = await service.reconcile('cmp-1', 'tx-1', { notes: 'manual' });
    const unreconciled = await service.unreconcile('cmp-1', 'tx-1');
    const list = await service.getReconciled('cmp-1', 25);

    expect(repository.markReconciled).toHaveBeenCalledWith('cmp-1', 'tx-1', { notes: 'manual' });
  ...
  it('returns empty automatic matches placeholder', async () => {
    ...
    expect(result).toEqual({ transactionId: 'tx-1', matches: [], confidence: 0 });
  });
