...
export class ReconciliationService {
  constructor(private readonly repository: ReconciliationRepoContract = reconciliationRepository) {}

  async getPending(companyId: string, limit = 10) {
    return this.repository.findPending(companyId, this.normalizeLimit(limit, 10));
  }

  async reconcile(companyId: string, transactionId: string, reconciliationData?: ReconcileInput) {
    return this.repository.markReconciled(companyId, transactionId, reconciliationData);
  }

  async getReconciled(companyId: string, limit = 50) {
    return this.repository.findReconciled(companyId, this.normalizeLimit(limit, 50));
  }
  ...
  async getStats(companyId: string) {
    ...
    const reconciledTotal = reconciled.reduce((sum, tx) => sum + this.parseAmount(tx.amount), 0);
    const pendingTotal = pending.reduce((sum, tx) => sum + this.parseAmount(tx.amount), 0);
    ...
    return {
      ...
      percentage: allTransactions.length > 0 ? (reconciled.length / allTransactions.length) * 100 : 0,
    };
  }

  async findMatches(companyId: string, transactionId: string) {
    const transaction = await this.repository.findById(companyId, transactionId);
    if (!transaction) {
      return null;
    }
