...
export class MissionsService {
  ...
  async createMission(companyId: string, cmd: RunIntentCommand): Promise<MissionSnapshot> {
    if (!VALID_INTENTS.has(cmd.intent)) {
      throw new MissionError(MissionErrorCode.INVALID_TRANSITION, 400, `Invalid intent: ${cmd.intent}`);
    }
  ...
  async getMission(missionId: string, companyId: string): Promise<MissionSnapshot | null> {
  ...
  async executeMission(
    ...
    validateTransition(mission.status as AccountingMissionStatus, AccountingMissionStatus.QUEUED);

    const newVersion = await optimisticUpdate(this.db, missionId, companyId, opts.expectedMissionVersion, {
  ...
  async approveMission(
    ...
    validateTransition(mission.status as AccountingMissionStatus, AccountingMissionStatus.APPROVED);
    ...
    const payloadHash = createHash("sha256")
      .update(JSON.stringify(payloadHashData, Object.keys(payloadHashData).sort()))
      .digest("hex");
    ...
    const receiptId = receiptHash.substring(0, 36).replace(/(.{8})(.{4})(.{4})(.{4})(.{12})/, "$1-$2-$3-$4-$5");

    const newVersion = await optimisticUpdate(this.db, missionId, companyId, cmd.expectedMissionVersion, {
  ...
  async rejectMission(
    ...
    validateTransition(mission.status as AccountingMissionStatus, AccountingMissionStatus.REJECTED);
  ...
  private async getMissionOrThrow(missionId: string, companyId: string): Promise<MissionSnapshot> {
