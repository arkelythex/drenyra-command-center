...
import { useCentralBoardStore, type JournalEntry } from "@/stores/central-board-store";
...
export function JournalPendingList() {
  ...
  const entries =
    storeEntries.length > 0 ? storeEntries : DEMO_JOURNAL_ENTRIES;
    ...
    async (entry: JournalEntry) => {
      ...
      try {
        ...
        toast.error("Error al aprobar asiento", {
          description:
            err instanceof Error ? err.message : "Error al registrar el asiento en el libro diario",
        });
      } finally {
        setApprovingId(null);
      }
