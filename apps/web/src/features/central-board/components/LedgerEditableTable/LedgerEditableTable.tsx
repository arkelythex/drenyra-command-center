...
import { useJournalEntries, useUpdateJournalEntry } from "@/features/drenyra/hooks/useJournalEntriesApi";
                  ...
                  entries.map((entry) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 align-top text-center">
                        <EditableCell
                          value={entry.cuenta}
                          onSave={(v) => console.log("update cuenta", entry.id, v)}
                        />
                      </td>
                      <td className="px-4 py-4 align-top text-center">
                        <span
                      ...
                      <td className="px-6 py-4 align-top text-right">
                        <EditableCell
                          value={entry.debe}
                          type="money"
                          onSave={(v) => console.log("update debe", entry.id, v)}
                        />
                      </td>
                      <td className="px-6 py-4 align-top text-right">
                        <EditableCell
                          value={entry.haber}
                          type="money"
                          onSave={(v) => console.log("update haber", entry.id, v)}
                        />
                      </td>
                    </tr>
                  ))
        ...
        <div className="mt-3 flex items-center justify-between rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-2)]/50 px-4 py-2">
          <div className="flex items-center gap-2">
            <span className="text-2xs text-[var(--text-muted)]">
              {entries?.length ?? 0} asientos · Click para editar cualquier celda
            </span>
