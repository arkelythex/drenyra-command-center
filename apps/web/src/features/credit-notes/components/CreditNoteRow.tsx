...
const creditNoteTypeConfig: Record<
	CreditNoteRecord["creditNoteType"],
	{ label: string; variant: "danger" | "info" | "warning" | "neutral" }
> = {
	...
	return (
		<tr className="border-b border-border transition-colors hover:bg-muted/40">
			...
			<td className="px-4 py-3">
				<StatusBadge
					status={status}
					label={statusLabel[creditNote.status]}
					size="sm"
				/>
			</td>
			...
			<td className="px-4 py-3">
				<DropdownMenu>
					...
					<DropdownMenuContent align="end" className="min-w-[160px]">
						...
						{creditNote.status === "DRAFT" && (
							<DropdownMenuItem
								onClick={() => onUpdateStatus?.(creditNote.id, "SENT")}
							>
						...
						{creditNote.status === "SENT" && (
							<>
								<DropdownMenuSeparator />
								<DropdownMenuItem
									onClick={() => onUpdateStatus?.(creditNote.id, "ACCEPTED")}
								>
									<CheckCircle className="mr-2 h-4 w-4 text-success" /> Aceptar
								</DropdownMenuItem>
								<DropdownMenuItem
									onClick={() => onUpdateStatus?.(creditNote.id, "REJECTED")}
								>
