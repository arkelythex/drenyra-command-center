  ...
  return (
    <PageShell>
      <PageHeader
        ...
        actions={
          <Button onClick={() => setIsCreateOpen(true)} className="h-10 px-5 rounded-xl text-xs font-black uppercase tracking-widest">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card variant="glass">
          ...
          <CardContent>
            <p className="text-2xl font-bold text-danger tabular-nums">{stats.anulacionCount}</p>
          </CardContent>
        </Card>
        ...
        <Card variant="glass">
          ...
          <CardContent>
            <p className="text-2xl font-bold text-info tabular-nums">{stats.descuentoCount}</p>
          </CardContent>
        </Card>
            ...
            {isLoading ? (
              ...
              creditNotes.map((cn) => (
                <CreditNoteRow
                  ...
                  creditNote={cn}
                  n={(v) => formatPEN(v)}
                  onView={(id) => setSelectedNote(creditNotes.find(cn => cn.id === id) || null)}
      ...
      <CreateCreditNoteDialog open={isCreateOpen} onOpenChange={setIsCreateOpen} />
        ...
        {selectedNote && (
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            <div className="relative max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-border bg-card p-6 shadow-2xl">
              ...
              <StatusBadge
                status={selectedNote.status === 'ACCEPTED' ? 'success' : selectedNote.status === 'REJECTED' ? 'danger' : selectedNote.status === 'SENT' ? 'warning' : 'neutral'}
              ...
              <div className="mt-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-2xs font-black uppercase tracking-widest text-muted-foreground mb-1">Factura Referencia</p>
                    <p className="text-sm font-medium text-foreground">{selectedNote.referenceInvoiceId}</p>
                  </div>
                  <div>
                    <p className="text-2xs font-black uppercase tracking-widest text-muted-foreground mb-1">Tipo</p>
                    <p className="text-sm font-medium text-foreground">{selectedNote.creditNoteType}</p>
                  </div>
                </div>

                <div>
                  <p className="text-2xs font-black uppercase tracking-widest text-muted-foreground mb-1">Motivo</p>
                  <p className="text-sm text-foreground">{selectedNote.reason}</p>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-2xs font-black uppercase tracking-widest text-muted-foreground mb-1">Base Imponible</p>
                    <p className="text-sm font-mono font-semibold tabular-nums text-foreground">{formatPEN(Number(selectedNote.baseAmount))}</p>
                  </div>
                  <div>
                    <p className="text-2xs font-black uppercase tracking-widest text-muted-foreground mb-1">IGV (18%)</p>
                    <p className="text-sm font-mono font-semibold tabular-nums text-foreground">{formatPEN(Number(selectedNote.igvAmount))}</p>
                  </div>
                </div>
                ...
                <div className="border-t border-border pt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-xl font-mono font-black tabular-nums text-foreground">{formatPEN(Number(selectedNote.totalAmount))}</p>
                  </div>
                </div>
