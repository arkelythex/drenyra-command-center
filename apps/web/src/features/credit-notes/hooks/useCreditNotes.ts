...
export const cnHooks = createCrudHooks<
	CreditNoteRecord,
...
>({
	...
	create: (companyId, data) =>
		creditNotesApi.create({ ...data, companyId }) as Promise<CreditNoteRecord>,
	update: (id, data) =>
		creditNotesApi.updateStatus(id, data.status) as Promise<CreditNoteRecord>,
	delete: async (id) => {
...
function normalizeStats(creditNotes: CreditNoteRecord[]): CreditNoteStats {
	...
	const anulacionCount = creditNotes.filter(
		(cn) => cn.creditNoteType === "ANULACION",
	).length;
	const descuentoCount = creditNotes.filter(
		(cn) => cn.creditNoteType === "DESCUENTO",
	).length;
	const devolucionCount = creditNotes.filter(
		(cn) => cn.creditNoteType === "DEVOLUCION",
	).length;
	const otrosCount = creditNotes.filter(
		(cn) => cn.creditNoteType === "OTROS",
	).length;
	const draftCount = creditNotes.filter((cn) => cn.status === "DRAFT").length;
	const sentCount = creditNotes.filter((cn) => cn.status === "SENT").length;
	const acceptedCount = creditNotes.filter(
		(cn) => cn.status === "ACCEPTED",
	).length;
	const rejectedCount = creditNotes.filter(
		(cn) => cn.status === "REJECTED",
	).length;
	const totalAmount = creditNotes.reduce(
		(sum, cn) => sum + (Number.parseFloat(cn.totalAmount) || 0),
		0,
	);
