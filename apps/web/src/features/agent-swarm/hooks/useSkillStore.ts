...
interface SkillStore {
	...
	getSkillsByCategory: (
		category: AccountingSkill["category"],
	) => AccountingSkill[];
}
			...
			skills: [
				{
					...
					name: "Validador SIRE",
					description:
						"Auditoría automática de registros de compras y ventas vs SUNAT.",
					category: "fiscal",
					version: "1.2.0",
					isInstalled: true,
				...
				},
				{
					...
					description:
						"Match inteligente de estados de cuenta BCP con el Ledger.",
					category: "finance",
					version: "2.0.4",
					isInstalled: false,
				...
				},
				{
					...
					name: "Generador PLE",
					description:
						"Construcción de libros electrónicos bajo normativa 2026.",
					category: "fiscal",
					version: "0.9.0",
					isInstalled: false,
			...
			installSkill: (id) =>
				set((state) => ({
					skills: state.skills.map((s) =>
						s.id === id ? { ...s, isInstalled: true } : s,
					),
				})),
			uninstallSkill: (id) =>
				set((state) => ({
					skills: state.skills.map((s) =>
						s.id === id ? { ...s, isInstalled: false } : s,
					),
				})),
