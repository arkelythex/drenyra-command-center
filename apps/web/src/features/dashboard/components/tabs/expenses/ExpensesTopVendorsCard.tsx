...
export function ExpensesTopVendorsCard({
	topVendors,
	totalExpenses,
}: ExpensesTopVendorsCardProps) {
	const maxVendorTotal = Math.max(
		...topVendors.map((vendor) => vendor.total),
		1,
	);
			...
			<div className="space-y-3">
				{topVendors.length === 0 ? (
					...
					topVendors.map((vendor, index) => {
						const share =
							totalExpenses > 0 ? (vendor.total / totalExpenses) * 100 : 0;
						const relativeWidth = Math.max(
							8,
							(vendor.total / maxVendorTotal) * 100,
						);
								...
								<div className="flex items-start justify-between gap-3">
									...
									<div className="text-right">
										<p className="text-sm font-semibold text-foreground">
											{formatPEN(vendor.total)}
										</p>
										<p className="text-xs text-muted-foreground">
											{vendor.billCount} docs
										</p>
									</div>
								</div>
