...
export function ExpensesChartTooltip({ active, payload }: ExpensesChartTooltipProps) {
  ...
  return (
    <div className="min-w-[220px] rounded-xl border border-border/40 bg-white/95 px-3 py-2.5 shadow-xl backdrop-blur-md">
      <p className="text-sm font-semibold text-foreground">{record.category}</p>
      <div className="mt-2 space-y-1.5 text-xs">
        <div className="flex items-center justify-between gap-3">
          <span className="text-muted-foreground">Monto</span>
          <span className="font-semibold text-foreground">{formatPEN(record.total)}</span>
        </div>
        <div className="flex items-center justify-between gap-3">
          <span className="text-muted-foreground">Participación</span>
          <span className="font-semibold text-foreground">{formatPercent(record.percentage)}</span>
        </div>
        <div className="flex items-center justify-between gap-3">
