...
const SireMatcher = React.lazy(() => import('../widgets/SireMatcher').then(m => ({ default: m.SireMatcher })));
const FiscalIndicators = React.lazy(() => import('../widgets/FiscalIndicators').then(m => ({ default: m.FiscalIndicators })));
const TaxCalendar = React.lazy(() => import('../widgets/TaxCalendar').then(m => ({ default: m.TaxCalendar })));
const DetraccionesWidget = React.lazy(() => import('../widgets/DetraccionesWidget').then(m => ({ default: m.DetraccionesWidget })));
      ...
      <Card variant="bordered" padding="none" className="rounded-2xl">
        <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <Text variant="hero" id="advanced-panel-title" className="mb-0.5 text-base font-semibold tracking-tight text-foreground">
                Panel avanzado
              </Text>
              <Text variant="label" className="text-label text-muted-foreground">
          ...
          >
            ...
            {showAdvancedPanel ? <ChevronUp className="ml-2" size={16} /> : <ChevronDown className="ml-2" size={16} />}
          </Button>
        </CardContent>
      </Card>
        ...
        <div className="ui-deferred-section grid grid-cols-1 gap-5 items-start 2xl:grid-cols-12">
          <div className="2xl:col-span-8 space-y-5">
            <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
              <Suspense fallback={<DashboardCardSkeleton className="h-[400px]" />}>
                <SireMatcher />
              </Suspense>
              <Suspense fallback={<DashboardCardSkeleton className="h-[400px]" />}>
                <FiscalIndicators />
          ...
          <div className="2xl:col-span-4 space-y-5">
            <Suspense fallback={<DashboardCardSkeleton className="h-[300px]" />}>
              <TaxCalendar />
            </Suspense>
            <Suspense fallback={<DashboardCardSkeleton className="h-[300px]" />}>
              <DetraccionesWidget />
