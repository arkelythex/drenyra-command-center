...
function buildMonthDeadlines(reference: Date): Deadline[] {
	...
	return items.map((item) => {
		...
		const daysLeft = Math.ceil(
			(startOfDay(dueDate).getTime() - today) / (24 * 60 * 60 * 1000),
		);
...
function getStatus(
	daysLeft: number,
): "urgent" | "upcoming" | "scheduled" | "overdue" {
				...
				{deadlines.map((item) => {
					...
					return (
						...
						>
							...
							<div
								className={cn(
									"ml-3 inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-label font-medium",
									badgeClass,
								)}
							>
								<Clock3 size={12} aria-hidden="true" />
								{item.daysLeft < 0
									? `${Math.abs(item.daysLeft)} d atraso`
									: `${item.daysLeft} d`}
							</div>
						</div>
					);
