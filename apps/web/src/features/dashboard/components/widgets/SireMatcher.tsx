	...
	return (
		<div
			className={cn(
				...
				"group relative h-full overflow-hidden border border-border/50 bg-card p-6 transition-[border-color,box-shadow] duration-200 sm:p-7",
				tokensToClasses.shadow("modal"),
				"shadow-sm hover:border-border/70 hover:shadow-md",
			)}
		>
			...
			<div className="relative z-10 mb-7 flex items-center gap-4">
				...
				<div className="flex flex-col">
					<Label
						className={cn(
							tokensToClasses.typography("label"),
							"font-black text-foreground uppercase tracking-[0.2em] leading-tight",
						)}
					>
						Monitor Inteligente SIRE
					</Label>
					<span
						className={cn(
							tokensToClasses.typography("xs"),
							"text-muted-foreground font-bold uppercase tracking-widest mt-0.5",
						)}
					>
						Validación RVIE / RCE 2026
					</span>
				</div>
			</div>
			...
			<div className="relative z-10 space-y-5">
				...
				<div
					className={cn(
						...
						"flex flex-col items-center justify-between gap-4 border border-border bg-muted/20 p-6 sm:flex-row sm:gap-0",
					)}
				>
					<div className="space-y-3 w-full sm:w-auto text-center sm:text-left">
						<div className="flex items-center justify-center sm:justify-start gap-2">
							...
							<p
								className={cn(
									tokensToClasses.typography("xs"),
									"text-muted-foreground uppercase font-bold tracking-[0.2em]",
								)}
							>
								MATCH RATE SIRE
							</p>
					...
					<div className="space-y-3 w-full sm:w-auto text-center sm:text-right">
						...
						<p className="text-lg font-black text-foreground font-mono tracking-tighter tabular-nums">
							{loading
								? "..."
								: `${summary.matched}/${summary.totalInvoices || 0}`}
						</p>
					</div>
				</div>
				...
				>
					<div
						className={cn(
							...
							isMatch
								? "bg-success text-[var(--text-primary)]"
								: "bg-danger text-[var(--text-primary)]",
						)}
					>
						{isMatch ? (
					...
					<div className="min-w-0 flex-1 py-3 text-center sm:py-0 sm:text-left">
						<p
							className={cn(
								tokensToClasses.typography("2xs"),
								"font-black uppercase tracking-[0.2em] text-foreground mb-2",
							)}
						>
						...
						{!isMatch && (
							<div className="flex flex-col sm:flex-row items-center gap-2">
								<div
									className={cn(
										tokensToClasses.typography("2xs"),
										"font-bold text-destructive uppercase tracking-tight flex items-center gap-2",
									)}
								>
									<span>INCONSISTENCIAS:</span>
									<span className="rounded-full border border-danger-subtle bg-background px-2 py-0.5 text-xs font-black text-danger">
										{discrepancyCount}
									</span>
								</div>
							</div>
					...
					{!isMatch && (
						<button
							onClick={() =>
								alert("Generando reporte de inconsistencias para SIRE 2026...")
							}
							className={cn(
								tokensToClasses.typography("2xs"),
								"w-full rounded-xl bg-danger px-5 py-2.5 font-bold uppercase tracking-[0.15em] text-[var(--text-primary)] shadow-sm transition-[transform,box-shadow,opacity] duration-200 hover:-translate-y-0.5 hover:opacity-90 hover:shadow-md sm:w-auto",
							)}
						>
							CORREGIR AHORA
						</button>
