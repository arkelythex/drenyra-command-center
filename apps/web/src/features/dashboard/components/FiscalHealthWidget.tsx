...
export const FiscalHealthWidget: React.FC = () => {
  ...
  return (
    <LiquidGlassCard intensity="medium" className="group relative flex h-full flex-col items-center justify-center overflow-hidden border-border/40 p-6">
      {/* Background Specular Leak */}
      <div className={cn(
        "absolute -right-20 -top-20 h-64 w-64 rounded-full blur-[72px] transition-colors duration-300",
        isOptimal ? "bg-success-subtle" : "bg-danger-subtle"
      )} />
      ...
      <div className="relative z-10 flex w-full flex-col items-center gap-7">
        <div className="relative flex h-56 w-56 items-center justify-center">
          ...
          <svg className="h-full w-full -rotate-90 transform" viewBox="0 0 100 100" aria-hidden="true">
            <defs>
              <linearGradient id="fiscal-score-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor={isOptimal ? "rgb(var(--premium-success-rgb))" : "rgb(var(--premium-danger-rgb))"} />
                <stop offset="100%" stopColor={isOptimal ? "rgba(var(--premium-success-rgb),0.68)" : "rgba(var(--premium-danger-rgb),0.68)"} />
              </linearGradient>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <div className="flex items-baseline gap-1">
              <Text variant="hero" className="text-6xl font-black tracking-tighter text-foreground">
                <AnimatedNumber value={displayScore} />
              </Text>
              <Text variant="label" className="text-base font-black text-muted-foreground">%</Text>
            </div>
            ...
            <motion.div
              ...
              className={cn(
                ...
                isOptimal ? "border-success-subtle bg-success-subtle" : "border-danger-subtle bg-danger-subtle"
              )}
            >
        ...
        <div className="flex w-full items-center justify-between border-t border-border/30 px-2 pt-6">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2.5">
              ...
              <Text variant="label" className="text-3xs font-black uppercase tracking-widest text-muted-foreground">SUNAT Status</Text>
            </div>
            <div className="flex items-center gap-2">
              {isOptimal ? <ShieldCheck size={18} className="text-success" /> : <ShieldAlert size={18} className="text-danger" />}
              <Text variant="body" className="text-sm font-black uppercase leading-none tracking-tight text-foreground">
          ...
          >
            <Plus size={20} className="text-foreground transition-transform duration-200 group-hover/btn:rotate-90" />
          </Button>
        </div>
      </div>

      {/* Glass shimmer overlay */}
      <div className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-white/5 to-transparent opacity-20" />
    </LiquidGlassCard>
  );
};
