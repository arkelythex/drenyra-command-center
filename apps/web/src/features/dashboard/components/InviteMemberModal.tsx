...
export const InviteMemberModal: React.FC<InviteMemberModalProps> = ({
	...
	const roles: { id: Role; label: string; desc: string; color: string }[] = [
		{
			id: "ADMIN",
			label: "Administrador",
			desc: "Acceso total a la configuración y finanzas.",
			color: "text-foreground",
		},
		{
			id: "CONTADOR",
			label: "Contador",
			desc: "Gestión de impuestos, SIRE y estados financieros.",
			color: "text-primary",
		},
		{
			id: "VISOR",
			label: "Visor",
			desc: "Solo lectura de reportes y documentos.",
	...
	return (
		<Dialog open={isOpen} onOpenChange={onOpenChange}>
			<DialogContent className="overflow-hidden rounded-[1.75rem] border-border bg-[var(--surface-1)]/96 p-0 shadow-xl  sm:max-w-[520px]">
				{!isSuccess ? (
					<div className="relative z-10 p-8 sm:p-10">
						<DialogHeader className="mb-8 text-left sm:text-left">
							...
							<DialogTitle className="text-2xl font-black leading-none tracking-tighter text-foreground uppercase">
								Invitar Operador
							</DialogTitle>
							<DialogDescription className="text-muted-foreground font-medium mt-3 text-sm leading-relaxed">
								Asigne un nuevo nodo humano al equipo de inteligencia financiera
								de su organización.
							</DialogDescription>
						</DialogHeader>
						...
						<div className="space-y-8">
							<div className="space-y-4">
								<label
									...
									className="ml-1 text-2xs font-black uppercase tracking-[0.3em] text-primary"
								>
									Canal de Acceso (Email)
								</label>
							...
							<div className="space-y-4">
								<span className="ml-1 text-2xs font-black uppercase tracking-[0.3em] text-primary">
									...
									{roles.map((r) => (
										...
										>
											...
											<div className="relative z-10 flex-1">
												...
												<p className="mt-1 text-2xs font-medium text-muted-foreground leading-snug">
													{r.desc}
												</p>
											</div>
											{role === r.id && (
					...
					<div className="relative z-10 flex animate-entrance flex-col items-center p-16 text-center">
						...
						<h3 className="mb-4 text-2xl font-black uppercase tracking-tighter text-foreground">
							Nodo Sincronizado
						</h3>
						<p className="text-sm font-medium text-muted-foreground/70 leading-relaxed max-w-xs uppercase tracking-widest text-2xs">
							Protocolo de acceso enviado a <br />
							<span className="font-black text-success">{email}</span> con
							privilegios de {role}.
						</p>
					</div>
				)}
