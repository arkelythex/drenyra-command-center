...
export function useCommandSuggestions(value: string): UseCommandSuggestionsResult {
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'ArrowDown') {
      ...
      setSelectedIndex((previousIndex) => (previousIndex + 1) % suggestions.length);
      return;
    }

    if (event.key === 'ArrowUp') {
      event.preventDefault();
      setSelectedIndex((previousIndex) => (previousIndex - 1 + suggestions.length) % suggestions.length);
      return;
    }
