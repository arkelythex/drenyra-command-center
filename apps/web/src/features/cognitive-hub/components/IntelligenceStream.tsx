	...
	return (
		<AnimatePresence>
			{isOpen && (
				...
				>
					<header className="flex items-center justify-between border-b border-border/80 bg-card/80 p-4 ">
						<div className="flex items-center gap-2">
							<Terminal size={12} className="text-primary" />
							<span className="text-2xs font-black uppercase tracking-widest text-foreground/60">
								Intelligence Stream
							</span>
						</div>
						<div className="flex items-center gap-2">
							<div className="h-1 w-1 rounded-full bg-[var(--premium-success)] animate-pulse" />
							<span className="text-[8px] text-[var(--premium-success)] uppercase">
								Live
							</span>
						</div>
					</header>
					...
					>
						{logs.map((log) => (
							<div key={log.id} className="text-2xs leading-relaxed group">
								<span className="mr-2 text-muted-foreground/45">
									[{log.timestamp}]
								</span>
								<span
									className={cn(
					...
					<footer className="flex items-center justify-between border-t border-border/80 bg-muted/10 p-4">
						<span className="text-[8px] uppercase tracking-widest text-muted-foreground/45">
							Buffer: 50/500
						</span>
						<button
							type="button"
