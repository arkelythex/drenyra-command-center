...
import {
	fetchCostStats,
	fetchRecentEvents,
} from "./cost-dashboard/cost-dashboard.api";
...
export const CostDashboard = ({
	...
	const agentEntries = useMemo(
		() =>
			stats
				? Object.entries(stats.historical.byAgent).sort(
	...
	const maxTrend = useMemo(
		() =>
			stats
				? Math.max(
						...stats.historical.trend.map((point) => point.spent),
						0.0001,
					)
				: 0.0001,
		[stats],
		...
		return (
			<div className="grid grid-cols-1 gap-4 animate-pulse md:grid-cols-2">
				{[...Array(4)].map((_, index) => (
					<div
						key={index}
						className="h-32 rounded-2xl border border-border/10 bg-foreground/5"
					/>
				))}
			</div>
		);
		...
		return (
			<div className="flex items-center gap-3 rounded-2xl border border-red-500/20 bg-red-500/5 p-6">
				...
				<p className="text-xs text-muted-foreground">
					Error cargando métricas de costo. Reintentando...
				</p>
			</div>
		);
	}
					...
					<div>
						...
						<div className="mt-0.5 flex items-center gap-1.5">
							...
							<span className="text-[8px] font-mono uppercase tracking-widest text-muted-foreground/40">
								{stats.meta.source === "database" ? "DB" : "Memoria"} ·{" "}
								{lastUpdated}
							</span>
						</div>
					</div>
				...
				<div className="space-y-4 rounded-2xl border border-border/20 bg-foreground/[0.03] p-5">
					...
					{stats.historical.trend.length > 0 ? (
						...
						<div className="flex h-20 items-center justify-center">
							<span className="text-3xs uppercase tracking-widest text-muted-foreground/40">
								Sin datos aún
							</span>
						</div>
					)}
