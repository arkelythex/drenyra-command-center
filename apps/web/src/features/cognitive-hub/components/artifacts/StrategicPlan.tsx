...
export const StrategicPlan = ({ steps, onConfirm }: StrategicPlanProps) => {
	return (
		<div className="mt-6 p-6 rounded-3xl bg-foreground/[0.03] border border-border/20  space-y-6">
			<header className="flex items-center justify-between border-b border-border/10 pb-4">
				<div className="flex items-center gap-3">
					<div className="h-2 w-2 rounded-full bg-primary animate-pulse" />
					<span className="text-2xs font-black uppercase tracking-[0.3em] text-foreground">
						Proposed Execution Roadmap
					</span>
				</div>
				<span className="text-3xs font-mono text-muted-foreground uppercase opacity-40">
					Plan v1.4
				</span>
			</header>
				...
				{steps.map((step, idx) => (
					<div key={step.id} className="flex gap-4 group">
							...
							<div
								className={cn(
									...
									step.status === "completed"
										...
										: "bg-background border-border text-muted-foreground/20",
								)}
							>
								{step.status === "completed" ? (
						...
						<div className="flex-1 pb-4">
							<span className="text-2xs font-black uppercase text-foreground/80 tracking-widest">
								{step.label}
							</span>
							<p className="text-label text-muted-foreground mt-1 leading-relaxed italic">
								{step.description}
							</p>
						</div>
					</div>
			...
			<footer className="pt-4 flex gap-3">
				...
				<button
					type="button"
					className="px-6 py-3 rounded-2xl bg-foreground/5 border border-border/20 text-2xs font-black uppercase tracking-widest text-muted-foreground hover:bg-foreground/10 transition-all"
				>
					Refine
				</button>
			</footer>
