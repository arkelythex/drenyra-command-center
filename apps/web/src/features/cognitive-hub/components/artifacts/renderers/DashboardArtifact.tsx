...
export const DashboardArtifact: React.FC<{ artifact: DashboardArt }> = ({
	artifact,
}) => (
	<div className="mt-6 space-y-4 animate-entrance">
				...
				<header className="flex justify-between items-start">
					<div className="flex flex-col">
						...
						{artifact.payload.ruleSource && (
							<span className="text-[8px] font-mono text-primary uppercase mt-1">
								Fuente: {artifact.payload.ruleSource}
							</span>
						)}
					</div>
				...
				<div className="relative h-20 w-20 flex items-center justify-center mb-4">
					<svg className="absolute inset-0 w-full h-full -rotate-90">
						...
						<circle
							...
							strokeDasharray={226}
							strokeDashoffset={
								226 - (226 * artifact.payload.statusScore) / 100
							}
							strokeLinecap="round"
							className={cn(
								"transition-[color,stroke-dashoffset,filter] duration-700 shadow-glow",
								artifact.payload.statusScore > 90
					...
					<span className="text-xl font-black text-foreground tabular-nums">
						{artifact.payload.statusScore}%
					</span>
				</div>
				<span className="text-3xs font-black uppercase tracking-widest text-muted-foreground">
		...
		{artifact.payload.gapAnalysis && (
			<div className="p-5 hub-panel-inset space-y-4">
				<span className="text-3xs font-black uppercase text-muted-foreground tracking-[0.3em]">
						...
						<div key={i} className="space-y-1.5">
							...
							<div className="h-1 w-full bg-foreground/5 rounded-full overflow-hidden">
								<motion.div
									...
									className={cn(
										"h-full",
										gap.value > 80 ? "bg-foreground" : "bg-amber-500",
									)}
								/>
							</div>
						</div>
