...
export const SheetDiffArtifact: React.FC<{ artifact: SheetDiffArt }> = ({ artifact }) => {
  ...
  return (
    <section className={cn(tokensToClasses.borderRadius('card'), "mt-6 border border-border/35 bg-background/50 p-5 backdrop-blur-3xl")}>
      ...
      <div className="mb-4 grid gap-3 md:grid-cols-3">
        <div className="rounded-xl border border-border/35 bg-background/40 p-3">
          <p className="text-2xs uppercase tracking-widest text-muted-foreground">Registros</p>
          <p className="text-lg font-black">{artifact.payload.summary.total}</p>
        </div>
        <div className="rounded-xl border border-border/35 bg-background/40 p-3">
          <p className="text-2xs uppercase tracking-widest text-muted-foreground">Corregidos</p>
          <p className="text-lg font-black text-[var(--premium-success)]">{totals.updated}</p>
        </div>
        <div className="rounded-xl border border-border/35 bg-background/40 p-3">
          <p className="text-2xs uppercase tracking-widest text-muted-foreground">Requieren revisión</p>
          ...
          {rows.map((row) => (
            ...
            >
              <div>
                ...
                {row.reason ? (
                  <p className="mt-1 text-label text-muted-foreground">{row.reason}</p>
                ) : null}
              </div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <ShieldAlert size={14} />
          <span>
            Aceptar cambios con <strong>{artifact.payload.acceptShortcut}</strong>
          </span>
        </div>
        <button
          type="button"
