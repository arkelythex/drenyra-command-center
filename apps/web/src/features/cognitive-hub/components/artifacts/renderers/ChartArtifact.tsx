...
export const ChartArtifact: React.FC<{ artifact: ChartArt }> = ({ artifact }) => (
  <div className={cn(tokensToClasses.borderRadius('modal'), "group relative mt-6 overflow-hidden border border-border/20 bg-foreground/[0.03] p-8 shadow-xl backdrop-blur-md")}>
    ...
    <div className="flex items-center justify-between mb-8 relative z-10">
      <div>
        <h4 className="text-sm font-black uppercase tracking-[0.2em] text-foreground/60 antialiased">{artifact.title}</h4>
        ...
        return (
          ...
          >
            <motion.div
              initial={{ height: 0 }}
              animate={{ height: '100%' }}
              transition={{ delay: i * 0.05, duration: 1.5, ease: [0.2, 0.8, 0.2, 1] }}
              className={cn(
                'w-full rounded-t-xl transition-[background-color,box-shadow,transform] duration-300',
                isMax ? 'bg-foreground shadow-glow' : 'bg-foreground/20 group-hover/bar:bg-foreground/40'
              )}
            />
