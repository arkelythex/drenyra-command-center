...
export const TableArtifact: React.FC<{ artifact: TableArt }> = ({ artifact }) => (
  <div className="relative mt-6 overflow-hidden rounded-3xl border border-border bg-background p-6 shadow-xl animate-entrance">
    ...
    <header className="mb-8 pl-4">
      <h4 className="text-sm font-black uppercase tracking-widest text-foreground">{artifact.title}</h4>
      <p className="text-3xs font-mono text-muted-foreground mt-1 uppercase">Sustento Legal Inmutable · Secure Audit Log</p>
    </header>
      ...
      {artifact.payload.events.map((event, i) => (
        <div key={i} className="relative flex gap-6 group">
          <div className={cn(
            ...
            event.type === 'decision' ? 'border-foreground scale-110 shadow-glow' : 'border-border'
          )} />

          <div className="flex-1 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-2xs font-black uppercase text-foreground/80 tracking-wider">{event.agent}</span>
              <span className="text-[8px] font-mono text-muted-foreground">{event.time}</span>
            </div>
              ...
              <div className="mt-3 pt-3 border-t border-border/10 flex flex-wrap items-center gap-3">
                {event.impact && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-3xs font-black uppercase text-foreground tracking-widest antialiased">Impact: {event.impact}</span>
                  </div>
                )}
                {event.rule && (
                  <div className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-primary/10 border border-primary/20">
                    ...
                    <span className="text-[8px] font-black text-primary uppercase tracking-tighter antialiased">Ref: {event.rule}</span>
                  </div>
                )}
              </div>
            </div>
