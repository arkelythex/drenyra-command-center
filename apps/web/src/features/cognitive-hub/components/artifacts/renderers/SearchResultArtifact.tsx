...
export const SearchResultArtifact: React.FC<{ artifact: SearchResultArt }> = ({
	artifact,
}) => (
	<div className="mt-6 rounded-xl border border-border/70 bg-card/92 p-5 shadow-xl animate-entrance">
		...
		<div className="space-y-4">
			{artifact.payload.results.map((res, idx) => (
				<div
					key={idx}
					className="rounded-lg border border-border/70 bg-background/50 p-4 transition-[background-color,border-color,box-shadow] duration-200 hover:border-border"
				>
					<div className="flex items-center justify-between mb-2">
						<span className="text-3xs font-black uppercase tracking-widest text-info">
							{res.source}
						</span>
