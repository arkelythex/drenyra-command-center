...
export const ComparisonArtifact: React.FC<{ artifact: ComparisonArt }> = ({ artifact }) => (
  <div className={cn(tokensToClasses.borderRadius('modal'), "mt-6 border border-border/40 bg-foreground/[0.03] p-8 shadow-xl backdrop-blur-md")}>
    <header className="flex items-center gap-4 mb-10">
      ...
      <div>
        ...
        <p className="text-3xs text-muted-foreground font-black uppercase tracking-widest mt-0.5 italic">Cross-Mission Optimization Engine</p>
      </div>
    </header>
      ...
      {artifact.payload.scenarios.map((sc, idx) => (
        <div key={idx} className="space-y-6">
          <div className="flex items-center justify-between border-b border-border/20 pb-4">
            <span className="text-2xs font-black uppercase tracking-widest text-foreground/40 italic">Escenario {idx + 1}</span>
            <span className="text-xs font-black uppercase text-foreground">{sc.name}</span>
          </div>

          <div className="space-y-4">
            {sc.metrics.map((m, mIdx) => (
              <div key={mIdx} className="flex justify-between items-center group">
                <span className="text-2xs font-bold text-muted-foreground uppercase">{m.label}</span>
                <div className="flex flex-col items-end">
                  <span className={cn(
                    'text-sm font-mono font-black tabular-nums',
                    m.highlight ? 'text-[var(--premium-success)]' : 'text-foreground'
                  )}>{m.value}</span>
                  {m.delta && (
                    <span className={cn('text-[8px] font-black', m.delta > 0 ? 'text-[var(--premium-success)]' : 'text-red-500')}>
                      {m.delta > 0 ? '+' : ''}{m.delta}%
                    </span>
                  )}
                </div>
