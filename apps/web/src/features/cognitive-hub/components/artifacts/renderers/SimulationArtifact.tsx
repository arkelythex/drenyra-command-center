...
export const SimulationArtifact: React.FC<{ artifact: SimulationArt }> = ({ artifact }) => {
  const { commitToLedger } = useAgentActions();

  return (
    <div className={cn(tokensToClasses.borderRadius('card'), "mt-6 overflow-hidden border border-border/40 bg-foreground/10 p-1 shadow-xl backdrop-blur-md")}>
      <div className="p-6 bg-background/40 rounded-[1.8rem] border border-border/20 space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <p className="text-3xs text-muted-foreground font-black uppercase tracking-widest">Simulación contable</p>
              </div>
            </div>
          </div>
          <div className="px-2 py-1 rounded bg-foreground text-background text-[8px] font-black uppercase tracking-widest">Vista previa</div>
        </header>
          ...
          <table className="w-full text-label font-mono">
            ...
            <tbody className="divide-y divide-border/10">
              {artifact.payload.entries?.map((row, i) => (
                <tr key={i} className="hover:bg-foreground/[0.02] transition-colors text-foreground/80">
                  <td className="px-4 py-3">{row.account}</td>
                  <td className="px-4 py-3 text-right tabular-nums">{row.debit > 0 ? row.debit.toLocaleString() : '-'}</td>
                  <td className="px-4 py-3 text-right tabular-nums">{row.credit > 0 ? row.credit.toLocaleString() : '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="p-4 rounded-xl bg-foreground/[0.03] border border-border/10 space-y-3">
          <span className="text-3xs font-black uppercase text-muted-foreground tracking-widest">Análisis de impacto</span>
          <div className="grid grid-cols-2 gap-6">
            <div className="flex flex-col gap-1">
              <span className="text-[8px] font-bold text-muted-foreground uppercase">Resultado Neto</span>
              <span className="text-xs font-black text-[var(--premium-success)]">+ S/ 1,200.00</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[8px] font-bold text-muted-foreground uppercase">Carga Impositiva</span>
              <span className="text-xs font-black text-foreground">S/ 450.00</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button className="rounded-2xl border border-border/20 bg-foreground/5 px-8 py-3.5 text-2xs font-black uppercase tracking-widest text-muted-foreground transition-[background-color,border-color,color,transform] duration-200 hover:bg-foreground/10 hover:text-foreground">
            Cancelar
          </button>
        </div>
      </div>
