...
export const ExplanationArtifact: React.FC<{ artifact: ExplanationArt }> = ({ artifact }) => (
  <div className="relative mt-6 overflow-hidden hub-panel-inset p-5 animate-entrance border-success-subtle">
    ...
    <div className="space-y-6 relative z-10">
      <header>
        <h4 className="text-base font-black uppercase tracking-tight text-foreground mb-1">{artifact.title}</h4>
        <div className="flex items-center gap-3">
          <span className="text-2xs font-mono font-black text-success uppercase tracking-widest">Análisis del agente auditor</span>
          <span className="h-1.5 w-1.5 rounded-full bg-success animate-pulse" />
          <span className="text-2xs font-mono font-black text-muted-foreground uppercase tracking-widest">{artifact.metadata?.agent}</span>
        </div>
      </header>
      ...
      <div className="space-y-4">
        <span className="px-1 text-3xs font-black uppercase tracking-[0.22em] text-muted-foreground">Lógica de decisión</span>
        <div className="grid grid-cols-1 gap-3">
          {[
            { label: 'Confianza de Acción', value: '99.9%', icon: ShieldCheck },
            { label: 'Ref. Legislativa', value: 'Resolución 2026-SUNAT', icon: BookMarked },
            { label: 'Impacto Fiscal', value: 'Ajuste de Céntimos', icon: Zap },
          ].map((stat, i) => (
            <div key={i} className="flex items-center justify-between rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-3)] p-4">
              <div className="flex items-center gap-3">
                <stat.icon size={14} className="text-success" />
                <span className="text-2xs font-bold text-muted-foreground uppercase">{stat.label}</span>
              </div>
              <span className="text-2xs font-mono font-black text-foreground">{stat.value}</span>
            </div>
          ))}
        </div>
      </div>
