...
export const HubRightRail = ({
	...
	return (
		<motion.aside
			initial={prefersReducedMotion ? { opacity: 0 } : { width: 0, opacity: 0, x: 20 }}
			animate={{ width: 440, opacity: 1, x: 0 }}
			exit={prefersReducedMotion ? { opacity: 0 } : { width: 0, opacity: 0, x: 20 }}
			transition={{ duration: 0.3, ease: [0.19, 1, 0.22, 1] }}
			className="relative z-50 flex h-full shrink-0 flex-col border-l border-white/10 bg-background/60 backdrop-blur-3xl shadow-[-20px_0_50px_rgba(0,0,0,0.1)]"
		>
			<div className="flex-1 overflow-hidden">
				<Suspense fallback={<HubRightRailPanelFallback label="Cargando artefacto" />}>
					<HubArtifactAside activeArtifact={activeArtifact} onClose={onCloseArtifact} />
				</Suspense>
			</div>
		</motion.aside>
	);
...
function HubRightRailPanelFallback({ label }: { label: string }) {
  return (
    <div
      className="h-full space-y-4 p-5"
      role="status"
      aria-live="polite"
    >
