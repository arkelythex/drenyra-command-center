...
export function buildDefaultDiscrepancyScenario(): DiscrepancyScenario {
  ...
  return {
    ...
    recommendation: 'Aplicar ajuste de redondeo y actualizar el asiento antes del cierre.',
    rows,
    originalData: [
      {
        documento: 'F001-4482',
    ...
    legalReferences: [
      ...
      },
      {
        ...
        sourceUrl: 'https://www.sunat.gob.pe/legislacion/superin/2021/112-2021.pdf',
...
export function buildDiscrepancyArtifactFromScenario(scenario: DiscrepancyScenario): HubArtifact {
  const updated = scenario.rows.filter((row) => row.status === 'updated').length;
  const flagged = scenario.rows.filter((row) => row.status === 'flagged').length;
