...
export const DiscrepancyComposer = ({
	...
	const flaggedCount = scenario.rows.filter(
		(row) => row.status === "flagged",
	).length;
	const updatedCount = scenario.rows.filter(
			...
			{isOpen ? (
				...
				>
					...
					<div className="mb-4 grid gap-2 rounded-[var(--radius-md)] border border-warning-subtle bg-warning-subtle p-3 sm:grid-cols-3">
						<div>
							<p className="text-2xs uppercase tracking-[0.1em] text-muted">
								Rows con conflicto
							</p>
						...
						<article className="rounded-lg border border-warning-subtle bg-warning-subtle p-3">
							...
							{isError ? (
								<p className="mt-2 text-label text-danger">
									Error al confirmar. Revisa impacto y vuelve a intentar la
									confirmación.
								</p>
							) : null}
							{isPendingUndo ? (
								<div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--surface-3)]">
									<div
										...
										style={{
											width: `${Math.min(100, Math.max(0, (undoSecondsLeft / 10) * 100))}%`,
										}}
									/>
								</div>
							) : null}
