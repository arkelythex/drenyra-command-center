	...
	return (
		<div className="rounded-xl border border-amber-400/20 bg-card p-4">
			...
			<p className="mb-3 text-2xs font-medium text-muted-foreground">
				Utilidades de prueba operativa (HITL/fallback), fuera de navegación
				principal.
			</p>
			...
			{error ? (
				<p className="mt-2 text-label text-destructive">{error}</p>
			) : null}
		</div>
	);
}
