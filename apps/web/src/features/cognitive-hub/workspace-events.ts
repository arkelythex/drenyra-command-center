export const COGNITIVE_WORKSPACE_ACTION_EVENT =
	"drenyra:cognitive-workspace-action";
...
export function dispatchCognitiveWorkspaceAction(
	action: CognitiveWorkspaceAction,
) {
	if (typeof window === "undefined") return;

	window.dispatchEvent(
		new CustomEvent<CognitiveWorkspaceActionDetail>(
			COGNITIVE_WORKSPACE_ACTION_EVENT,
			{
				detail: { action },
			},
		),
