...
function mapCsvRowsToDiff(text: string): SheetDiffRow[] {
  ...
  const totalIndex = headers.findIndex((header) => /total|importe|monto/.test(header));
  ...
  return lines.slice(1, 13).map((line, index) => {
    ...
    const subtotal = totalValue !== null && igvValue !== null ? totalValue - igvValue : null;
    const expectedIgv = subtotal !== null ? Number((subtotal * 0.18).toFixed(2)) : null;
    const correctedTotal =
      subtotal !== null && expectedIgv !== null ? Number((subtotal + expectedIgv).toFixed(2)) : totalValue;
    const gap =
      totalValue !== null && correctedTotal !== null ? Math.abs(totalValue - correctedTotal) : 0;
    ...
    const record = [serie && number ? `${serie}-${number}` : `Fila ${index + 2}`, ruc && `RUC ${ruc}`]
      .filter(Boolean)
      .join(' · ');
...
export async function buildSheetDiffArtifactFromInput(input: {
  ...
  if (!matchedFile) {
    return buildSheetDiffArtifact(input.content, 'Entrada manual', buildFallbackRows());
  }
  ...
  try {
    ...
    return buildSheetDiffArtifact(input.content, matchedFile.name, buildFallbackRows());
  }
}
