...
function looksLikeNaturalLanguageQuestion(input: string): boolean {
  return /[?¿]/.test(input) || /^(cómo|como|qué|que|cuándo|cuando|dónde|donde|por qué|porque)\b/i.test(input);
}
...
export function extractNavigationIntent(input: string): NavigationIntent | null {
  const normalized = input.trim().toLowerCase();
  const slashCommand = resolveSlashCommand(normalized);
  ...
  for (const patternConfig of NAVIGATION_PATTERNS) {
    if (!patternConfig.patterns.some((pattern) => pattern.test(normalized))) continue;
...
export function getNavigationSuggestions(
  ...
  const suggestions = NAVIGATION_PATTERNS.map(({ patterns, target, title }) => {
    const matches = patterns.filter((pattern) => pattern.test(normalized)).length;
