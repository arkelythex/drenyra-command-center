			...
			{open && (
				<>
					...
					<div className="absolute right-0 top-full z-20 mt-1 w-52 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)] py-1 shadow-lg">
						...
						<button
							type="button"
							className="flex w-full items-center gap-2 px-4 py-2 text-xs text-[var(--color-success)] hover:bg-[var(--surface-2)]"
							onClick={() => setOpen(false)}
						>
							<ShieldCheck size={14} />
							Approve
						...
						<button
							type="button"
							className="flex w-full items-center gap-2 px-4 py-2 text-xs text-[var(--color-danger)] hover:bg-[var(--surface-2)]"
							onClick={() => setOpen(false)}
						>
							<ThumbsDown size={14} />
							Reject
