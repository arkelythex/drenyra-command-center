...
import { SettingSwitch, SettingsSection } from '@/features/settings/components/SettingsPrimitives';
      ...
      >
        <div className="flex flex-col gap-5 rounded-2xl border border-border/60 bg-background/40 p-4 sm:flex-row sm:items-center">
          ...
          <div className="min-w-0">
            <p className="text-lg font-black uppercase tracking-tight text-foreground">{fullName}</p>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input value={email} onChange={(event) => setEmail(event.target.value)} className="pl-10" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input value={phone} onChange={(event) => setPhone(event.target.value)} className="pl-10" />
            </div>
          </div>
        </div>
      </SettingsSection>
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <Input value={cpcc} onChange={(event) => setCpcc(event.target.value)} className="font-mono" />
          </div>
        ...
        <div className="rounded-2xl border border-border/60 bg-background/50 p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-foreground">Firma profesional validada</p>
              <p className="text-xs text-muted-foreground">
                Tu identidad contable está habilitada para aprobaciones y reportes auditables.
              </p>
            </div>
          </div>
        </div>
      ...
      >
        <div className="space-y-3 rounded-2xl border border-border/60 bg-background/40 p-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-bold text-foreground">Mostrar perfil en directorio interno</p>
              <p className="text-xs text-muted-foreground">Permite que el equipo vea tu cargo y datos de contacto.</p>
            </div>
            <SettingSwitch
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-bold text-foreground">Permitir menciones en tareas</p>
              <p className="text-xs text-muted-foreground">Recibirás notificaciones cuando te asignen revisiones o cierres.</p>
            </div>
            <SettingSwitch checked={allowMentions} onCheckedChange={setAllowMentions} label="Permitir menciones" />
          </div>
        </div>
      </SettingsSection>
    </SettingsShell>
