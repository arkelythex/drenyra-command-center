...
export function CashflowForecastView({
	...
	return (
		<div className="grid h-full gap-6 lg:grid-cols-[minmax(0,1.2fr)_380px]">
			<Card
				variant="default"
				className={`border-[var(--border-default)] bg-[var(--surface-1)] p-6 ${glassClassName}`}
			>
				<div className="flex items-center justify-between gap-4 border-b border-border/50 pb-4">
					<div>
						<Text
							variant="label"
							className="text-label tracking-[0.24em] text-muted-foreground"
						>
							FORECAST MENSUAL
						</Text>
					...
					{forecast.forecast.map((month) => (
						...
						>
							<div>
								<Text
									variant="label"
									className="text-2xs tracking-[0.2em] text-muted-foreground"
								>
									MES
								</Text>
			...
			<div className="space-y-4">
				<Card
					variant="default"
					className="border-[var(--border-default)] bg-[var(--surface-1)] p-5"
				>
					<Text
						variant="label"
						className="text-label tracking-[0.22em] text-muted-foreground"
					>
						SEÑAL OPERATIVA
					</Text>
					<Text variant="data" className="mt-2 text-2xl">
					...
					<p className="mt-2 text-xs text-muted-foreground">
						Proyectado {formatMoney(projection.summary.netCashflow)} vs real{" "}
						{formatMoney(actual.netCashflow)}
					</p>
				</Card>
