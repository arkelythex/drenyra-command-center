...
export function CashflowBoardHeader({
	...
	return (
		<>
			...
			<div className="relative z-40 mt-14 flex flex-col gap-6 border-b border-border/50 bg-background/40 px-4 py-4  sm:hidden">
				<div className="flex w-full items-center gap-2">
					...
					<div className="flex items-center gap-2">
						<button
							type="button"
							className="flex h-10 w-10 items-center justify-center rounded-xl bg-foreground text-background shadow-sm transition-[background-color,box-shadow,transform] hover:scale-[1.03] hover:bg-foreground/90 active:scale-95"
						>
				...
				<div className="relative z-10 flex w-full items-center gap-5 md:w-auto">
					...
					<div className="min-w-0 flex-1">
						...
						<div className="mt-1.5 flex items-center gap-3">
							...
							<div
								className={`rounded-lg border border-border/50 bg-muted/30 p-0.5 shadow-sm ${glassBackdropClassName}`}
							>
								{(["board", "forecast"] as const).map((view) => (
									<Button
										...
										size="sm"
										onClick={
											view === "board" ? onSetBoardView : onSetForecastView
										}
										className={cn(
				...
				<div className="mt-4 flex w-full items-center gap-4 md:mt-0 md:w-auto">
					<div className="hidden items-center gap-3 rounded-xl border border-border/50 bg-card px-4 py-2 shadow-sm sm:flex">
						...
						<div className="flex flex-col">
							...
							<Text
								variant="data"
								className="text-sm leading-none tabular-nums text-foreground"
							>
								{totalCashLabel}
							</Text>
						</div>
