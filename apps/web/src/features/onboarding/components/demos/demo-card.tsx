...
export const DemoCard = ({ demo, onOpen }: DemoCardProps) => {
	...
	return (
		// biome-ignore lint/a11y/useKeyWithClickEvents: demo card opens player on click
		<article
			className={cn(
				tokensToClasses.borderRadius("card"),
				"group relative cursor-pointer overflow-hidden border border-border/30 bg-[var(--surface-1)] p-1 shadow-xl",
			)}
			...
			<div className="flex h-full flex-col space-y-5 rounded-[1.8rem] border border-border/10 bg-background/60 p-6">
				...
				<div className="grid grid-cols-2 gap-2">
					...
					<div className="rounded-xl border border-border/10 bg-foreground/[0.03] p-2">
						...
						<span className="text-xs font-mono font-black text-foreground/60">
							{demo.resolutionTimeSeconds}s
						</span>
					</div>
				</div>
				...
				<button
					type="button"
					className="shadow-glow flex w-full items-center justify-center gap-2 rounded-xl bg-foreground py-3 text-3xs font-black uppercase tracking-[0.2em] text-background transition-[background-color,box-shadow,opacity] duration-150 group-hover:opacity-90 active:scale-95 motion-reduce:transition-none motion-reduce:active:scale-100"
				>
