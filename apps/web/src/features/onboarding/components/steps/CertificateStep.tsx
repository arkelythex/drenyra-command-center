...
import { Key, UploadCloud } from "lucide-react";
		...
		<div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
			<div className="space-y-2">
				...
				<p className="text-sm text-muted-foreground">
					Puedes cargarlo ahora o terminar el setup y configurarlo después.
				</p>
			</div>
			...
			<div className="border-2 border-dashed border-border rounded-xl p-8 flex flex-col items-center justify-center gap-4 hover:bg-muted/50 transition-colors cursor-pointer">
				...
				{data.certificate && (
					<p className="text-xs text-[var(--premium-success)] font-bold">
			...
			<div className="space-y-2">
				<label className="text-2xs font-black uppercase tracking-widest text-muted-foreground">
					Credenciales SOL (Usuario Secundario)
				</label>
				<div className="grid grid-cols-2 gap-4">
					<Input
						placeholder="Usuario SOL"
						value={data.solUser || ""}
						onChange={(e) => onUpdate({ solUser: e.target.value })}
					/>
					<Input
						type="password"
						placeholder="Clave SOL"
						value={data.solPass || ""}
						onChange={(e) => onUpdate({ solPass: e.target.value })}
					/>
				</div>
			</div>
