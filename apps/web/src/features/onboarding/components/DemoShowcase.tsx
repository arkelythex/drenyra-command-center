...
export const DemoShowcase = ({ autoPlayId, onComplete }: DemoShowcaseProps) => {
  const [activeDemo, setActiveDemo] = useState<DemoId | null>(autoPlayId ?? null);

  return (
    <>
      {activeDemo ? <DemoPlayer demoId={activeDemo} onClose={() => setActiveDemo(null)} /> : null}
      ...
      <section className="w-full space-y-8">
        <div className="space-y-3 text-center">
          ...
          <h2 className="text-2xl font-black uppercase tracking-tighter text-foreground">Lo que ARKELYTHEX hace</h2>
          <p className="mx-auto max-w-md text-sm text-muted-foreground">
            Tres escenarios reales que demuestran cómo el enjambre de agentes protege tu empresa de
            multas, errores y pérdidas fiscales.
          </p>
        </div>
