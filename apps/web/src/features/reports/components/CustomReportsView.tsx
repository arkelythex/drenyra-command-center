import { MotionDiv, containerVariants, entranceVariants } from '@/components/ui/motion-primitives';
...
export const CustomReportsView = () => {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-background/50 font-sans text-foreground backdrop-blur-sm">
      <ReportsHeader
        ...
        <div className="mx-auto max-w-[1920px]">
          ...
          >
            {MOCK_REPORTS.map((report) => (
              <MotionDiv key={report.id} variants={entranceVariants} className="group flex h-[460px] flex-col">
                <ReportCard report={report} onOpen={() => trigger('light')} onDownload={() => trigger('light')} />
              </MotionDiv>
            ))}
