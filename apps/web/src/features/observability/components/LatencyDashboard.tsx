...
import type { LatencySummary, LatencyTrendItem, LatencyRecentEvent } from "../types";
  ...
  return (
    ...
    >
      <Card
        ...
        animateOnHover
        className={cn(
          highlight && "ring-1 ring-[var(--color-warning)]/30",
        )}
      >
...
function PercentileBars({ summary }: { summary: LatencySummary }) {
  const bars = useMemo(() => {
    const items = [
      { label: "Avg", key: "avgLatencyMs" as const, color: "var(--color-info, #3b82f6)", value: summary.avgLatencyMs },
      { label: "P50", key: "p50LatencyMs" as const, color: "var(--color-success, #22c55e)", value: summary.p50LatencyMs },
      { label: "P95", key: "p95LatencyMs" as const, color: "var(--color-warning, #f59e0b)", value: summary.p95LatencyMs },
      { label: "P99", key: "p99LatencyMs" as const, color: "var(--color-danger, #ef4444)", value: summary.p99LatencyMs },
    ];
