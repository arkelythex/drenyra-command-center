...
import { Brain, Clock, AlertTriangle, RefreshCw } from "lucide-react";
    ...
    <div className="flex flex-col items-center gap-3 px-4 py-12 text-center">
      ...
      <div>
        ...
        <p className="mt-1 max-w-xs text-2xs text-[var(--text-tertiary)]">
          Once the AI Control Plane completes invoice processing runs, each run with a generated
          memory summary will appear here in chronological order.
        </p>
      </div>
    </div>
  );
    ...
    <Card variant="glass" padding="lg">
      <CardHeader className="mb-3">
        <div className="flex items-center gap-2">
          ...
          {isLoading && (
            <span className="ml-auto text-2xs text-[var(--text-tertiary)]">Loading…</span>
          )}
        </div>
      </CardHeader>
        ...
        {isLoading ? (
          ...
          <div className="pl-1">
            {entries.map((entry, index) => (
              <AgentMemoryTimelineEntry key={entry.runId} entry={entry} index={index} />
            ))}
          </div>
        )}
      </CardContent>
