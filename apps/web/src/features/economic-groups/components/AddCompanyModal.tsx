...
export const AddCompanyModal: React.FC<AddCompanyModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      {/* Backdrop */}
      <div
        className="absolute inset-0 ui-overlay backdrop-blur-sm"
        onClick={handleClose}
        role="presentation"
        tabIndex={-1}
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg backdrop-blur-xl bg-card/95 border border-border rounded-2xl shadow-2xl p-6">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="backdrop-blur-xl bg-primary/20 p-2 rounded-lg">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              maxLength={11}
              className="w-full backdrop-blur-xl bg-card/70 border border-border rounded-lg px-4 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 transition-colors font-mono"
              disabled={isSubmitting}
            />
            {errors.ruc && (
          ...
          <div>
            ...
            <input
              ...
              className="w-full backdrop-blur-xl bg-card/70 border border-border rounded-lg px-4 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 transition-colors"
              disabled={isSubmitting}
            />
            {errors.businessName && (
              <p className="text-sm text-red-400 mt-1">{errors.businessName.message}</p>
            )}
          </div>
          ...
          {/* Error Message */}
          {error && (
            <div className="backdrop-blur-xl bg-red-500/10 border border-red-500/20 rounded-lg p-3">
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={handleClose}
              className="flex-1 backdrop-blur-xl bg-card/70 hover:bg-muted/70 border border-border text-foreground font-medium py-3 rounded-lg transition-colors"
              disabled={isSubmitting}
            >
              Cancelar
            </button>
