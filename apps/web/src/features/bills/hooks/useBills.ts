...
import { BILLS_STALE_TIME_MS, STATUS_TRANSITIONS, SUCCESS_MESSAGES } from './use-bills.constants';
...
import type { Bill, BillsView, BillStatus } from './use-bills.types';
import { persistBillStatusTransition } from './use-bills.workflow';

export type { Bill, BillApprovalState, BillStatus, BillWorkflowEvent } from './use-bills.types';
  ...
  return previous.map((bill) =>
    bill.id === billId
      ? {
          ...bill,
          status: nextStatus,
          paidDate: nextStatus === 'paid' ? new Date().toISOString() : bill.paidDate,
        }
      : bill,
  );
}
...
export const useBills = () => {
  ...
  const columnTotals = useMemo(() => getColumnTotals(billsByStatus), [billsByStatus]);
