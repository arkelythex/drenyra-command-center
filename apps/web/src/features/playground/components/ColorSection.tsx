...
function TextPreview({ variable, value, label }: ColorCard) {
	return (
		<div className="flex items-center gap-4 px-4 py-3 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-1)]">
			<span className="text-xs font-mono text-[var(--text-muted)] w-36 shrink-0 truncate">
				{variable}
			</span>
			<span className="text-sm text-[var(--text-muted)] w-20 shrink-0">
				{value}
			</span>
...
export function ColorSection() {
	...
	useEffect(() => {
		...
		const current =
			document.documentElement.getAttribute("data-accent") || "ember";
	...
	const surfaceColors: ColorCard[] = [
		{
			label: "Background 0",
			variable: "--color-bg-0",
			value: hexFromRgb(resolvedVars["--color-bg-0"] || ""),
		},
		{
			label: "Background 1",
			variable: "--color-bg-1",
			value: hexFromRgb(resolvedVars["--color-bg-1"] || ""),
		},
		{
			label: "Background 2",
			variable: "--color-bg-2",
			value: hexFromRgb(resolvedVars["--color-bg-2"] || ""),
		},
		{
			label: "Surface 1",
			variable: "--surface-1",
			value: hexFromRgb(resolvedVars["--surface-1"] || ""),
		},
		{
			label: "Surface 2",
			variable: "--surface-2",
			value: hexFromRgb(resolvedVars["--surface-2"] || ""),
		},
		{
			label: "Surface 3",
			variable: "--surface-3",
			value: hexFromRgb(resolvedVars["--surface-3"] || ""),
		},
	];
	...
	const textColors: ColorCard[] = [
		{
			...
			value: resolvedVars["--text-primary"] || "",
		},
		{
			label: "Secondary",
			variable: "--text-secondary",
			value: resolvedVars["--text-secondary"] || "",
		},
		{
			label: "Muted",
			variable: "--text-muted",
			value: resolvedVars["--text-muted"] || "",
		},
		{
			label: "Disabled",
			variable: "--text-disabled",
			value: resolvedVars["--text-disabled"] || "",
		},
	];
	...
	const semanticColors: ColorCard[] = [
		{
			...
			value: hexFromRgb(resolvedVars["--color-success"] || ""),
		},
		{
			label: "Warning",
			variable: "--color-warning",
			value: hexFromRgb(resolvedVars["--color-warning"] || ""),
		},
		{
			label: "Danger",
			variable: "--color-danger",
			value: hexFromRgb(resolvedVars["--color-danger"] || ""),
		},
		{
			label: "Info",
			variable: "--color-info",
			value: hexFromRgb(resolvedVars["--color-info"] || ""),
		},
	];
