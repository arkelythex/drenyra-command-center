...
function groupVar(name: string): string {
	...
	if (name.startsWith("--bg-") || name.startsWith("--surface"))
		return "surface";
	if (name.startsWith("--text-")) return "text";
	if (name.startsWith("--border-") || name.startsWith("--stroke"))
		return "border";
						...
						<div key={group}>
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
								{items.map((v) => (
									...
									>
										<span className="text-[var(--text-muted)] shrink-0 max-w-[200px] truncate">
											{v.name}
										</span>
