...
describe("useInventory", () => {
	...
	it("falls back to demo-mode company key when auth user is missing", async () => {
		...
		await waitFor(
			() => {
				expect(
					queryClient.getQueryData(["inventory", "list", "demo-mode"]),
				).toBeTruthy();
			},
			{ timeout: 2500 },
		);
		...
		expect(
			queryClient.getQueryData(["inventory", "summary", "demo-mode"]),
		).toBeTruthy();
		expect(
			queryClient.getQueryData(["inventory", "alerts", "demo-mode"]),
		).toBeTruthy();
		expect(
			queryClient.getQueryData(["inventory", "list", "demo-mode"]),
		).toBeTruthy();
