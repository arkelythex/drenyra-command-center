...
import { MotionDiv, containerVariants, entranceVariants, AnimatedNumber } from '@/components/ui/motion-primitives';
import { PackageSearch, Warehouse, AlertTriangle, Search } from 'lucide-react';
      ...
      <div className="h-full flex flex-col bg-[var(--bg-0)] font-sans text-foreground overflow-hidden relative">
        ...
        <div className="relative z-40 mt-14 flex flex-col gap-6 border-b border-[var(--border-default)] bg-[var(--bg-1)] px-4 py-4 shadow-md sm:hidden">
            <div className="flex gap-3 w-full items-center">
                ...
                <Button variant="outline" size="icon" aria-label="Almacén" className="h-12 w-12 rounded-2xl border-border shadow-md hover:bg-muted/70">
                    <Warehouse size={18} />
                </Button>
            </div>
        </div>
            ...
            <div className="flex items-center gap-6 relative z-10 w-full md:w-auto group">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-2xs font-bold text-muted-foreground/70 uppercase tracking-[0.16em]">Corte operativo</span>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-row items-center gap-4 w-full md:w-auto relative z-10">
                 ...
                 <div className="hidden xl:flex items-center gap-10 px-8 border-r border-border font-mono">
                    <div className="text-right space-y-1">
                        <span className="block text-3xs font-black uppercase tracking-widest text-muted-foreground/50">Valor total</span>
                        <AnimatedNumber 
                          value={parseFloat(metrics.totalValorization.replace(/[^0-9.]/g, ''))} 
                          formatter={PEN_FORMATTER} 
                          className="text-lg font-black tracking-tighter text-foreground tabular-nums"
                        />
                    </div>
                    <div className="text-right space-y-1">
                        <span className="block text-3xs font-black uppercase tracking-widest text-muted-foreground/50">Rotación</span>
                        <p className="text-lg font-black tracking-tighter text-primary tabular-nums">{metrics.rotationRate}</p>
                    </div>
                 </div>

                 <Button variant="outline" className="h-12 rounded-2xl border-border px-8 text-2xs font-black uppercase tracking-[0.2em] text-foreground shadow-sm transition-[background-color,border-color,transform,box-shadow] duration-200 hover:bg-muted/70 hover:shadow-md">
        ...
        >
          ...
          <div className={cn(
                ...
                (activeTab === 'resumen' || activeTab === 'operaciones') ? "flex" : "hidden lg:flex"
            )}>
            <MotionDiv 
                variants={entranceVariants} 
                className={cn("shrink-0", activeTab === 'resumen' ? "block" : "hidden lg:block")}
            >
            ...
            >
              <div className="px-10 py-8 border-b border-border flex flex-col sm:flex-row justify-between items-center bg-muted/40 gap-6">
                  <div className="flex flex-col gap-1">
                      ...
                      <p className="text-2xs font-medium text-muted-foreground/60 uppercase tracking-widest pl-5">Kardex operativo por almacén</p>
                  </div>
                  <div className="flex bg-muted/60 p-1 rounded-xl border border-border shadow-inner">
                      {['Entradas', 'Salidas', 'Transferencias'].map(filter => (
                          <button key={filter} className="h-8 rounded-lg px-5 text-3xs font-black uppercase tracking-widest text-muted-foreground/70 transition-[background-color,color] duration-200 hover:bg-muted/70 hover:text-foreground">
                              {filter}
                          </button>
                      ))}
                  </div>
          ...
          <div className={cn(
            ...
            <MotionDiv variants={entranceVariants} className="overflow-hidden rounded-3xl border border-[var(--border-default)] bg-[var(--surface-1)] shadow-xl transition-[background-color,border-color,box-shadow] duration-200 hover:bg-[var(--surface-2)] hover:shadow-lg">
               <CostStructure />
             </MotionDiv>
             <MotionDiv variants={entranceVariants} className="overflow-hidden rounded-3xl border border-[var(--border-default)] bg-[var(--surface-1)] shadow-xl transition-[background-color,border-color,box-shadow] duration-200 hover:bg-[var(--surface-2)] hover:shadow-lg">
               <SmartProcurement />
