...
export const PaymentPreviewArtifactCard = ({
	artifact,
	onEvent,
}: PaymentPreviewArtifactCardProps) => {
	...
	usePaymentPreviewKeyboardShortcuts({
		...
		hasDraft: (beneficiaryId) =>
			Boolean(controller.draftsByBeneficiary[beneficiaryId]),
		...
		onApplyInlineEdit: (beneficiaryId) =>
			void controller.applyInlineEditById(beneficiaryId),
		...
		<div className="space-y-4">
			<div className="grid grid-cols-1 gap-3 md:grid-cols-3">
				...
				<div className="rounded-2xl border border-border bg-card/70 p-3">
					...
					<p className="mt-1 text-sm font-black">
						{formatMoney(controller.totalAmount, artifact.data.currency)}
					</p>
				</div>
			</div>
			...
			<PaymentBeneficiaryList
				...
				onPromptChange={(beneficiaryId, prompt) =>
					controller.setPromptsByBeneficiary((prev) => ({
						...prev,
						[beneficiaryId]: prompt,
					}))
				}
				onSuggestInlineEdit={controller.suggestInlineEditById}
				onApplyInlineEdit={(beneficiaryId) =>
			...
			<div className="flex flex-wrap items-center justify-end gap-2">
				<ActionButton
					icon={<Copy size={12} className="mr-1" />}
					label="Copiar Tabla"
					onClick={controller.handleCopyTable}
				/>
				<ActionButton
