		...
		try {
			...
			onEvent(
				emitPaymentEvent(
					artifact,
					"copy-table",
					"Se copio tabla de beneficiarios al portapapeles.",
				),
			);
		} catch {
	...
	const handleExportExcel = () => {
		...
		onEvent(
			emitPaymentEvent(
				artifact,
				"export-excel",
				"Se exporto tabla de beneficiarios a CSV.",
			),
		);
	};
	...
	const handleConfirmPayment = async () => {
		...
		onEvent({
			...
			payload: {
				policy: {
					key:
						confirmAction?.policyGate?.policyKey ?? "PAYMENT_BATCH_EXECUTION",
	...
	const handleCancel = () => {
		onEvent(
			emitPaymentEvent(
				artifact,
				"cancel-payment",
				"Se canceló la vista previa del lote de pagos.",
				"ROLLED_BACK",
			),
		);
	};
	...
	const handleDownloadVoucher = () => {
		onEvent(
			emitPaymentEvent(
				artifact,
				"download-voucher",
				"Se generó el resumen del lote en formato PDF.",
			),
		);
	};
