...
export function assessInlineEditRisk(
	before: number,
	after: number,
): InlineEditRiskAssessment {
...
export function getNextSelectionId(
	...
	const nextIndex =
		(safeIndex + delta + beneficiaries.length) % beneficiaries.length;
...
export function ensureSelectedBeneficiary(
	...
	const exists = beneficiaries.some(
		(beneficiary) => beneficiary.id === selectedId,
	);
	return exists ? selectedId : beneficiaries[0].id;
}

export function buildPaymentInlineSuggestions(
	beneficiary: PaymentBeneficiary,
): string[] {
