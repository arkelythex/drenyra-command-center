    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="truncate text-2xs text-muted-foreground">{beneficiary.bankAccount}</p>
        </div>

        <div className="flex items-center gap-2">
          <span className="font-black">{formatMoney(beneficiary.amount, currency)}</span>
          <button
            type="button"
            onClick={(event) => {
      ...
      {draft ? (
        <div className="mt-2 rounded-lg border border-primary/20 bg-primary/10 px-2 py-1.5">
          ...
          <p className="text-3xs uppercase tracking-wider text-primary/70">Riesgo: {draft.patch.riskLevel}</p>
        </div>
      ) : null}
