...
export function usePaymentPreviewKeyboardShortcuts({
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (!isEditableTarget && (event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 'k') {
        event.preventDefault();
        onToggleInlineEditor(selectedBeneficiaryId);
        return;
      }

      if (!isEditableTarget && (event.metaKey || event.ctrlKey) && event.key === 'Enter' && editingBeneficiaryId === selectedBeneficiaryId) {
        event.preventDefault();
      ...
      if (!isEditableTarget && event.key === 'Escape' && editingBeneficiaryId === selectedBeneficiaryId) {
        event.preventDefault();
        onCloseInlineEditor();
      }
    };
