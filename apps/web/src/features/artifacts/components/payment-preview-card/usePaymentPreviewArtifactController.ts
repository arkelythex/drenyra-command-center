...
export function usePaymentPreviewArtifactController({ artifact, onEvent }: UsePaymentPreviewArtifactControllerInput) {
  const { requestApproval } = usePolicyGate();
  const confirmAction = artifact.actions.find((action) => action.id === 'confirm-payment');

  const [beneficiaries, setBeneficiaries] = useState<PaymentBeneficiary[]>(artifact.data.beneficiaries);
  const [selectedBeneficiaryId, setSelectedBeneficiaryId] = useState<string | null>(artifact.data.beneficiaries[0]?.id ?? null);
  const [editingBeneficiaryId, setEditingBeneficiaryId] = useState<string | null>(null);
  const [promptsByBeneficiary, setPromptsByBeneficiary] = useState<Record<string, string>>({});
  const [draftsByBeneficiary, setDraftsByBeneficiary] = useState<Record<string, PaymentBeneficiaryDraft>>({});

  const totalAmount = useMemo(() => computeTotalAmount(beneficiaries), [beneficiaries]);
  const tableRows = useMemo(
    () => buildPaymentTableRows(beneficiaries, artifact.data.currency, artifact.data.provider),
    [artifact.data.currency, artifact.data.provider, beneficiaries],
  );
  const beneficiariesById = useMemo(
    () => new Map(beneficiaries.map((beneficiary) => [beneficiary.id, beneficiary])),
    [beneficiaries],
  );

  useEffect(() => {
    const ensured = ensureSelectedBeneficiary(beneficiaries, selectedBeneficiaryId);
  ...
  const moveSelection = (direction: 'up' | 'down') => {
    const nextId = getNextSelectionId(beneficiaries, selectedBeneficiaryId, direction);
    setSelectedBeneficiaryId(nextId);
  };

  const toggleInlineEditor = (beneficiaryId: string) => {
    setSelectedBeneficiaryId(beneficiaryId);
    setEditingBeneficiaryId((current) => (current === beneficiaryId ? null : beneficiaryId));
  };
  ...
  const applyInlineEditById = async (beneficiaryId: string) => {
    ...
    if (risk.requiresPolicyGate) {
      ...
      if (!policyDecision.allowed || !policyDecision.proof) {
        toast.error(policyDecision.reason ?? 'Policy gate rechazo editar el monto.');
        onEvent(
          emitPaymentEvent(
            artifact,
            'policy-gate-denied',
            policyDecision.reason ?? `Beneficiario ${beneficiary.name}: edicion bloqueada por policy gate.`,
          ),
        );
        return;
      }
    ...
    setBeneficiaries((current) =>
      current.map((item) => (item.id === beneficiaryId ? { ...item, amount: draft.nextAmount } : item)),
    );
