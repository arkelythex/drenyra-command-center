...
const FILTERS: readonly SireStatusFilter[] = [
	"ALL",
	"MISMATCH",
	"MISSING_LOCAL",
	"MISSING_SUNAT",
...
export function SireDiffToolbar({
	...
	return (
		<div className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-muted/40 px-3 py-2">
			...
			<ActionButton
				icon={<Copy size={11} className="mr-1" />}
				label="Copiar Tabla"
				onClick={onCopyTable}
			/>
			<ActionButton
				icon={<FileSpreadsheet size={11} className="mr-1" />}
				label="Exportar Excel"
				onClick={onExportExcel}
			/>
			<span className="text-3xs font-black uppercase tracking-wider text-primary/70">
				↑↓ fila | Cmd/Ctrl+K edit | Cmd/Ctrl+Enter aplicar
			</span>
		</div>
	);
}
