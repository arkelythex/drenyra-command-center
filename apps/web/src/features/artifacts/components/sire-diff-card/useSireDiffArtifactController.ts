...
import { commitSireDiffResolutions } from "@/features/sire/api/sire-diff-commit.api";
import {
	buildTsvText,
	copyTextToClipboard,
	downloadCsvFile,
...
export function useSireDiffArtifactController({
	artifact,
	onEvent,
}: UseSireDiffArtifactControllerInput) {
	const { requestApproval } = usePolicyGate();
	const { companyContext } = useActiveCompanyContext();
	const companyId = companyContext.companyId ?? "";
	const acceptBatchAction = artifact.actions.find(
		(action) => action.id === "accept-sunat-batch",
	);

	const [rows, setRows] = useState<SireDiffRow[]>(artifact.data.rows);
	const [decisions, setDecisions] = useState<Record<string, RowDecision>>(() =>
		createInitialDecisions(artifact.data.rows),
	);
	...
	const submitGate = useMemo(
		() =>
			resolveSubmitGate({
				...
				pendingDecisions: totals.pending,
			}),
		[artifact.data, totals.pending],
	);
	...
	const { selectedRowId, setSelectedRowId, moveSelection } =
		useSireRowSelection({
			visibleRows,
	...
	const applyBatch = async (decision: Exclude<RowDecision, "PENDING">) => {
		if (decision === "ACCEPT_SUNAT" && submitGate.submitBlocked) {
			toast.error(
				submitGate.submitBlockReason ?? "SUNAT batch action blocked.",
			);
			return;
		}
		...
		if (decision === "ACCEPT_SUNAT") {
			...
			if (!policyDecision.allowed || !policyDecision.proof) {
				toast.error(
					policyDecision.reason ?? "Policy gate rechazo la accion por lote.",
				);
				onEvent(
					createArtifactEvent(
						artifact,
		...
		const nextDecisions = applyDecisionInBatch(rows, decision);
		...
		try {
			const result = await commitSireDiffResolutions({
				companyId,
				period: artifact.data.period,
				artifactId: artifact.id,
				traceId: artifact.metadata.traceId,
				sunatSource: artifact.data.sunatSource ?? "unavailable",
				summary,
				rows,
			...
			onEvent(
				createArtifactEvent(
					artifact,
					"sire-diff-commit",
					`Resoluciones persistidas (${result.eventId}).`,
					"COMMITTED",
				),
			);
			if (!result.submitBlocked) {
				toast.success("SIRE diff resolutions committed — submit gate cleared.");
			}
		} catch (error) {
			toast.error(
				error instanceof Error
					? error.message
					: "Failed to persist SIRE diff commit",
			);
		}
	};
	...
	const suggestInlineEdit = (row: SireDiffRow) => {
		...
		if (!draft) {
			onEvent(
				createArtifactEvent(
					artifact,
					"inline-ai-edit-invalid",
					`Fila ${row.id}: no se pudo interpretar la instruccion.`,
				),
			);
			return;
		}
		...
		onEvent({
			...createArtifactEvent(
				...
				`Fila ${row.id}: sugerencia IA generada.`,
			),
	...
	const applyInlineEdit = async (row: SireDiffRow) => {
		...
		const nextRow = buildRowFromDraft(row, artifact, draft);
		const patches =
			draft.patches ?? buildSireInlinePatches(row, nextRow, draft.note);
		const hasHighRiskPatch = patches.some(
			(patch) => patch.riskLevel === "HIGH" || patch.riskLevel === "CRITICAL",
		);
		...
		if (hasHighRiskPatch) {
			...
			if (!policyDecision.allowed) {
				toast.error(
					policyDecision.reason ?? "Policy gate rechazo el patch inline.",
				);
				onEvent(
					createArtifactEvent(
						artifact,
						...
						policyDecision.reason ??
							`Fila ${row.id}: patch inline bloqueado por policy gate.`,
					),
				);
				return;
			}
		...
		setRows((prev) =>
			prev.map((candidate) => (candidate.id === row.id ? nextRow : candidate)),
		);
		setDecisions((prev) => ({
			...prev,
			[row.id]:
				nextRow.status === "MATCH" ? "KEEP_LOCAL" : (prev[row.id] ?? "PENDING"),
		}));
		...
		onEvent({
			...createArtifactEvent(
				artifact,
				"inline-ai-edit-apply",
				`Fila ${row.id}: se aplico ajuste sugerido por IA.`,
			),
			payload: { patches, patchCount: patches.length },
		});
	};
		...
		try {
			...
			onEvent(
				createArtifactEvent(
					artifact,
					"copy-table",
					"Se copio tabla SIRE al portapapeles.",
				),
			);
		} catch {
	...
	const handleExportExcel = () => {
		downloadCsvFile(
			`sire-diff-${artifact.data.period}.csv`,
			[...SIRE_EXPORT_HEADERS],
			exportRows,
		);
		...
		onEvent(
			createArtifactEvent(
				artifact,
				"export-excel",
				"Se exporto conciliacion SIRE a CSV.",
			),
		);
	};
