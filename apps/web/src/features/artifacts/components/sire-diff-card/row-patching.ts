...
export function buildRowFromDraft(
	row: SireDiffRow,
	...
	const difference = Number(
		((nextLocalRecord?.total ?? 0) - (nextSunatRecord?.total ?? 0)).toFixed(2),
	);
...
function updateLocalRecord(
	...
	return {
		...(row.localRecord ??
			row.sunatRecord ??
			buildFallbackRecord(row.id, artifact)),
		total: draft.localTotal,
		currency: artifact.data.currency,
	};
}

function updateSunatRecord(
	row: SireDiffRow,
	artifact: SireDiffArtifact,
	draft: RowDraft,
): SireDocumentRecord | undefined {
