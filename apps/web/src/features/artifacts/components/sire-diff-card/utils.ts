...
export function statusClass(status: SireDiffRow["status"]): string {
	if (status === "MATCH")
		return "border-[rgba(var(--premium-success-rgb),0.30)] bg-[rgba(var(--premium-success-rgb),0.10)] text-[var(--premium-success)]";
	if (status === "MISMATCH")
...
export function buildSummary(rows: SireDiffRow[]): SireSummaryView {
	...
	const totalDifference = Number(
		rows.reduce((acc, row) => acc + row.difference, 0).toFixed(2),
	);
...
export function createInitialDecisions(
	...
	return Object.fromEntries(
		rows.map((row) => [
			row.id,
			row.status === "MATCH" ? "KEEP_LOCAL" : "PENDING",
		]),
	);
}
...
export function buildSireExportRows(
	rows: SireDiffRow[],
...
export function getNextSelectedRowId(
	...
	const nextIndex =
		(safeIndex + delta + visibleRows.length) % visibleRows.length;
...
export function buildSireInlineSuggestions(row: SireDiffRow): string[] {
	...
	if (
		sunatTotal !== undefined &&
		localTotal !== undefined &&
		Math.abs(sunatTotal - localTotal) > 0.01
	) {
