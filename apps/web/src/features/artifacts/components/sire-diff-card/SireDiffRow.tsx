import { Check, PencilLine, RotateCcw, Sparkles } from 'lucide-react';
...
import { buildSireInlineSuggestions, formatCurrency, statusClass, statusLabel } from './utils';
...
export function SireDiffRowView({
  ...
  const rowTone =
    row.status === 'MATCH' ? 'bg-[rgba(var(--premium-success-rgb),0.05)]' : row.status === 'MISMATCH' ? 'bg-amber-500/5' : 'bg-red-500/5';
  const rowSelectionTone = isSelected ? 'ring-1 ring-inset ring-primary/35' : '';
  const inlineSuggestions = buildSireInlineSuggestions(row);
  const { ghostSuggestion, ghostCompletion, acceptGhostSuggestion } = useInlineGhostSuggestion({
    value: prompt,
    suggestions: inlineSuggestions,
  });

  return (
    <tr className={`align-top ${rowTone} ${rowSelectionTone}`} onClick={onSelectRow}>
      ...
      <td className="px-3 py-3">
        <SireDiffRecordCard record={row.localRecord} currency={currency} draftTotal={draft?.localTotal} />
      </td>
      <td className="px-3 py-3">
        <SireDiffRecordCard record={row.sunatRecord} currency={currency} draftTotal={draft?.sunatTotal} />
      </td>

      <td className={row.difference === 0 ? 'px-3 py-3 text-muted-foreground' : 'px-3 py-3 font-black text-amber-200'}>
        {formatCurrency(row.difference, currency)}
      </td>

      <td className="px-3 py-3 text-muted-foreground">
        <p>{row.reason}</p>
        {draft ? <p className="mt-1 text-2xs font-semibold text-primary">{draft.note}</p> : null}
        {draft?.patches?.length ? (
          <p className="mt-1 text-2xs uppercase tracking-wide text-primary/80">
            Diff IA: {draft.patches.length} patch{draft.patches.length === 1 ? '' : 'es'}
          </p>
        ) : null}
      </td>
              ...
              <div className="relative">
                {ghostSuggestion ? (
                  <div className="pointer-events-none absolute inset-0 flex h-8 items-center px-2 text-2xs uppercase tracking-wider">
                    ...
                    <span className="text-muted-foreground/35">{ghostCompletion}</span>
                  </div>
                ) : null}
                <input
                  type="text"
