...
export function useSireDiffKeyboardShortcuts({
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (!isEditableTarget && (event.metaKey || event.ctrlKey) && event.key === 'Enter' && editingRowId === selectedRowId) {
        event.preventDefault();
      ...
      if (!isEditableTarget && event.key === 'Escape' && editingRowId === selectedRowId) {
        event.preventDefault();
        onCloseInlineEditor();
      }
    };
