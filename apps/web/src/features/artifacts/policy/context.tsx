...
export function PolicyGateProvider({ children }: { children: React.ReactNode }) {
  ...
  const requestApproval = useCallback((request: PolicyGateRequest): Promise<PolicyGateResult> => {
  ...
  const closeAsCancelled = useCallback(() => {
    if (!pending) return;
    pending.resolve({ allowed: false, reason: 'Solicitud cancelada por usuario.' });
    setPending(null);
    setError(null);
  }, [pending]);
  ...
  const handleApprove = useCallback(() => {
    ...
    if (gate?.requiresDualApproval) {
      ...
      if (normalizedApprover.toLowerCase() === normalizedRequester.toLowerCase()) {
  ...
  const value = useMemo<PolicyGateContextValue>(() => ({ requestApproval }), [requestApproval]);

  return (
    <PolicyGateContext.Provider value={value}>
      {children}

      <Dialog open={Boolean(pending)} onOpenChange={(open) => !open && closeAsCancelled()}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-base font-black uppercase tracking-widest">Policy Gate</DialogTitle>
            <DialogDescription>
              Confirma accion critica con trazabilidad completa (doble firma cuando aplique).
            </DialogDescription>
          </DialogHeader>
          ...
          {pending ? (
            <div className="space-y-3">
              <div className="rounded-xl border border-border bg-muted/40 px-3 py-2 text-label uppercase tracking-wider text-muted-foreground">
                Accion: <span className="font-mono text-foreground">{pending.request.actionLabel}</span>
              </div>
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                <MetaPill label="Riesgo" value={pending.request.riskLevel} />
                <MetaPill label="Trace" value={pending.request.traceId} mono />
                <MetaPill label="Policy" value={pending.request.policyGate?.policyKey ?? 'N/A'} mono />
                <MetaPill
                  label="Control"
                  value={pending.request.policyGate?.requiresDualApproval ? 'DOBLE_FIRMA' : 'FIRMA_UNICA'}
                  mono
                />
              </div>

              <div className="space-y-1">
                <label className="text-label font-semibold uppercase tracking-wider text-muted-foreground">Solicitante</label>
                <Input
                  value={requester}
              ...
              <div className="space-y-1">
                <label className="text-label font-semibold uppercase tracking-wider text-muted-foreground">
                  Motivo {pending.request.policyGate?.requiresReason ? '(obligatorio)' : '(opcional)'}
                </label>
                <Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Justificacion operativa/compliance" />
              </div>
              ...
              {error ? (
                <div className="rounded-lg border border-destructive/30 bg-destructive/10 px-2 py-1.5 text-xs text-destructive">{error}</div>
              ) : null}
            </div>
          ) : null}

          <DialogFooter>
            <Button variant="outline" onClick={closeAsCancelled}>Cancelar</Button>
...
function MetaPill({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="rounded-xl border border-border bg-muted/40 px-3 py-2 text-label uppercase tracking-wider text-muted-foreground">
      {label}: <span className={mono ? 'font-mono text-foreground' : 'text-foreground'}>{value}</span>
    </div>
  );
}
