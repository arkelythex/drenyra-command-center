...
export function SessionExpiryNotification() {
  ...
  const handleExtendSession = async () => {
    ...
    let extensionTimestamps: number[] = storedTimestamps ? JSON.parse(storedTimestamps) : [];
    ...
    if (extensionTimestamps.length >= MAX_EXTENSIONS_PER_HOUR) {
      ...
      const timeUntilNextAllowed = RATE_LIMIT_WINDOW_MS - (now - oldestExtension);
    ...
    try {
      ...
      localStorage.setItem(RATE_LIMIT_STORAGE_KEY, JSON.stringify(extensionTimestamps));

      const remainingExtensions = MAX_EXTENSIONS_PER_HOUR - extensionTimestamps.length;
      ...
      captureError(error instanceof Error ? error : new Error('Session extension failed'), {
      ...
      toast.error('Error al extender sesión', {
        description: 'No pudimos extender tu sesión. Por favor inicia sesión nuevamente.',
      });
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
        <div className="w-full max-w-md rounded-2xl border border-border bg-card/95 p-6 shadow-xl backdrop-blur-md">
          {/* Icon */}
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-muted-foreground/80">
              ¿Deseas extender tu sesión? Si no actúas, serás desconectado automáticamente.
            </p>
          </div>
