...
describe("LoginForm", () => {
	...
	it("should validate email format", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText(/correo corporativo válido/i),
			).toBeInTheDocument();
		});
	});
	...
	it("should toggle password visibility", async () => {
		...
		if (
			toggleButton &&
			toggleButton !== screen.getByRole("button", { name: /autenticar/i })
		) {
...
describe("resolveSafeLoginRedirect", () => {
	it("only accepts same-app absolute paths", () => {
		...
		expect(
			resolveSafeLoginRedirect("?redirect=/safe?next=https://evil.com"),
		).toBe("/");
	});
});
