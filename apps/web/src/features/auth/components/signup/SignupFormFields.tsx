...
import { MotionDiv, entranceVariants } from '@/components/ui/motion-primitives';
import { Text } from '@/components/atoms/text';
import type { SignupFormData, PasswordStrength } from './signup-form.validation';
...
export const SignupFormFields = ({
  ...
  <>
    <MotionDiv variants={entranceVariants} className="space-y-3">
      <Text variant="label" className="ml-1 text-sm font-medium text-foreground">
        Nombre Completo
      </Text>
      ...
      {errors.name ? <Text variant="label" id="name-error" className="ml-1 text-xs text-red-400">{errors.name.message}</Text> : null}
    </MotionDiv>
    ...
    <MotionDiv variants={entranceVariants} className="space-y-3">
      ...
      {errors.email ? <Text variant="label" id="email-error" className="ml-1 text-xs text-red-400">{errors.email.message}</Text> : null}
    </MotionDiv>
    ...
    <MotionDiv variants={entranceVariants} className="space-y-3">
      ...
      <div className="group relative">
        ...
        <div className="absolute right-4 top-1/2 -translate-y-1/2">
          {rucValidation.status === 'loading' ? <Loader2 className="h-5 w-5 animate-spin text-primary" /> : null}
          {rucValidation.status === 'valid' ? <Check className="h-5 w-5 text-[var(--premium-success)]" /> : null}
          {rucValidation.status === 'invalid' ? <X className="h-5 w-5 text-red-500" /> : null}
        </div>
      </div>
      ...
      {rucValidation.status === 'valid' && rucValidation.companyName ? (
        <div className="ml-1 flex items-center gap-2">
          ...
          <Text variant="label" className="text-xs text-[var(--premium-success)]">{rucValidation.companyName}</Text>
        </div>
      ) : null}
      ...
      {rucValidation.status === 'invalid' && rucValidation.error ? (
        <div className="ml-1 flex items-center gap-2">
          ...
          <Text variant="label" className="text-xs text-red-400">{rucValidation.error}</Text>
        </div>
      ) : null}

      {errors.ruc ? <Text variant="label" id="ruc-error" className="ml-1 text-xs text-red-400">{errors.ruc.message}</Text> : null}
      <Text variant="label" className="ml-1 text-xs text-muted-foreground/80">RUC de 11 dígitos de su empresa (SUNAT 2026)</Text>
    </MotionDiv>
    ...
    <MotionDiv variants={entranceVariants} className="space-y-3">
            ...
            {[...Array(5)].map((_, index) => (
              <div
                ...
                className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                  index < passwordStrength.score ? passwordStrength.color : 'bg-border'
                }`}
              />
            ))}
          </div>
      ...
      {errors.password ? <Text variant="label" id="password-error" className="ml-1 text-xs text-red-400">{errors.password.message}</Text> : null}
    </MotionDiv>
    ...
    <MotionDiv variants={entranceVariants} className="space-y-3">
      ...
      {errors.confirmPassword ? (
        <Text variant="label" id="confirmPassword-error" className="ml-1 text-xs text-red-400">{errors.confirmPassword.message}</Text>
      ) : null}
    </MotionDiv>

    <MotionDiv variants={entranceVariants} className="flex items-start space-x-3 px-1">
      <Checkbox
        id="acceptTerms"
        checked={acceptTerms}
      ...
      />
      <label htmlFor="acceptTerms" className="cursor-pointer select-none text-sm leading-relaxed text-muted-foreground">
        Acepto los <a href="/terms" className="text-primary hover:underline">Términos y Condiciones</a> y la{' '}
        <a href="/privacy" className="text-primary hover:underline">Política de Privacidad</a> (Requerido por SUNAT 2026)
      </label>
    </MotionDiv>

    {errors.acceptTerms ? (
      <Text variant="label" className="-mt-2 ml-1 text-xs text-red-400">{errors.acceptTerms.message}</Text>
    ) : null}
  </>
);
