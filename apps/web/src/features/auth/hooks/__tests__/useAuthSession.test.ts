...
describe("useAuthSession", () => {
	...
	it("should expose synthetic demo access when no Better Auth session exists", async () => {
		...
		await waitFor(() => {
			expect(result.current.isAuthenticated).toBe(true);
			expect(result.current.user?.companyName).toBe(
				"NEBULA OPERACIONES LOGISTICAS S.A.C.",
			);
	...
	it("should show loading state during session check", () => {
		let resolveSnapshot: (
			value: { user: User; session: Session } | null,
		) => void = () => {};
	...
	it("should clear Zustand store when session becomes null", async () => {
		...
		const firstRender = renderHook(() => useAuthSession(), {
			wrapper: createWrapper(),
		});
