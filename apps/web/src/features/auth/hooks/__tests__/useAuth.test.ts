...
function mockLoginApiResponse(status: number, payload?: unknown): void {
  vi.mocked(api.api.auth.login.post).mockResolvedValueOnce({
    ...
    error: status >= 200 && status < 300 ? null : (payload ?? { error: "Request failed" }),
    status,
    headers: new Headers(),
  } as never);
}
  ...
  describe('login', () => {
    ...
    it('should set loading state during login', async () => {
      ...
      const loginPromise = new Promise((resolve) => {
        resolveLogin = () => resolve({ data: {}, error: null, status: 200, headers: new Headers() });
      });
      vi.mocked(api.api.auth.login.post).mockReturnValueOnce(loginPromise as never);
      vi.mocked(readAuthSessionSnapshot).mockResolvedValueOnce({
        user: mockUser,
        session: mockSession,
      });
