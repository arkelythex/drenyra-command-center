...
import { RefreshCw, ArrowUpRight, ArrowDownLeft, ShieldCheck } from 'lucide-react';
...
export const CBDCWalletBalance: React.FC<CBDCWalletBalanceProps> = ({
  ...
  return (
    <Card className={cn("overflow-hidden border border-border/60 bg-[var(--surface-1)]/92 shadow-[var(--shadow-sm)]", className)}>
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center gap-2">
            <span className={cn(
              ...
              syncStatus === 'synced' ? "bg-success shadow-[0_0_8px_rgba(var(--premium-success-rgb),0.32)]" :
                syncStatus === 'syncing' ? "bg-warning animate-pulse" : "bg-danger"
            )} />
            {syncStatus === 'syncing' && (
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="text-2xs text-muted-foreground text-center pt-2 opacity-50">
            {lastSync ? `Sincronizado: ${lastSync.toLocaleTimeString()}` : 'Iniciando conexión...'}
          </div>
        </div>
      </CardContent>
    </Card>
