...
export async function parseTransactionsCsv(file: File): Promise<ImportTransactionRow[]> {
  ...
  const header = parseCsvLine(lines[0], delimiter).map((item) => item.toLowerCase());

  const hasHeader = header.some((item) =>
    ['date', 'fecha', 'description', 'descripcion', 'concepto', 'amount', 'monto', 'importe'].includes(item),
  );
  const startIndex = hasHeader ? 1 : 0;

  const dateIdx = hasHeader ? findColumnIndex(header, ['date', 'fecha']) : 0;
  const descIdx = hasHeader ? findColumnIndex(header, ['description', 'descripcion', 'concepto', 'detalle']) : 1;
  const amountIdx = hasHeader ? findColumnIndex(header, ['amount', 'monto', 'importe', 'valor']) : 2;
  const typeIdx = hasHeader ? findColumnIndex(header, ['type', 'tipo', 'dc', 'debitcredit']) : 3;
  const refIdx = hasHeader ? findColumnIndex(header, ['reference', 'referencia', 'ref', 'nro', 'numero']) : 4;
