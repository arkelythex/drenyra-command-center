...
const debitNoteSchema = z.object({
	referenceInvoiceId: z
		.string()
		.min(1, "La factura de referencia es requerida"),
	reason: z.string().min(1, "El motivo es requerido"),
	series: z.string().regex(/^[FB]D\d{2}$/, "La serie debe ser FD01 o BD01"),
...
export function CreateDebitNoteDialog({
	...
	const updateCalculatedTotal = (base: string, igv: string) => {
		...
		setCalculatedTotal(
			currency === "PEN" ? formatPEN(total) : `S/ ${total.toFixed(2)}`,
		);
	};
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[500px]">
				<DialogHeader>
					...
					<DialogDescription>
						Crea una nota de débito SUNAT tipo 08 para incrementar el monto de
						una factura
					</DialogDescription>
				</DialogHeader>
