	...
	return (
		<tr className="border-b border-border transition-colors hover:bg-muted/40">
			...
			<td className="px-4 py-3">
				<StatusBadge
					status={status}
					label={statusLabel[debitNote.status]}
					size="sm"
				/>
			</td>
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end" className="min-w-[160px]">
						...
						{debitNote.status === "DRAFT" && (
							<DropdownMenuItem
								onClick={() => onUpdateStatus?.(debitNote.id, "SENT")}
							>
						...
						{debitNote.status === "SENT" && (
							<>
								<DropdownMenuSeparator />
								<DropdownMenuItem
									onClick={() => onUpdateStatus?.(debitNote.id, "ACCEPTED")}
								>
									<CheckCircle className="mr-2 h-4 w-4 text-success" /> Aceptar
								</DropdownMenuItem>
								<DropdownMenuItem
									onClick={() => onUpdateStatus?.(debitNote.id, "REJECTED")}
								>
