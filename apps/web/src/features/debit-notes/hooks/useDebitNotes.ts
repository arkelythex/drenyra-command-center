...
export const dnHooks = createCrudHooks<DebitNoteRecord, CreateDebitNotePayload, { status: string }>({
  key: "debit-notes",
  list: (companyId) => debitNotesApi.list({ companyId }),
  getById: (id) => debitNotesApi.getById(id) as Promise<DebitNoteRecord>,
  create: (companyId, data) => debitNotesApi.create({ ...data, companyId }) as Promise<DebitNoteRecord>,
  update: (id, data) => debitNotesApi.updateStatus(id, data.status) as Promise<DebitNoteRecord>,
  delete: async (id) => { await debitNotesApi.delete(id); },
});
...
function normalizeStats(debitNotes: DebitNoteRecord[]): DebitNoteStats {
  ...
  const acceptedCount = debitNotes.filter((dn) => dn.status === 'ACCEPTED').length;
  const rejectedCount = debitNotes.filter((dn) => dn.status === 'REJECTED').length;
  const totalAmount = debitNotes.reduce((sum, dn) => sum + (Number.parseFloat(dn.totalAmount) || 0), 0);

  return {
    total, draftCount, sentCount, acceptedCount, rejectedCount, totalAmount,
  };
}
