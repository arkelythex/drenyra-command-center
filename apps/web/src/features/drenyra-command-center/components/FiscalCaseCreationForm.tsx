...
const CASE_TYPE_OPTIONS: Array<{
...
}> = [
	{
		...
		description: "Cierre mensual con evidencia y aprobaciones.",
	},
	{
		value: "CPE_REVIEW",
		label: "Revisión CPE",
		description: "Revisión de comprobantes, CDR y UBL.",
	},
	{
		value: "SIRE_REVIEW",
		label: "Revisión SIRE",
		description: "Validación de propuesta SIRE y periodo tributario.",
	},
	{
		value: "LEDGER_REVIEW",
		label: "Revisión contable",
		description: "Revisión de libro mayor y asientos contables.",
	},
	{
		value: "CONCILIATION",
		label: "Conciliación",
		description: "Cruce contable-bancario sin ejecución automática.",
	},
	{
		value: "EVIDENCE_REVIEW",
		label: "Revisión de evidencia",
		description: "Completar sustento fiscal antes de decidir.",
	},
];
...
function validateForm(state: FormState): string | null {
	...
	if (state.description.trim().length < 10)
		return "La descripción debe explicar el objetivo fiscal del caso.";
	if (!Number.isInteger(riskScore) || riskScore < 0 || riskScore > 100)
		return "El score de riesgo debe ser un entero entre 0 y 100.";
	return null;
}

export function FiscalCaseCreationForm({
	onSubmit,
	isPending,
	errorMessage,
}: FiscalCaseCreationFormProps) {
	const titleId = useId();
	const descriptionId = useId();
	const errorId = useId();
	const [state, setState] = useState<FormState>(initialFormState);
	const [localError, setLocalError] = useState<string | null>(null);
	const selectedType = CASE_TYPE_OPTIONS.find(
	...
	return (
		<form
			onSubmit={submit}
			className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-2)]/70 p-4 shadow-2xl shadow-black/10"
			aria-label="Create fiscal case"
		>
			<div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
				<div>
					...
					<h3 className="mt-1 text-base font-bold">
						Abrí un workspace con evidencia y aprobación
					</h3>
					<p className="mt-1 text-xs text-[var(--text-tertiary)]">
						El caso no ejecuta acciones fiscales reales; prepara trazabilidad
			...
			<div className="mt-4 grid gap-3 xl:grid-cols-[1.1fr_1fr]">
				<label className="space-y-1 text-xs font-semibold" htmlFor={titleId}>
					...
					<input
						...
						placeholder="Ej. Cierre fiscal junio 2026"
						className="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 py-2 text-sm font-normal outline-none transition focus:border-[var(--color-info)]"
					/>
				</label>
				<label className="space-y-1 text-xs font-semibold">
					...
					<select
						value={state.type}
						onChange={(event) =>
							update("type", event.target.value as FiscalCaseType)
						}
						className="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 py-2 text-sm font-normal outline-none transition focus:border-[var(--color-info)]"
					>
				...
				<label
					className="space-y-1 text-xs font-semibold xl:col-span-2"
				...
				>
					...
					<textarea
						...
						placeholder="Qué se revisa, qué fuente fiscal se usará y por qué requiere trazabilidad."
						rows={3}
						className="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 py-2 text-sm font-normal outline-none transition focus:border-[var(--color-info)]"
					/>
				</label>
			...
			<div className="mt-3 grid gap-3 md:grid-cols-3">
				<label className="space-y-1 text-xs font-semibold">
					...
					<select
						...
						onChange={(event) =>
							update("riskLevel", event.target.value as FiscalRiskLevel)
						}
						className="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 py-2 text-sm font-normal outline-none transition focus:border-[var(--color-info)]"
					>
				...
				<label className="space-y-1 text-xs font-semibold">
					...
					<input
						...
						onChange={(event) => update("riskScore", event.target.value)}
						inputMode="numeric"
						className="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 py-2 text-sm font-normal outline-none transition focus:border-[var(--color-info)]"
					/>
				</label>
				<label className="space-y-1 text-xs font-semibold">
					...
					<select
						...
						onChange={(event) =>
							update("autonomyLevel", event.target.value as AutonomyLevel)
						}
						className="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 py-2 text-sm font-normal outline-none transition focus:border-[var(--color-info)]"
					>
