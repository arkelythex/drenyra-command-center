...
import type { DrenyraAgentType } from "@arkelythex/domain/drenyra";
...
import { SettingsPanel, DEFAULT_SETTINGS, type CommandCenterSettings } from "./SettingsPanel";
...
import { DrenyraCommandPalette, type PaletteCmd } from "./DrenyraCommandPalette";
...
export function DrenyraCommandCenter() {
	...
	const [selectedAgent, setSelectedAgent] = useState<DrenyraAgentType>("SIRE_AGENT");
	...
	const handleResultClick = useCallback((result: SearchResult) => {
		...
		window.dispatchEvent(
			new CustomEvent("drenyra:scroll-to-message", { detail: result.messageId }),
		);
	}, []);
	...
	const handleRunAgent = useCallback(() => startRun.mutate(selectedAgent), [startRun, selectedAgent]);
	...
	const paletteCommands: PaletteCmd[] = [
		{
			...
			shortcut: "⌘N",
			action: () =>
				document.querySelector<HTMLElement>('[data-action="new-case"]')?.click(),
		},
		{
			label: "Correr Agente",
			icon: Sparkles,
		...
		},
		{
			...
			shortcut: "⌘U",
			action: () =>
				document.querySelector<HTMLElement>('[data-action="upload-evidence"]')?.click(),
		},
		{ separator: true },
		{
			label: "Modo Compacto",
			...
			shortcut: "⌘1",
			action: () =>
				window.dispatchEvent(new CustomEvent("drenyra:density-change", { detail: "compact" })),
		},
		{
			label: "Modo Detalle",
			icon: LayoutDashboard,
			shortcut: "⌘2",
			action: () =>
				window.dispatchEvent(new CustomEvent("drenyra:density-change", { detail: "detail" })),
		},
		{
			label: "Solo Números",
			icon: LayoutDashboard,
			shortcut: "⌘3",
			action: () =>
				window.dispatchEvent(new CustomEvent("drenyra:density-change", { detail: "numbers-only" })),
		},
		{ separator: true },
		{
			label: "Clear Chat",
	...
	// ── Panel sections ──────────────────────────────────────────────────
	const sidebarContent = !casesQuery.isFetched && casesQuery.isLoading ? (
		...
		<div className="grid min-h-[calc(100vh-4rem)] grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)_360px]">
			...
			<main data-onboarding="chat" className="min-w-0 p-4 lg:p-6">
				...
				<CommandCenterChat
					...
					isBusy={isBusy}
					onCreateCase={(request: CreateFiscalCaseRequest) => createCase.mutate(request)}
			...
			{mobileSidebarOpen && (
				<div className="fixed inset-0 z-40 lg:hidden">
					<div className="fixed inset-0 bg-black/50" onClick={() => setMobileSidebarOpen(false)} role="presentation" tabIndex={-1} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setMobileSidebarOpen(false); } }} />
					<aside
			...
			{mobileRightPanelOpen && (
				<div className="fixed inset-0 z-40 lg:hidden">
					<div className="fixed inset-0 bg-black/50" onClick={() => setMobileRightPanelOpen(false)} role="presentation" tabIndex={-1} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setMobileRightPanelOpen(false); } }} />
					<aside
			...
			{showSearch && (
				<div
					...
					role="presentation"
					tabIndex={-1}
					onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setShowSearch(false); } }}
				>
			...
			{showSettings && (
				<div className="fixed inset-0 z-50 flex justify-end">
					<div className="absolute inset-0 bg-black/40" onClick={() => setShowSettings(false)} role="presentation" tabIndex={-1} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setShowSettings(false); } }} />
