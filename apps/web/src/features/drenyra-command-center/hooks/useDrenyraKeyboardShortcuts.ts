...
export function useDrenyraKeyboardShortcuts({
  ...
  useEffect(() => {
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (event.key === "u" && (event.metaKey || event.ctrlKey) && !event.shiftKey) {
        event.preventDefault();
        const uploadBtn = document.querySelector('[data-action="upload-evidence"]');
      ...
      if ((event.metaKey || event.ctrlKey) && !event.shiftKey && ["1", "2", "3"].includes(event.key)) {
        event.preventDefault();
        const densityMap: Record<string, DensityMode> = { "1": "compact", "2": "detail", "3": "numbers-only" };
        window.dispatchEvent(new CustomEvent("drenyra:density-change", { detail: densityMap[event.key] }));
      }
    }

    document.addEventListener("keydown", handleKeyDown);
    return () => document.removeEventListener("keydown", handleKeyDown);
  }, [showPalette, showSearch, showSettings, showShortcuts, activeCaseId, startRun, setShowPalette, setShowSearch, setShowSettings, setShowShortcuts]);
}
