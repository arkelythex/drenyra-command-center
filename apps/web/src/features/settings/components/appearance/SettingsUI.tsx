...
export function SettingsButton({ ref, className, variant = 'secondary', size = 'sm', active, children, ...props }: SettingsButtonProps) {
	const variants = {
		primary: "bg-[var(--accent)] text-[var(--surface)] border-[var(--accent)]/20 hover:opacity-90 shadow-[0_0_20px_-5px_var(--accent)]",
		secondary: "bg-[var(--ink)]/[0.03] text-[var(--ink)]/70 border-[var(--border)] hover:border-[var(--accent)]/30 hover:bg-[var(--ink)]/[0.06] hover:text-[var(--ink)]",
		ghost: "bg-transparent text-[var(--ink)]/40 border-transparent hover:text-[var(--ink)]/80 hover:bg-[var(--ink)]/5",
		danger: "text-[var(--diff-removed)] border-[var(--diff-removed)]/20 bg-[var(--diff-removed)]/10 hover:bg-[var(--diff-removed)]/20",
	};
	...
	return (
		<button
			...
			className={cn(
				...
				variants[variant],
				sizes[size],
				active && "border-[var(--accent)]/40 bg-[var(--accent)]/[0.05] text-[var(--accent)]",
				className
			)}
			{...props}
		>
