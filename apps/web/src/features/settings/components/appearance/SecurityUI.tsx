import { Laptop, Lock, ShieldCheck, Zap } from "lucide-react";
import { cn } from "@/lib/utils";

export const SecurityHealth = ({ score = 85 }: { score?: number }) => {
	return (
		<div className="relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-1)] p-8 shadow-xl">
			{/* Background Glow */}
...
export const SessionCard = ({
	...
	return (
		...
		>
			<div className="flex items-center gap-4">
				<div
					className={cn(
						...
						isCurrent
							? "border-[var(--accent)]/20 bg-[var(--accent)]/10 text-[var(--accent)]"
							: "border-[var(--border)] bg-[var(--ink)]/5 text-[var(--ink)]/30",
					)}
				>
			...
			<button
				type="button"
				className="rounded-xl border border-[var(--border)] bg-[var(--ink)]/5 px-4 py-2 text-xs font-black uppercase tracking-widest text-[var(--ink)]/40 opacity-0 transition-all group-hover:opacity-100 hover:border-[var(--diff-removed)]/30 hover:bg-[var(--diff-removed)]/10 hover:text-[var(--diff-removed)]"
			>
				Terminate
			</button>
		</div>
