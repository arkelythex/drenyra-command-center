...
import { COMPLIANCE_ACTIONS, COMPLIANCE_ACTIVITY, TAB_CONFIG } from './compliance-view/data';
...
const RucRegistryTab = lazy(async () =>
  import('./tabs/RucRegistryTab').then((module) => ({ default: module.RucRegistryTab })),
);
...
const RiskMapTab = lazy(async () =>
  import('./tabs/RiskMapTab').then((module) => ({ default: module.RiskMapTab })),
);
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto flex w-full max-w-[1680px] flex-col gap-6 px-4 py-4 sm:px-5 sm:py-5 lg:px-6 lg:py-6 2xl:px-8">
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-end md:gap-5">
                    <div>
                      ...
                      <p className="mt-2 text-sm font-medium text-success">
                        RUC coverage {syncStats.rucCoverage} · CPE integrity {syncStats.cpeIntegrity}
                      </p>
                    </div>

                    <div className="grid gap-3 sm:grid-cols-3">
                      <MetricPill label="Alertas" value={String(syncStats.riskAlerts)} />
                      <MetricPill label="SIRE match" value={String(syncStats.sireMatches)} />
                      <MetricPill label="Detracciones" value={String(syncStats.pendingDetractions)} />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="min-w-0 space-y-2">
                  ...
                  <div className="space-y-1">
                    <h2 className="text-base font-semibold tracking-tight text-[var(--text-primary)] sm:text-lg">
                      Prioriza el padron RUC antes de aceptar propuestas regulatorias.
                    </h2>
                    <p className="max-w-3xl text-sm leading-6 text-[var(--text-secondary)]">
                      Hay un tercero no habido y comprobantes asociados con incidente. Resolver esa cadena reduce rechazo SUNAT y evita aprobar con soporte incompleto.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2 text-xs text-[var(--text-secondary)]">
                    <span className="chip px-2.5 py-1">Confidence 93%</span>
                    <span className="chip chip-warning px-2.5 py-1">Bloqueo con impacto alto</span>
                  </div>
                </div>
              </div>

              <div className="flex shrink-0 flex-wrap gap-3">
                <Button onClick={() => setActiveTab('ruc')}>Abrir padron RUC</Button>
          ...
          <section className="grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_360px]">
            <ComplianceActionCenter actions={COMPLIANCE_ACTIONS} onSelectTab={setActiveTab} />
            <ComplianceSidebar activity={COMPLIANCE_ACTIVITY} syncStats={syncStats} />
          </section>
            ...
            <CardHeader className="border-b border-[var(--border-subtle)] px-5 py-4 sm:px-6">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <CardDescription>
                    Cada modulo profundiza la decision seleccionada sin perder el contexto operativo.
                  </CardDescription>
                </div>
...
function MetricPill({ label, value }: { label: string; value: string }): JSX.Element {
  return (
    <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-2)]/80 px-4 py-3">
      <p className="text-label uppercase tracking-[0.16em] text-[var(--text-tertiary)]">{label}</p>
