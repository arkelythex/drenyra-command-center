...
export const CpeValidatorTab = () => {
	...
	const guidance = catalogQuery.data?.find(
		(entry) => entry.code === effectiveCode,
	);
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-2">
				<div>
					...
					<p className="text-label text-muted-foreground uppercase tracking-[0.2em] mt-1">
						Integridad XML/CDR • Protocolo NIIF
					</p>
				</div>
				<div className="flex flex-col sm:flex-row gap-3">
					<div className="grid grid-cols-3 gap-4 text-center">
						...
						<div className="rounded-lg border border-danger-subtle bg-danger-muted p-2">
							<p className="text-2xs font-black uppercase tracking-widest text-danger">
								RECHAZADOS
							</p>
					...
					<div className="flex gap-3">
						...
						<Button
							...
							onClick={() =>
								selectedRow && validationMutation.mutate(selectedRow)
							}
					...
					<CardContent className="p-0 overflow-x-auto">
						<table className="w-full text-left border-separate border-spacing-0">
							<thead className="bg-muted/30">
								<tr>
									<th className="px-10 py-4 text-xs font-black text-muted-foreground uppercase tracking-[0.2em] border-b border-border/50">
										Documento / Emisor
									</th>
									<th className="px-4 py-4 text-xs font-black text-muted-foreground uppercase tracking-[0.2em] border-b border-border/50 text-center">
										Estado SUNAT
									</th>
									<th className="px-4 py-4 text-xs font-black text-muted-foreground uppercase tracking-[0.2em] border-b border-border/50 text-center">
										CDR
									...
									<th className="px-10 py-4 text-xs font-black text-muted-foreground uppercase tracking-[0.2em] border-b border-border/50 text-right">
										Monto
									</th>
								</tr>
							</thead>
									...
									>
										<td className="px-10 py-6">
											<div className="flex items-center gap-5">
												<div className="rounded-xl border border-border bg-muted p-2.5 text-muted-foreground transition-[background-color,color,border-color] duration-200 group-hover:border-foreground group-hover:bg-foreground group-hover:text-background">
										...
										<td className="px-4 py-6 text-center">
											{cpe.detraction ? (
												<span className="text-xs font-black text-foreground bg-muted px-2 py-1 rounded border border-border uppercase tracking-tighter">
													Sujeto a 10%
												</span>
											) : (
