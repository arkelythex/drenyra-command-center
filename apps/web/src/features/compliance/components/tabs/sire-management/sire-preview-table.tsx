...
function SirePreviewTableMessage(props: {
	message: string;
	tone: "muted" | "danger";
}) {
	const toneClass =
		props.tone === "danger" ? "text-danger" : "text-muted-foreground";

	return (
		<tr>
			<td
				colSpan={5}
				className={`px-10 py-10 text-center text-sm font-semibold ${toneClass}`}
			>
				{props.message}
			</td>
		</tr>
...
export const SirePreviewTable = ({
	rows,
	isLoading,
	hasError,
}: SirePreviewTableProps) => {
	return (
