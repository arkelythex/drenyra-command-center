import type { SireSummary, SireSummaryCardData, SireTableRowData } from './sire-management.types';
...
export function buildSireSummaryCards(summary: SireSummary): SireSummaryCardData[] {
  return [
    {
      icon: 'sales',
      title: 'RVIE - Ventas',
    ...
    },
    {
      icon: 'differences',
      title: 'Diferencias',
      badge: summary.differenceCount === 0 ? 'Sin Alertas' : `${summary.differenceCount} Alertas`,
...
export function buildSireSummaryPlaceholderCards(mode: 'loading' | 'error'): SireSummaryCardData[] {
  const badge = mode === 'loading' ? 'Cargando' : 'Sin datos';
  const unit = mode === 'loading' ? 'Esperando resumen' : 'Resumen no disponible';
