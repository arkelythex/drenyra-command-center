...
const getExportFeedbackLabel = (
	ledgerType: "ventas" | "compras",
	format: "TXT" | "EXCEL",
) => {
	if (ledgerType === "ventas" && format === "TXT") return "RVIE Ventas TXT";
	if (ledgerType === "compras" && format === "EXCEL")
		return "RCE Compras Excel";
	return `${ledgerType.toUpperCase()} ${format}`;
};

const getSummaryStatusMessage = (state: {
	isLoading: boolean;
	isError: boolean;
}) => {
	if (state.isLoading) {
		return {
			...
			description:
				"Esperando confirmacion del resumen SIRE demo desde backend.",
			tone: "info" as const,
		};
	}
	...
	if (state.isError) {
		return {
			...
			description:
				"No se pudo obtener el resumen SIRE demo. Se muestra estado sin datos.",
...
export const SireManagementTab = () => {
	...
	const lastVerifiedAt = formatSireSummaryTimestamp(
		demoSummaryData?.generatedAt,
	);
					...
					<div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
						<div>
							...
							<p className="mt-3 pl-1 text-xs leading-relaxed text-muted-foreground">
								Export demo anclado al dataset seed de{" "}
								{SIRE_DEMO_EXPORT_PERIOD_LABEL} para validación del pitch.
							</p>
							...
							>
								<p className="text-label font-semibold tracking-[0.05em]">
									{summaryStatus.label}
								</p>
								...
								<p className="mt-1 text-label font-medium text-foreground/75">
									{lastVerifiedAt
										? `Ultima verificacion: ${lastVerifiedAt}`
										: "Ultima verificacion pendiente"}
								</p>
							</div>
						</div>
						...
						<div className="flex flex-col items-stretch gap-3 md:min-w-[360px] md:items-end">
							...
							<Button
								...
								variant="outline"
								size="sm"
								className="h-10 rounded-xl border-border/60 px-6 text-label font-medium tracking-[0.05em]"
							>
								<Link to="/cumplimiento/sire-diff">
									<ArrowRightLeft size={14} className="mr-2 opacity-70" />
									Run three-way diff
								</Link>
							</Button>
