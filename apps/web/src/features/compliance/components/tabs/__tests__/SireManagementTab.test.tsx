...
describe('SireManagementTab', () => {
  ...
  it('renders SIRE summary and shows export feedback on top actions', () => {
    ...
    expect(screen.getByText(/export demo anclado al dataset seed de marzo 2026 para validación del pitch/i)).toBeInTheDocument();
    expect(screen.getByText('Resumen verificado')).toBeInTheDocument();
    expect(screen.getByText(/resumen sire confirmado contra el dataset demo del backend/i)).toBeInTheDocument();
    expect(screen.getByText('Ultima verificacion: 31/03/2026 13:45 PET')).toBeInTheDocument();
    ...
    expect(screen.getByText('RVIE Ventas TXT listo para Marzo 2026')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: /rce excel/i }));
    expect(screen.getByText('RCE Compras Excel listo para Marzo 2026')).toBeInTheDocument();
  ...
  it('shows a loading state instead of fallback rows while summary is pending', () => {
    ...
    expect(screen.getByText('Ultima verificacion pendiente')).toBeInTheDocument();
    expect(screen.getAllByText('Cargando').length).toBeGreaterThan(0);
    expect(screen.getByText('... Coincidencias')).toBeInTheDocument();
    expect(screen.queryByText('Cliente Demo Retail SAC')).not.toBeInTheDocument();
  });
  ...
  it('shows an explicit error state when summary fetch fails', () => {
    ...
    expect(screen.getAllByText('Resumen no disponible').length).toBeGreaterThan(0);
    expect(screen.getByText(/se muestra estado sin datos/i)).toBeInTheDocument();
    expect(screen.getByText('Ultima verificacion pendiente')).toBeInTheDocument();
    expect(screen.getAllByText('Sin datos').length).toBeGreaterThan(0);
    expect(screen.getByText('— Coincidencias')).toBeInTheDocument();
    expect(screen.getByText('No se pudo cargar el preview SIRE para este periodo demo.')).toBeInTheDocument();
  });
});
