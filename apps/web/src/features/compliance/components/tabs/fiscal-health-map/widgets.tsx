...
function getRiskCount(
	...
	return risks.filter(
		(risk) => risk.category === category && risk.severity === severity,
	).length;
}
		...
		<div className="grid grid-cols-5 gap-2">
			...
			{severities.map((severity) => (
				<div
					key={severity}
					className={cn(
						"py-2 text-center text-2xs font-bold uppercase rounded",
...
export function RiskCard({ risk, index }: { risk: RiskItem; index: number }) {
	...
	return (
		...
		>
			<article
				className={cn(
					"group cursor-pointer rounded-xl border p-4 shadow-sm transition-[border-color,background-color,box-shadow] duration-200 hover:shadow-md",
					config.bg,
					config.border,
				)}
			>
				<div className="mb-2 flex items-start justify-between">
					<span
						className={cn(
							"rounded px-2 py-0.5 text-2xs font-black uppercase tracking-wider",
							config.bg,
							config.color,
						)}
					>
...
export function ContributorHealthCard({
	contributor,
}: {
		...
		<div className="flex items-center justify-between rounded-lg border border-border/30 bg-card/55 p-3 transition-[border-color,background-color] duration-200 hover:border-border/60 hover:bg-card/80">
			...
			<div className="text-right">
				...
				<p className="mt-1 text-2xs text-muted-foreground">
					{contributor.documentsCount} docs
				</p>
			</div>
		</div>
	);
