...
export const useInvoices = (filters: UseInvoicesFilters = {}) => {
  ...
  const { data: remoteInvoices = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return invoices
        .filter((invoice): invoice is ApiInvoiceListItem => invoice.status !== 'CANCELLED')
  ...
  const columnTotals = useMemo(() => ({
    sent: invoicesByStatus.sent.reduce((acc: number, i: Invoice) => acc + i.amount, 0),
    overdue: invoicesByStatus.overdue.reduce((acc: number, i: Invoice) => acc + i.amount, 0),
  }), [invoicesByStatus]);

  // --- Real API Mutations ---
  const createInvoice = useMutation({
    mutationFn: async (
      data: Omit<CreateInvoicePayload, 'companyId'>,
    ) => {
  ...
  const updateInvoiceStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string, status: InvoiceStatus }) => {
      return await invoicingApi.updateStatus(id, status.toUpperCase() as ApiInvoiceStatus);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: invoiceKeys.all });
    }
