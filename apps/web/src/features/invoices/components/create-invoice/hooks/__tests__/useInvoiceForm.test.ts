...
describe('fiscal date helpers', () => {
  it('formats fiscal dates from local calendar parts instead of UTC serialization', () => {
    ...
    expect(formatFiscalDateInput(addFiscalDays(localDate, 30))).toBe('2026-03-30');
  });
});
...
describe('useInvoiceForm', () => {
  ...
  it('adds, updates and removes line items', () => {
    ...
    act(() => {
      result.current.actions.updateItem(initialId, 'description', 'Servicio mensual');
      result.current.actions.updateItem(initialId, 'quantity', 3);
      result.current.actions.updateItem(initialId, 'unitPrice', 45.5);
    });

    const updated = result.current.formState.items.find((item) => item.id === initialId);
  ...
  it('validates fiscal issue and due dates before submit', async () => {
    ...
    expect(alertMock).toHaveBeenCalledWith('Ingrese una fecha de emisión válida');
    expect(onSubmit).not.toHaveBeenCalled();
    ...
    expect(alertMock).toHaveBeenCalledWith('Ingrese una fecha de vencimiento válida');
    expect(onSubmit).not.toHaveBeenCalled();
  ...
  it('submits payload and runs success callback', async () => {
    ...
    act(() => {
      ...
      result.current.actions.updateItem(firstItemId, 'description', 'Servicio profesional');
