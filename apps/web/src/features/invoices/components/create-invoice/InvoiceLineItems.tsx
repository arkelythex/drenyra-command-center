		...
		<div className="space-y-10">
			<div className="flex items-center justify-between px-2">
				<div className="flex flex-col gap-1">
					<Label
						className={cn(
							"text-2xs font-black uppercase tracking-[0.3em] text-primary flex items-center gap-2",
					...
					<p className="text-label text-muted-foreground/60 font-medium uppercase tracking-tight">
						Listado de conceptos y servicios devengados
					</p>
				</div>
			...
			<div className="space-y-8">
				{items.map((item) => {
					...
					return (
						<div
							key={item.id}
							className="group relative overflow-hidden rounded-3xl border border-border bg-card/80 p-8 shadow-lg  transition-[background-color,border-color,box-shadow] duration-300 hover:bg-card sm:p-10"
						>
							...
							<div className="grid grid-cols-12 gap-8 items-end">
								<div className="col-span-12 lg:col-span-5 space-y-4">
									<Label
										className={cn(
											"text-2xs font-black uppercase tracking-widest text-muted-foreground/50 flex items-center gap-2 pl-1",
											LEGIBILITY.textShadow.light,
										)}
									>
										<Tag size={12} />
								...
								<div className="col-span-6 md:col-span-2 space-y-4">
									<Label className="text-2xs font-black uppercase tracking-widest text-muted-foreground/50 pl-1">
										Cantidad
									</Label>
									<Input
										type="number"
								...
								<div className="col-span-10 md:col-span-2 space-y-4">
									...
									<div className="relative group/sel">
										<select
											value={item.taxType}
											onChange={(e) =>
												onUpdate(item.id, "taxType", e.target.value)
											}
											onFocus={() => trigger("light")}
							...
							<div className="flex flex-wrap items-center justify-end gap-x-12 gap-y-6 pt-8 border-t border-border/70 mt-2">
								<div className="flex flex-col items-end gap-1">
									<span className="text-3xs font-black uppercase tracking-[0.2em] text-muted-foreground/40">
										Subtotal Operación
									</span>
									<span className="font-mono text-sm font-black text-muted-foreground/80 tracking-tighter tabular-nums">
										{n(
											subtotal,
											currency as unknown as Parameters<typeof n>[1],
										)}
									</span>
								</div>
								<div className="flex flex-col items-end gap-1">
									...
									<span className="font-mono text-sm font-black text-muted-foreground/80 tracking-tighter tabular-nums">
										{n(igv, currency as unknown as Parameters<typeof n>[1])}
									</span>
								</div>
								<div className="flex flex-col items-end gap-1 pl-12 border-l border-border">
									<div className="flex items-center gap-2">
										...
										<span
											className={cn(
												"text-2xs font-black uppercase tracking-[0.2em] text-foreground",
												LEGIBILITY.textShadow.light,
											)}
										>
											Total Concepto
									...
									<span
										className={cn(
											"font-mono text-2xl font-black text-foreground tabular-nums tracking-tighter",
				...
				{items.length === 0 && (
					<button
						type="button"
						className="group flex w-full cursor-pointer flex-col items-center justify-center rounded-4xl border-2 border-dashed border-border bg-card/60 py-24  transition-[background-color,border-color,box-shadow] hover:bg-card/80"
						onClick={handleAdd}
					>
						...
						<p
							className={cn(
								"text-xs font-black text-foreground uppercase tracking-[0.3em] mb-3",
								LEGIBILITY.textShadow.light,
							)}
						>
							Comprobante Vacío
						</p>
						<p className="text-label text-muted-foreground/60 uppercase tracking-widest font-bold max-w-xs text-center leading-relaxed">
							Inicie el detalle de bienes o servicios para la liquidación fiscal
						</p>
						<span className="mt-10 inline-flex h-12 items-center rounded-2xl border border-primary/30 px-10 text-2xs font-black uppercase tracking-[0.2em] text-primary shadow-xl">
