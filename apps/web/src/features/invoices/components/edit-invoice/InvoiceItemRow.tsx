    ...
    <div className="bg-background border border-border rounded-lg p-4 space-y-3">
      <div className="grid grid-cols-12 gap-3">
        ...
        <div className="col-span-2">
          <select
            ...
            value={item.taxType}
            onChange={(e) =>
              onUpdate(index, 'taxType', e.target.value as InvoiceItem['taxType'])
            }
      ...
      <div className="flex justify-end gap-4 text-label font-mono">
        <span className="text-muted-foreground">
          Subtotal: <span className="text-foreground font-bold">{currency} {item.subtotal.toFixed(2)}</span>
        </span>
        <span className="text-muted-foreground">
          IGV: <span className="text-foreground font-bold">{currency} {item.igv.toFixed(2)}</span>
        </span>
        <span className="text-muted-foreground">
          Total: <span className="text-foreground font-bold">{currency} {item.total.toFixed(2)}</span>
        </span>
      </div>
    </div>
  );
