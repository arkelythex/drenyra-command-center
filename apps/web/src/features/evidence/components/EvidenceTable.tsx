	...
	return (
		<div className="overflow-hidden rounded-2xl border border-[var(--border-subtle)]">
				...
				<table className="w-full min-w-[760px]">
					...
					<tbody>
						{items.map((item) => (
							<tr
								key={item.id}
								className="border-t border-[var(--border-subtle)] text-xs text-[var(--text-secondary)]"
							>
								...
								<td className="px-4 py-3">
									<span className="rounded-full bg-[var(--surface-2)] px-2 py-1 text-2xs font-semibold">
			...
			<div className="flex items-center justify-between border-t border-[var(--border-subtle)] px-4 py-3 text-xs text-[var(--text-tertiary)]">
				<span>
					Página {currentPage} de {pages} · {total} resultados
				</span>
