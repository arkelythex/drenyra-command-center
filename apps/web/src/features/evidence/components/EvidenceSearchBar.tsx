...
function FilterSelect<T extends string>({
	...
	return (
		<Select
			value={value ?? "all"}
			onValueChange={(next) =>
				onValueChange(next === "all" ? undefined : (next as T))
			}
		>
...
export function EvidenceSearchBar({
	...
	const update = (next: Partial<EvidenceFilters>) =>
		...
		<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
			...
			<Select
				...
				onValueChange={(companyId) =>
					update({ companyId: companyId === "all" ? undefined : companyId })
				}
			>
