      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5 custom-scrollbar">
        <Card className="border-border/60 bg-[var(--surface-2)]">
          ...
          <CardContent className="space-y-4 p-5">
            ...
            <div className="rounded-2xl border border-border/60 bg-background p-4">
              <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Impacto</p>
              <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">
                {activeCandidate?.impact ?? "No aplicar automatización hasta completar soporte contable."}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/60 bg-[var(--surface-2)]">
          <CardHeader className="border-b border-border/50">
            ...
            <CardDescription>Fuentes visibles antes de decidir.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 p-5">
            {(activeCandidate?.sourceRecords ?? ["Extracto bancario activo", "No hay evidencia contable vinculada"]).map((record) => (
              <div
                key={record}
              ...
              >
                ...
                <p className="text-sm leading-6 text-[var(--text-secondary)]">{record}</p>
              </div>
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="border-border/60 bg-[var(--surface-2)]">
          <CardHeader className="border-b border-border/50">
            ...
            <CardDescription>Cambios que se aplicarían si apruebas.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 p-5">
            {(activeCandidate?.proposedDiff ?? [
              ...
              >
                <p className="text-sm leading-6 text-[var(--text-secondary)]">{diff}</p>
              </div>
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="border-border/60 bg-[var(--surface-2)]">
          ...
          <CardContent className="space-y-4 p-5">
            ...
            <div className="grid gap-3">
              <Button onClick={() => handleDecision("approve")} disabled={!activeCandidate}>
            ...
            <div className="rounded-2xl border border-border/60 bg-background p-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm leading-6 text-[var(--text-secondary)]">
                  El rail mantiene la fuente, el impacto, el diff propuesto y la acción tomada en un solo lugar.
                </p>
              </div>
            </div>
          </CardContent>
