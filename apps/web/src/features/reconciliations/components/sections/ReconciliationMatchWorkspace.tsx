				...
				<div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]">
					<Card className="border-border/60 bg-[var(--surface-1)]">
						<CardHeader className="border-b border-border/50">
							...
							<CardDescription>
								Detalle operativo del extracto activo.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4 p-5">
							...
							<div className="grid gap-4 sm:grid-cols-3">
								...
								<Metric
									label="Monto"
									value={formatMoney(activeTransaction.amount)}
									mono
								/>
								<Metric
									label="Estado"
									value={activeTransaction.status.replace("_", " ")}
								/>
							</div>
...
function Metric({
	...
	return (
		<div className="rounded-2xl border border-border/60 bg-[var(--surface-2)] p-4">
			...
			<p
				className={`mt-2 text-sm font-semibold text-foreground ${mono ? "font-mono tabular-nums" : ""}`}
			>
				{value}
			</p>
		</div>
...
function EmptyPanel() {
	return (
		<div className="rounded-2xl border border-dashed border-border/60 bg-[var(--surface-2)] p-6 text-center">
			...
			<p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">
				Este movimiento requiere investigación manual o creación de un nuevo
				asiento.
			</p>
		</div>
	);
