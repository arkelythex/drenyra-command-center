...
import type { ArtifactInteractionEvent, WorkspaceArtifact } from '@/features/artifacts/types/artifact.types';
...
const ArtifactEventContext = createContext<ArtifactEventContextType | undefined>(undefined);

export const ArtifactEventProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [activeTraceId, setActiveTraceId] = useState<string | null>(null);
  const [activeArtifact, setActiveArtifact] = useState<WorkspaceArtifact | null>(null);
  const [lastArtifactEvent, setLastArtifactEvent] = useState<ArtifactInteractionEvent | null>(null);
  const [artifactEvents, setArtifactEvents] = useState<ArtifactInteractionEvent[]>([]);

  const syncArtifactEvents = useCallback((events: ArtifactInteractionEvent[]) => {
  ...
  const pushArtifactEvent = useCallback((event: ArtifactInteractionEvent) => {
...
export const useArtifactEvents = (): ArtifactEventContextType => {
  ...
  if (!context) {
    throw new Error('useArtifactEvents must be used within ArtifactEventProvider');
  }
  return context;
};
