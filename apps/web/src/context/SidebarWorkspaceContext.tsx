...
const SidebarWorkspaceContext = createContext<SidebarWorkspaceContextType | undefined>(undefined);

export const SidebarWorkspaceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [isWorkspaceMode, setIsWorkspaceMode] = useState(() => readInitialPreferences().isWorkspaceMode);
  const [streamWidth, setStreamWidthState] = useState(() => readInitialPreferences().streamWidth);
...
export const useSidebarWorkspace = (): SidebarWorkspaceContextType => {
  ...
  if (!context) {
    throw new Error('useSidebarWorkspace must be used within SidebarWorkspaceProvider');
  }
  return context;
};
