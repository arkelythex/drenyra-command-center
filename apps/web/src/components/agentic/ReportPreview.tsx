...
const REPORTS: ReportConfig[] = [
  { id: 'balance', label: 'Balance General', icon: PieChart, description: 'Activos, Pasivos y Patrimonio' },
  { id: 'income', label: 'Estado de Resultados', icon: TrendingUp, description: 'Ingresos y Gastos' },
  { id: 'cashflow', label: 'Flujo de Efectivo', icon: DollarSign, description: 'Efectivo neto operativo' },
];

// Mock data for each report type
const MOCK_DATA: Record<ReportType, Array<{ name: string; value: number; previousValue?: number; isTotal?: boolean; isNegative?: boolean }>> = {
  balance: [
    ...
    { name: 'Total Activo', value: 9880000, previousValue: 9790000, isTotal: true },
    { name: 'Cuentas por Pagar', value: 1250000, previousValue: 1100000, isNegative: true },
    { name: 'Deuda Bancaria', value: 2100000, previousValue: 2300000, isNegative: true },
    { name: 'Total Pasivo', value: 3350000, previousValue: 3400000, isTotal: true, isNegative: true },
    { name: 'Capital', value: 5000000, previousValue: 5000000 },
    { name: 'Resultados Acumulados', value: 1530000, previousValue: 1390000 },
    { name: 'Total Patrimonio', value: 6530000, previousValue: 6390000, isTotal: true },
  ],
  income: [
    { name: 'Ventas Netas', value: 8900000, previousValue: 8200000 },
    { name: 'Costo de Ventas', value: -5340000, previousValue: -4920000, isNegative: true },
    { name: 'Utilidad Bruta', value: 3560000, previousValue: 3280000, isTotal: true },
    { name: 'Gastos Operativos', value: -1850000, previousValue: -1720000, isNegative: true },
    { name: 'EBITDA', value: 1710000, previousValue: 1560000, isTotal: true },
    { name: 'Depreciación', value: -320000, previousValue: -310000, isNegative: true },
    { name: 'Utilidad Operativa', value: 1390000, previousValue: 1250000, isTotal: true },
    { name: 'Gastos Financieros', value: -180000, previousValue: -195000, isNegative: true },
    { name: 'Utilidad Neta', value: 1210000, previousValue: 1055000, isTotal: true },
  ],
  cashflow: [
      ...
      {report.map((item, i) => (
        <div
          ...
          className={cn(
            ...
            item.isTotal && "border-t border-[var(--border-subtle)] bg-[var(--surface-2)]/50 font-semibold",
            !item.isTotal && "hover:bg-[var(--surface-2)]/30",
          )}
        >
          <span className={cn(
            "truncate",
            item.isTotal ? "text-[var(--text-primary)]" : "text-[var(--text-secondary)]",
            item.isNegative && !item.isTotal && "text-[var(--color-danger)]/80",
          )}>
            {item.name}
          </span>
          <span className={cn(
...
export function ReportPreview() {
  const getActiveCompany = useAccountingStore((s) => s.getActiveCompany);
  const getActivePeriod = useAccountingStore((s) => s.getActivePeriod);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="border-b border-[var(--border-subtle)] px-4 py-3">
        ...
        <p className="text-2xs text-[var(--text-muted)] mt-0.5">
          {activeCompany?.name ?? 'Empresa'} · {activePeriod?.label ?? 'Período actual'}
        </p>
      </div>
