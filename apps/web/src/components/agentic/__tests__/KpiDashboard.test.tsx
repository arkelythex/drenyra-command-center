...
vi.mock('@/lib/utils', () => ({
	...
	n: (value: number) => `S/ ${value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
}));
...
const DEFAULT_KPI_METRICS = [
	{ id: 'kpi-1', label: 'Liquidez Corriente', value: 1.85, previousValue: 1.72, variance: 7.6, trend: 'up' as const, format: 'number' as const },
	{ id: 'kpi-2', label: 'Margen Neto', value: 12.4, previousValue: 11.8, variance: 5.1, trend: 'up' as const, format: 'percentage' as const },
	{ id: 'kpi-3', label: 'Días de Cobranza', value: 45, previousValue: 52, variance: -13.5, trend: 'up' as const, format: 'days' as const },
	{ id: 'kpi-4', label: 'ROE', value: 18.2, previousValue: 19.1, variance: -4.7, trend: 'down' as const, format: 'percentage' as const },
	{ id: 'kpi-5', label: 'Efectivo disponible', value: 2840000, previousValue: 3100000, variance: -8.4, trend: 'down' as const, format: 'currency' as const },
];
...
vi.mock('@/stores/accounting-store', () => ({
	useAccountingStore: (selector: (s: Record<string, unknown>) => unknown) =>
		selector({
			...
			getActiveCompany: () => ({ id: 'comp-1', name: 'TechCorp Perú SAC', ruc: '20123456789' }),
			getActivePeriod: () => ({ id: 'per-2026-04', label: 'Abril 2026', year: 2026, month: 4, isClosed: false, startDate: '2026-04-01', endDate: '2026-04-30' }),
		}),
}));
...
describe('KpiDashboard', () => {
	...
	it('renders trend indicators in footer', () => {
		...
		expect(screen.getByText(/Deterioro vs período anterior/i)).toBeInTheDocument();
	});
