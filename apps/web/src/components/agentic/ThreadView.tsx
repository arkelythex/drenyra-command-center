...
import { CheckpointHistory, type Checkpoint } from "./CheckpointHistory";
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-2)]">
      <div className="flex items-center">
        ...
        {toolCall.status === "completed" && toolCall.entityType && toolCall.entityId && onShowCheckpoints && (
          <button
            onClick={() => onShowCheckpoints(toolCall.entityType!, toolCall.entityId!)}
      ...
      >
        ...
        <AnimatePresence>
          {message.toolCalls?.map((tc) => (
            ...
            >
              <ToolCallCard toolCall={tc} onShowCheckpoints={onShowCheckpoints} />
            </motion.div>
          ))}
        </AnimatePresence>
...
export function ThreadView({ messages, isStreaming, loadingHistory }: ThreadViewProps) {
  ...
  const [checkpoints] = useState<Checkpoint[]>(() => [
    ...
    },
    {
      ...
      beforeSnapshot: { account: "4011", debit: 0, credit: 1800, period: "2026-02" },
      afterSnapshot: { account: "4011", debit: 0, credit: 1800, period: "2026-03" },
      diff: '-  "period": "2026-02"\n+  "period": "2026-03"',
      agentId: "agent-contable-2",
      ...
      >
        {loadingHistory && (
          <div className="flex items-center justify-center gap-2 py-4">
            ...
            <span className="text-xs text-[var(--text-muted)]">Loading conversation history...</span>
          </div>
        )}
          ...
          {displayMessages.length === 0 && !isStreaming ? (
            ...
            <>
              {displayMessages.map((msg) => (
                <MessageBubble key={msg.id} message={msg} onShowCheckpoints={onShowCheckpoints} />
              ))}
            </>
          )}
