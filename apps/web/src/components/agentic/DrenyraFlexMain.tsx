  ...
  const handleSend = useCallback(
    async (text: string) => {
      if (text.startsWith("/")) {
        ...
        if (intent) {
          navigate({ to: intent.target as unknown as Parameters<typeof navigate>[0]['to'] });
          return;
        }
      }
        ...
        left={
          <div className="flex h-full flex-col">
            ...
            <div className="flex-1 overflow-y-auto">
              <Suspense fallback={<div className="flex h-full items-center justify-center p-8"><span className="text-sm text-[var(--text-muted)]">Loading conversation...</span></div>}>
                <ThreadView messages={messages} isStreaming={isStreaming} loadingHistory={loadingHistory} />
              </Suspense>
            </div>
            ...
            <Composer onSend={handleSend} isSending={isStreaming} onFileUpload={handleUploadEvidence} />
          </div>
        }
        right={<CentralBoard />}
      />
