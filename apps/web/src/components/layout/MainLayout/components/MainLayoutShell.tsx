...
export function MainLayoutShell({ children }: MainLayoutShellProps) {
	...
	useEffect(() => {
		const onResize = () => {
			const tab = matchMedia("(min-width:640px) and (max-width:1023px)").matches;
	...
	const handleArtifactEvent = useCallback(
		...
		[ae.activeArtifact, ae.pushArtifactEvent, ae.setActiveArtifact, ae.setActiveTraceId, ae.setLastArtifactEvent],
	);

	const sidebarWidth = sl.isFocusMode
		? "pointer-events-none w-0 opacity-0 lg:w-0"
		: cn("w-[88vw] max-w-[336px] opacity-100", sl.isCollapsed ? "lg:w-[64px]" : "lg:w-[260px]");

	return (
		<div className="flex w-full h-[100dvh] text-foreground overflow-hidden font-sans relative transition-colors duration-300 selection:bg-primary/20 bg-[var(--akx-workspace-bg)]">
			<a
				href="#main-content"
			...
			{sl.isMobileOpen && (
				<div
					...
					role="presentation"
					tabIndex={-1}
					onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); sl.setIsMobileOpen(false); } }}
				/>
			)}
			<div
				className={cn(
					...
					sl.isMobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0",
					sidebarWidth,
				)}
				role={sl.isMobileOpen ? "dialog" : undefined}
				aria-modal={sl.isMobileOpen ? "true" : undefined}
			>
				<Sidebar
