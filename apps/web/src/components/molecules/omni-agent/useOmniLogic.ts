...
import { CommandItem, GHOST_SUGGESTIONS, NAVIGATION_COMMANDS, PLACEHOLDERS, QUICK_ACTIONS } from './constants';
import { resolveArtifactFromQuery } from '@/features/artifacts/artifact-factories';

export const useOmniLogic = () => {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState('');
  const [mode, setMode] = useState<'default' | 'navigation' | 'actions' | 'voice'>('default');
  ...
  const filteredList = useMemo(() => {
    ...
    if (mode === 'navigation') {
      ...
      if (cleanQuery.startsWith('/')) cleanQuery = cleanQuery.substring(1).trim();
    ...
    return sourceList.filter((item) => {
      ...
      const keywordMatch = item.keywords?.some((keyword) => keyword.includes(cleanQuery));
  ...
  const ghostSuggestion = useMemo(() => {
    ...
    return GHOST_SUGGESTIONS.find((suggestion) => suggestion.startsWith(normalized) && suggestion !== normalized) ?? '';
  }, [mode, query]);
  ...
  const handleAction = async () => {
    ...
    const traceId = resolvedArtifact?.metadata.traceId ?? `tr_${Date.now().toString(36)}`;
    ...
    if (resolvedArtifact) {
      ...
      setCot((prev) => [...prev, `Artifact generado: ${resolvedArtifact.title}`]);
    } else {
      setCot((prev) => [...prev, 'Comando procesado. Sin visualización requerida.']);
    }
  ...
  useEffect(() => {
    ...
    const down = (event: KeyboardEvent) => {
      ...
      if (event.key === 'ArrowDown') {
        event.preventDefault();
        setSelectedIndex((current) => (listLength > 0 ? (current + 1) % listLength : 0));
      } else if (event.key === 'ArrowUp') {
        event.preventDefault();
        setSelectedIndex((current) => (listLength > 0 ? (current - 1 + listLength) % listLength : 0));
  ...
  useEffect(() => {
    if (scrollContainerRef.current && filteredList.length > 0) {
      const selectedElement = scrollContainerRef.current.children[selectedIndex] as HTMLElement | undefined;
