				...
				{artifactEvents.length > 0 ? (
					<div className="space-y-2 rounded-2xl border border-primary/20 bg-primary/10 px-3 py-2">
						<div className="flex items-center justify-between">
							<p className="text-2xs font-black uppercase tracking-wide text-primary">
						...
						<div className="space-y-1.5">
							{artifactEvents.slice(0, 4).map((event) => (
								<div
									key={event.id}
									className="rounded-lg border border-primary/15 bg-muted/40 px-2 py-1.5"
								>
									<p className="text-2xs font-semibold uppercase tracking-tight text-primary">
										{event.message}
									</p>
				...
				{isThinking || cot.length > 0 ? (
					<div className="animate-in space-y-4 fade-in duration-500">
						<div className="mb-2 flex items-center gap-2">
							...
							<span className="text-3xs font-black uppercase tracking-[0.3em] text-foreground">
								Active Trace
							</span>
						</div>
						{cot.map((step, index) => (
							<div
								key={`${step}-${index}`}
								className="group flex animate-in items-start gap-4 slide-in-from-left-2 duration-300"
							>
								<div
									className={cn(
					...
					<div className="flex h-full flex-col items-center justify-center space-y-4 text-center opacity-20">
						...
						<p className="text-2xs font-black uppercase tracking-[0.3em]">
							Awaiting Command
						</p>
					</div>
				)}
			...
			<div className="border-t border-border bg-card/80 p-4 ">
				<div className="flex items-center gap-3 rounded-2xl border border-border bg-muted/40 p-2 transition-[background-color,border-color,box-shadow] duration-200 focus-within:border-primary/40">
					...
					<div className="flex gap-1 pr-2">
						<kbd className="h-5 rounded border border-border bg-muted px-1.5 text-3xs font-mono text-muted-foreground">
							⌘
						</kbd>
						<kbd className="h-5 rounded border border-border bg-muted px-1.5 text-3xs font-mono text-muted-foreground">
							Tab
