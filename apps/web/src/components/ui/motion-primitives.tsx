...
export function MotionDiv(
  { className, variants, initial = false, animate, tagName = "div", ref, ...props }: HTMLMotionProps<"div"> & { tagName?: MotionTag } & { ref?: Ref<HTMLDivElement> }
) {
  const shouldReduceMotion = useReducedMotion() === true;
  // Polymorphic tags share MotionProps but ref targets differ; keep div ref for the wrapper API.
  const Component = (MOTION_TAG_COMPONENTS[tagName] ?? motion.div) as typeof motion.div;
...
export function MotionButton(
  { className, whileHover, whileTap, ref, ...props }: HTMLMotionProps<"button"> & { ref?: Ref<HTMLButtonElement> }
) {
  const shouldReduceMotion = useReducedMotion() === true;
  ...
  return (
    <motion.button
      ...
      transition={shouldReduceMotion ? undefined : { duration: 0.1, ease: "easeOut" }}
      className={cn(className)}
      {...props}
    />
  );
...
export function AnimatedNumber({
  ...
  const springValue = useSpring(0, {
    stiffness: shouldReduceMotion ? 1000 : MOTION_VARIANTS.springGentle.stiffness,
  ...
  if (shouldReduceMotion) {
    return <span ref={ref} className={cn("tabular-nums", className)}>{staticDisplayValue}</span>;
  }

  return <motion.span ref={ref} className={cn("tabular-nums", className)}>{displayValue}</motion.span>;
}
