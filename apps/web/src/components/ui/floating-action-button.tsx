...
import {
	QUICK_ACTIONS,
	QUICK_ROUTES,
} from "./floating-action-button/constants";
...
export function FloatingActionButton({
	...
	const filteredRoutes = useMemo(
		() =>
			query.startsWith("/")
				? QUICK_ROUTES.filter((route) =>
						route.path.toLowerCase().includes(query.toLowerCase().slice(1)),
					)
				: [],
		[query],
	);
	...
	const handleKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
		...
		if (event.key === "ArrowDown") {
			...
			setSelectedIndex((previous) =>
				filteredRoutes.length ? (previous + 1) % filteredRoutes.length : 0,
			);
			return;
		}
		...
		if (event.key === "ArrowUp") {
			...
			setSelectedIndex((previous) =>
				filteredRoutes.length
					? (previous - 1 + filteredRoutes.length) % filteredRoutes.length
					: 0,
			);
			return;
		}
