  ...
  return (
    ...
    >
      <AnimatePresence>
        {filteredRoutes.length > 0 ? (
          <motion.div
            ...
            className="custom-scrollbar z-20 mb-2 max-h-48 w-full overflow-y-auto rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-surface)] shadow-xl backdrop-blur-lg"
          >
            {filteredRoutes.map((route, index) => (
              ...
              >
                <route.icon size={16} className="shrink-0 text-[var(--accent)]" />
                <div className="flex min-w-0 flex-col">
                  <span className="truncate text-sm font-medium text-primary">{route.label}</span>
                  <span className="truncate font-mono text-2xs text-secondary">{route.path}</span>
                </div>
                {index === selectedIndex ? (
                  <ArrowRight size={14} className="ml-auto shrink-0 text-secondary" />
                ) : null}
              </button>
            ))}
          </motion.div>
      ...
      <motion.div
        layout
        transition={{ duration: prefersReducedMotion ? 0.1 : 0.16, ease: 'easeOut' }}
        className="relative z-30 flex h-14 w-full shrink-0 items-center gap-3 overflow-hidden rounded-full border border-[var(--glass-border)] bg-[var(--glass-surface)] px-4 shadow-xl backdrop-blur-lg"
      >
