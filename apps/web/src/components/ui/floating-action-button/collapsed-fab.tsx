	...
	return (
		...
		>
				...
				{isOpen ? (
					...
					>
						{actions.map((action, index) => (
							...
							>
								<span className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)]/95 px-3 py-1.5 text-xs font-medium tracking-wide text-primary shadow-md ">
									{action.label}
								</span>
								...
								<Button
									...
									className="group relative h-11 w-11 overflow-hidden rounded-full border border-[var(--glass-border)] bg-[var(--glass-surface)] shadow-md  transition-[background-color,border-color,box-shadow,transform] duration-150"
			...
			<Button
				size="icon"
				aria-label={
					isOpen ? "Cerrar acciones rapidas" : "Abrir acciones rapidas"
				}
				className={cn(
