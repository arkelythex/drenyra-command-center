...
import type { ExportPlugin, ExportFormat } from "../export-service";
...
describe("CSV export", () => {
  ...
  it("escapes cells containing quotes", async () => {
    ...
    const blob = await exportData({ filename: "test.csv", format: "csv", data });
    const text = await blob.text();
  ...
  it("handles empty data array", async () => {
    const blob = await exportData({ filename: "empty.csv", format: "csv", data: [] });
    const buffer = await blob.arrayBuffer();
    const bytes = new Uint8Array(buffer);
...
describe("TSV export", () => {
  ...
  it("includes BOM", async () => {
    const blob = await exportData({ filename: "test.tsv", format: "tsv", data: sampleData });
    const buffer = await blob.arrayBuffer();
    const bytes = new Uint8Array(buffer);
    expect(bytes[0]).toBe(0xef);
    expect(bytes[1]).toBe(0xbb);
...
describe("JSON export", () => {
  ...
  it("pretty-prints JSON", async () => {
    const blob = await exportData({ filename: "test.json", format: "json", data: sampleData });
    const text = await blob.text();
