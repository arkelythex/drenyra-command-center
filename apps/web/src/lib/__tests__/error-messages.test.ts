...
describe("presentError", () => {
	it("maps backend tenant codes to accountant-friendly Spanish messages", () => {
		expect(
			presentError(new Error("COMPANY_SCOPE_REQUIRED"), "Fallback"),
		).toMatchObject({
	...
	it("maps banking resource errors to contextual Spanish messages", () => {
		expect(
			presentError(new Error("ACCOUNT_NOT_FOUND"), "Fallback"),
		).toMatchObject({
	...
	it("preserves fallback title while keeping the raw description for unknown errors", () => {
		expect(
			presentError(new Error("trace unavailable"), "No se pudo cargar"),
		).toMatchObject({
