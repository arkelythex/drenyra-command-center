...
export function buildApiUrl(path: string): string {
  ...
  const baseUrl = useProxyBase ? runtimeConfig.apiUrl : runtimeConfig.directApiUrl;
...
async function parseResponseBody(response: Response): Promise<unknown> {
  ...
  if (isJson) {
    try {
      ...
      throw new HttpClientError('Invalid JSON response', 'parse_error', response.status);
    }
  }
...
export async function requestJson<T>(
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      throw new HttpClientError(`Request timeout after ${timeoutMs}ms`, 'timeout');
    }
