  ...
  persist(
    (set, get) => ({
      ...
      pinThread: (id) =>
        set((state) => ({
          threads: state.threads.map((t) =>
            t.id === id ? { ...t, status: 'pinned' as const, updatedAt: new Date().toISOString() } : t,
          ),
        })),
      ...
      setThreadBrainId: (id, brainThreadId) =>
        set((state) => ({
          threads: state.threads.map((t) =>
            t.id === id ? { ...t, brainThreadId, updatedAt: new Date().toISOString() } : t,
          ),
        })),
      ...
      reorderThreads: (orderedIds) =>
        set((state) => {
          ...
          const remaining = state.threads.filter(t => !orderedIds.includes(t.id));
