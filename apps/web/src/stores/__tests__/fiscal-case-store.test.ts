...
describe("FiscalCaseStore", () => {
  ...
  it("should update fiscal case status", () => {
    ...
    expect(useFiscalCaseStore.getState().fiscalCases[0].status).toBe("resolved");

    useFiscalCaseStore.getState().updateFiscalCaseStatus(caseId, "in-review");
    expect(useFiscalCaseStore.getState().fiscalCases[0].status).toBe("in-review");
  });
  ...
  it("should not modify other cases when updating status", () => {
    ...
    expect(useFiscalCaseStore.getState().fiscalCases[1].status).toBe("resolved");
  });
