...
import type { DocumentItem, JournalEntry } from "../central-board-store";
...
describe("CentralBoardStore", () => {
	...
	it("should update a journal entry", () => {
		...
		useCentralBoardStore
			.getState()
			.updateJournalEntry("je-1", { glosa: "Updated" });
		expect(useCentralBoardStore.getState().journalEntries[0].glosa).toBe(
			"Updated",
		);
	});
	...
	it("should not affect other entries when approving one", () => {
		...
		expect(
			useCentralBoardStore
				.getState()
				.journalEntries.find((e) => e.id === "je-1")?.status,
		).toBe("pending");
		expect(
			useCentralBoardStore
				.getState()
				.journalEntries.find((e) => e.id === "je-2")?.status,
		).toBe("rejected");
	});
