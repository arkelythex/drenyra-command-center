...
| Mission state machine (transitions, guards, recovery paths) | `packages/mission-domain` (uses protocol) | `drenyra-ai/missions/transitions` | **Extracted** — `mission-status.ts`, `mission-transitions.ts`, `mission-contracts.ts`, `mission-events.ts` are adapter shims re-exporting `drenyra-ai/missions`; legacy divergent command types (RunIntentCommand/ApproveCommand/…) had no external consumers (apps/web define their own) and were **retired**. `mission-errors.ts` stays local by design: its 13-code taxonomy (incl. domain-only `FORBIDDEN`) and status-code mappings (FORBIDDEN→403, HARNESS_TIMEOUT→500) differ from the canonical 31-code spec, and no external consumer needs the shared class identity |
...
| Candidate identity + materiality + review lenses | `packages/drenyra-orchestrator` (partial) + `packages/domain/src/feos` | `drenyra-ai/candidates` + `drenyra-ai/review` | **Extracted (lenses)** — `drenyra-orchestrator/review-lenses.ts` + `work-routing.ts` are adapter shims re-exporting `drenyra-ai/review` (their own tests pass 22/22 through the shims). Candidate identity/materiality live in `drenyra-ai/candidates`; `packages/domain/src/feos/candidate-review.ts` (FEOS lifecycle) stays until its consumers migrate |
