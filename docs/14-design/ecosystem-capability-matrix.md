...
| Mission state machine (transitions, guards, recovery paths) | `packages/mission-domain` (uses protocol) | `drenyra-ai/missions/transitions` | **Extracted** — `mission-status.ts` + `mission-transitions.ts` are adapter shims re-exporting `drenyra-ai/missions` (explicit names); legacy `mission-errors.ts` (13-code taxonomy incl. domain-only `FORBIDDEN`), `mission-contracts.ts` and `mission-events.ts` (divergent command naming consumed by apps/web) remain as documented legacy adapters pending consumer alignment |
