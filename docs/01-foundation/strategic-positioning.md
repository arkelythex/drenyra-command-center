...
Los contadores, estudios contables y pymes de LATAM operan con herramientas desconectadas:
...
## Ventaja competitiva 2026

| Factor                          | Detalle                                                                                                |
| ------------------------------- | ------------------------------------------------------------------------------------------------------ |
| **Exact Candidate Authority**   | No se aprueban intenciones. Se congela, hashea y verifica el payload exacto antes de ejecutar.         |
| **R0–R3 Strict Tool Contracts** | JSON Schema obligatorio para R2/R3. Pi 0.82+ permite constrained tool sampling.                        |
| **Agent Event Projection**      | Eventos de agentes se mapean a objetos profesionales (Findings, Tasks, Receipts), no a tokens de chat. |
| **Estado UNKNOWN**              | Si no sabemos qué pasó, no asumimos éxito. Reconciliación obligatoria.                                 |
| **PI-CI/CD contable**           | Cada cambio ejecuta static checks → accounting tests → fiscal tests → risk tests antes de gate.        |
...
- [FEOS Documentation Index](../00-INDEX.md) — Mapa maestro de docs
