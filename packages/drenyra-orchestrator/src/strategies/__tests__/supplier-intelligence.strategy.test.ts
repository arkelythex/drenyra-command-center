...
describe("supplier intelligence edge cases", () => {
	...
	it("should return empty for null/missing input", () => {
		const strategy = createSupplierIntelligenceStrategy();
		const result = strategy.execute(null, baseContext) as import("../types").Anomaly[];
		expect(result).toHaveLength(0);
	});
	...
	it("should handle single supplier with zero transactions", () => {
		...
		const input: SupplierIntelligenceInput = {
			suppliers: [{ id: "s1", name: "Supplier 1", ruc: "20111111111", createdAt: daysAgo(365) }],
			transactions: [],
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
		expect(result).toHaveLength(0);
	});
	...
	it("should handle transactions with all paid on time", () => {
		...
		const input: SupplierIntelligenceInput = {
			suppliers: [
				{ id: "s1", name: "Good Supplier", ruc: "20111111111", createdAt: daysAgo(365) },
				{ id: "s2", name: "Another Supplier", ruc: "20222222222", createdAt: daysAgo(365) },
			],
			transactions: [
				{
					id: "t1", supplierId: "s1", supplierName: "Good Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "1",
					amount: 1000, currency: "PEN", issueDate: daysAgo(30),
					dueDate: daysAgo(20), paymentDate: daysAgo(20), paid: true,
				},
				{
					id: "t2", supplierId: "s1", supplierName: "Good Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "2",
					amount: 2000, currency: "PEN", issueDate: daysAgo(15),
					dueDate: daysAgo(5), paymentDate: daysAgo(4), paid: true,
				},
				{
					id: "t3", supplierId: "s2", supplierName: "Another Supplier",
					supplierRuc: "20222222222", documentType: "01", serie: "F001", numero: "1",
					amount: 3000, currency: "PEN", issueDate: daysAgo(10),
					dueDate: daysAgo(2), paymentDate: daysAgo(2), paid: true,
				},
			],
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
...
describe("concentration risk detection", () => {
	it("should detect when a single supplier exceeds threshold", () => {
		...
		const input: SupplierIntelligenceInput = {
			suppliers: [
				{ id: "s1", name: "Big Supplier", ruc: "20111111111", createdAt: daysAgo(365) },
				{ id: "s2", name: "Small Supplier", ruc: "20222222222", createdAt: daysAgo(365) },
			],
			transactions: [
				{
					id: "t1", supplierId: "s1", supplierName: "Big Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "1",
					amount: 80000, currency: "PEN", issueDate: daysAgo(10),
					dueDate: daysAgo(5), paymentDate: daysAgo(5), paid: true,
				},
				{
					id: "t2", supplierId: "s1", supplierName: "Big Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "2",
					amount: 20000, currency: "PEN", issueDate: daysAgo(5),
					dueDate: daysFromNow(5), paymentDate: null, paid: false,
				},
				{
					id: "t3", supplierId: "s2", supplierName: "Small Supplier",
					supplierRuc: "20222222222", documentType: "01", serie: "F001", numero: "3",
					amount: 10000, currency: "PEN", issueDate: daysAgo(3),
					dueDate: daysFromNow(7), paymentDate: null, paid: false,
				},
			],
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
		const concAnomalies = result.filter((a) => a.detectionMethod === "supplier_concentration");
	...
	it("should NOT flag when spend is evenly distributed", () => {
		...
		const input: SupplierIntelligenceInput = {
			suppliers: [
				{ id: "s1", name: "Supplier A", ruc: "20111111111", createdAt: daysAgo(365) },
				{ id: "s2", name: "Supplier B", ruc: "20222222222", createdAt: daysAgo(365) },
				{ id: "s3", name: "Supplier C", ruc: "20333333333", createdAt: daysAgo(365) },
			],
			transactions: [
				{
					id: "t1", supplierId: "s1", supplierName: "Supplier A",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "1",
					amount: 30000, currency: "PEN", issueDate: daysAgo(10),
					dueDate: daysAgo(5), paymentDate: daysAgo(5), paid: true,
				},
				{
					id: "t2", supplierId: "s2", supplierName: "Supplier B",
					supplierRuc: "20222222222", documentType: "01", serie: "F001", numero: "2",
					amount: 35000, currency: "PEN", issueDate: daysAgo(8),
					dueDate: daysAgo(3), paymentDate: daysAgo(3), paid: true,
				},
				{
					id: "t3", supplierId: "s3", supplierName: "Supplier C",
					supplierRuc: "20333333333", documentType: "01", serie: "F001", numero: "3",
					amount: 35000, currency: "PEN", issueDate: daysAgo(5),
					dueDate: daysFromNow(5), paymentDate: null, paid: false,
				},
			],
		};
...
describe("payment delay trend detection", () => {
	it("should flag supplier with chronic late payments", () => {
		...
		const input: SupplierIntelligenceInput = {
			suppliers: [{ id: "s1", name: "Late Payer Supplier", ruc: "20111111111", createdAt: daysAgo(365) }],
			transactions: payments,
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
		const delayAnomalies = result.filter((a) => a.detectionMethod === "payment_delay_trend");
	...
	it("should flag when avg delay exceeds threshold across multiple payments", () => {
		...
		const input: SupplierIntelligenceInput = {
			suppliers: [{ id: "s1", name: "Very Late Supplier", ruc: "20111111111", createdAt: daysAgo(365) }],
			transactions: payments,
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
		const delayAnomalies = result.filter((a) => a.detectionMethod === "payment_delay_trend");
	...
	it("should NOT flag suppliers with fewer than 3 payments", () => {
		...
		const input: SupplierIntelligenceInput = {
			suppliers: [{ id: "s1", name: "Few Payments", ruc: "20111111111", createdAt: daysAgo(365) }],
			transactions: payments,
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
		const delayAnomalies = result.filter((a) => a.detectionMethod === "payment_delay_trend");
...
describe("debt aging detection", () => {
	it("should flag supplier with past-due invoices", () => {
		const strategy = createSupplierIntelligenceStrategy();
		const input: SupplierIntelligenceInput = {
			suppliers: [{ id: "s1", name: "Aging Debt Supplier", ruc: "20111111111", createdAt: daysAgo(365) }],
			transactions: [
				{
					id: "t1", supplierId: "s1", supplierName: "Aging Debt Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "1",
					amount: 10000, currency: "PEN", issueDate: daysAgo(150),
					dueDate: daysAgo(100), paymentDate: null, paid: false,
				},
				{
					id: "t2", supplierId: "s1", supplierName: "Aging Debt Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "2",
					amount: 5000, currency: "PEN", issueDate: daysAgo(50),
					dueDate: daysAgo(20), paymentDate: null, paid: false,
				},
			],
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
		const debtAnomalies = result.filter((a) => a.detectionMethod === "debt_aging");
	...
	it("should NOT flag invoices that are not yet due", () => {
		const strategy = createSupplierIntelligenceStrategy();
		const input: SupplierIntelligenceInput = {
			suppliers: [{ id: "s1", name: "Current Supplier", ruc: "20111111111", createdAt: daysAgo(365) }],
			transactions: [
				{
					id: "t1", supplierId: "s1", supplierName: "Current Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "1",
					amount: 5000, currency: "PEN", issueDate: daysAgo(5),
					dueDate: daysFromNow(25), paymentDate: null, paid: false,
				},
			],
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
	...
	it("should NOT flag paid invoices", () => {
		const strategy = createSupplierIntelligenceStrategy();
		const input: SupplierIntelligenceInput = {
			suppliers: [{ id: "s1", name: "Paid Up Supplier", ruc: "20111111111", createdAt: daysAgo(365) }],
			transactions: [
				{
					id: "t1", supplierId: "s1", supplierName: "Paid Up Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "1",
					amount: 5000, currency: "PEN", issueDate: daysAgo(60),
					dueDate: daysAgo(30), paymentDate: daysAgo(28), paid: true,
				},
			],
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
...
describe("duplicate supplier detection", () => {
	it("should detect same RUC under different names", () => {
		...
		const input: SupplierIntelligenceInput = {
			suppliers: [
				{ id: "s1", name: "Proveedor ABC SAC", ruc: "20111111111", createdAt: daysAgo(365) },
				{ id: "s2", name: "ABC Proveedores SAC", ruc: "20111111111", createdAt: daysAgo(180) },
			],
			transactions: [],
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
		const rucAnomalies = result.filter((a) => a.detectionMethod === "duplicate_supplier_ruc");
	...
	it("should detect shared bank account across different RUCs", () => {
		...
		const input: SupplierIntelligenceInput = {
			suppliers: [
				{ id: "s1", name: "Supplier A", ruc: "20111111111", bankAccount: "123-456-789", createdAt: daysAgo(365) },
				{ id: "s2", name: "Supplier B", ruc: "20222222222", bankAccount: "123-456-789", createdAt: daysAgo(365) },
			],
			transactions: [],
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
		const accountAnomalies = result.filter((a) => a.detectionMethod === "shared_bank_account");
	...
	it("should not flag unique RUCs with different names", () => {
		...
		const input: SupplierIntelligenceInput = {
			suppliers: [
				{ id: "s1", name: "Supplier One", ruc: "20111111111", createdAt: daysAgo(365) },
				{ id: "s2", name: "Supplier Two", ruc: "20222222222", createdAt: daysAgo(365) },
			],
			transactions: [],
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
		...
		const input: SupplierIntelligenceInput = {
			...
			transactions: [
				// s1 = 80% of spend (concentration)
				{
					id: "t1", supplierId: "s1", supplierName: "Big Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "1",
					amount: 80000, currency: "PEN", issueDate: daysAgo(20),
					dueDate: daysAgo(5), paymentDate: daysAgo(10), paid: true,
				},
				// s3 new, high value
				{
					id: "t2", supplierId: "s3", supplierName: "New Supplier",
					supplierRuc: "20333333333", documentType: "01", serie: "F001", numero: "1",
					amount: 20000, currency: "PEN", issueDate: daysAgo(2),
					dueDate: daysFromNow(28), paymentDate: null, paid: false,
				},
				// s1 also has late payments (3 payments to meet min threshold)
				{
					id: "t3", supplierId: "s1", supplierName: "Big Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "2",
					amount: 10000, currency: "PEN", issueDate: daysAgo(60),
					dueDate: daysAgo(50), paymentDate: daysAgo(15), paid: true,
				},
				{
					id: "t4", supplierId: "s1", supplierName: "Big Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "3",
					amount: 10000, currency: "PEN", issueDate: daysAgo(90),
					dueDate: daysAgo(80), paymentDate: daysAgo(40), paid: true,
				},
				{
					id: "t5", supplierId: "s1", supplierName: "Big Supplier",
					supplierRuc: "20111111111", documentType: "01", serie: "F001", numero: "4",
					amount: 10000, currency: "PEN", issueDate: daysAgo(120),
					dueDate: daysAgo(110), paymentDate: daysAgo(80), paid: true,
				},
			],
		};
		const result = strategy.execute(input, baseContext) as import("../types").Anomaly[];
