const BLOCKED_PATTERNS = {
    DELETE_LEDGER: /(?:borrar|eliminar|delete).*(?:libro.*mayor|ledger|asientos.*contables)/i,
    ...
    OVERRIDE_FISCAL: /(?:override|modificar|cambiar).*(?:igv|sunat|impuesto|factura.*electronica)/i,
    DISABLE_AUDIT: /(?:desactivar|disable|eliminar).*(?:audit|log|registro.*auditoria)/i,
    MODIFY_TAX_DATA: /(?:modificar|cambiar|editar).*(?:datos.*tributarios|ruc|registros.*sunat)/i,
    EXPORT_SENSITIVE: /(?:exportar|descargar|dump|export).*(?:contraseñas|passwords|credenciales|keys|secretos)/i,
};
...
export function evaluatePrompt(prompt) {
    ...
    for (const [action, pattern] of Object.entries(BLOCKED_PATTERNS)) {
        if (pattern.test(lowerPrompt)) {
            const requiresOverride = ["OVERRIDE_FISCAL", "MODIFY_TAX_DATA"].includes(action);
            return {
                allowed: false,
...
export function evaluateActionType(action) {
    ...
    return {
        ...
        requiresAdmin: upperAction.includes("OVERRIDE") || upperAction.includes("DELETE"),
    };
}
