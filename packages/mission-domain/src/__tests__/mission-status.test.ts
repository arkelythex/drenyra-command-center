...
describe("transition()", () => {
	...
	it("RUNNING -> AWAITING_APPROVAL is valid", () => {
		expect(transition("RUNNING", "AWAITING_APPROVAL")).toBe(
			"AWAITING_APPROVAL",
		);
	});
	...
	it("REJECTED -> REVISION_REQUESTED is valid", () => {
		expect(transition("REJECTED", "REVISION_REQUESTED")).toBe(
			"REVISION_REQUESTED",
		);
	});
	...
	it("COMPLETED -> any throws INVALID_TRANSITION", () => {
		expect(() => transition("COMPLETED", "DRAFT")).toThrow(
			"INVALID_TRANSITION",
		);
	...
	it("REJECTED -> COMPLETED throws INVALID_TRANSITION", () => {
		expect(() => transition("REJECTED", "COMPLETED")).toThrow(
			"INVALID_TRANSITION",
		);
	});

	it("QUEUED -> COMPLETED throws INVALID_TRANSITION", () => {
		expect(() => transition("QUEUED", "COMPLETED")).toThrow(
			"INVALID_TRANSITION",
		);
	});

	it("AWAITING_APPROVAL -> COMPLETED throws INVALID_TRANSITION", () => {
		expect(() => transition("AWAITING_APPROVAL", "COMPLETED")).toThrow(
			"INVALID_TRANSITION",
		);
	});
...
describe("isRunnable()", () => {
	...
	it("BLOCKED is NOT runnable", () =>
		expect(isRunnable("BLOCKED")).toBe(false));
	it("AWAITING_APPROVAL is NOT runnable", () =>
		expect(isRunnable("AWAITING_APPROVAL")).toBe(false));
	it("APPROVED is NOT runnable", () =>
		expect(isRunnable("APPROVED")).toBe(false));
