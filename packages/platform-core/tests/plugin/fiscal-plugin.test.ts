...
describe("FiscalPlugin", () => {
      ...
      policies: Array<{
        ...
        policy: {
          ...
          evaluate: (context: {
            action: string;
            agentType: string;
            task: { id: string; type: string; priority: string; input: Record<string, unknown> };
  ...
  it("evaluates the sunat-readonly policy correctly", () => {
    ...
    const queryResult = sunatPolicy.policy.evaluate({
      ...
      task: { id: "t1", type: "fiscal-compliance", priority: "medium", input: {} },
    ...
    const modifyResult = sunatPolicy.policy.evaluate({
      ...
      task: { id: "t2", type: "fiscal-sunat-filing", priority: "high", input: {} },
    ...
    const deleteResult = sunatPolicy.policy.evaluate({
      ...
      agentType: "fiscal-sunat-filing",
      task: { id: "t3", type: "fiscal-sunat-filing", priority: "high", input: {} },
    }) as { allowed: boolean; reason?: string };
