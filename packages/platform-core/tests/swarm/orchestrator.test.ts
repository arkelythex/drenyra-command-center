...
describe("Orchestrator", () => {
  ...
  describe("execute single task", () => {
    ...
    it("returns a failed TaskResult when no agent matches", async () => {
      ...
      expect(result.error).toContain("No agent registered for type: unknown-type");
    });
  });
  ...
  describe("executeParallel", () => {
    it("executes a task across multiple agents in parallel", async () => {
      ...
      const results = await orchestrator.executeParallel(task, ["agent-a", "agent-b"], "all-results");
    ...
    it("returns empty array when no agents are specified", async () => {
      ...
      const results = await orchestrator.executeParallel(task, [], "all-results");
