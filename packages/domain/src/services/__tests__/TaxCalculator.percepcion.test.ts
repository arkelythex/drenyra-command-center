...
describe("TaxCalculator — Percepción IGV", () => {
  ...
  it("shouldApplyPercepcion returns true for >= S/ 700 in PEN", () => {
    expect(TaxCalculator.shouldApplyPercepcion(Money.fromAmount(700, "PEN"))).toBe(true);
    expect(TaxCalculator.shouldApplyPercepcion(Money.fromAmount(1000, "PEN"))).toBe(true);
  });

  it("shouldApplyPercepcion returns false for < S/ 700 or wrong currency", () => {
    expect(TaxCalculator.shouldApplyPercepcion(Money.fromAmount(699.99, "PEN"))).toBe(false);
    expect(TaxCalculator.shouldApplyPercepcion(Money.fromAmount(1000, "USD"))).toBe(false);
    expect(TaxCalculator.shouldApplyPercepcion(Money.fromAmount(0, "PEN"))).toBe(false);
  });
