const redactionPatterns: readonly {
	...
	{ label: "token", pattern: /\b(?:token|secret|api[_-]?key|password)\s*[:=]\s*[A-Za-z0-9._-]{8,}\b/gi },
	{ label: "accountNumber", pattern: /\b(?:account(?:Number)?|cuenta)\s*[:=]?\s*[0-9 -]{10,24}\b|\b[0-9 -]{14,24}\b/gi },
...
function redactString(value: string): string {
	return redactionPatterns.reduce(
		(current, item) => current.replace(item.pattern, `[REDACTED_${item.label}]`),
		value,
	);
}
