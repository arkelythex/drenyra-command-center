...
export class FinancialReportsService {
    ...
    constructor() {
        this.balanceDataSource = this.requireDependency(SERVICE_TOKENS.BALANCE_REPORT_DATA_SOURCE, "BALANCE_REPORT_DATA_SOURCE");
        this.organizationDataSource = this.requireDependency(SERVICE_TOKENS.ORGANIZATION_REPORT_DATA_SOURCE, "ORGANIZATION_REPORT_DATA_SOURCE");
        this.ledgerDataSource = inject(SERVICE_TOKENS.LEDGER_REPORT_DATA_SOURCE);
        this.openingBalanceDataSource = inject(SERVICE_TOKENS.OPENING_BALANCE_DATA_SOURCE);
    }
    ...
    async generateTrialBalance(organizationId, period) {
        const [orgInfo, accountData] = await Promise.all([
            ...
            this.balanceDataSource.getAccountBalances(organizationId, period.startDate, period.endDate),
        ]);
    ...
    async generateBalanceSheet(organizationId, period) {
        ...
        const totalAssets = [...currentAssets, ...nonCurrentAssets].reduce((sum, acc) => sum + acc.balance, 0);
        const totalLiabilities = [
            ...currentLiabilities,
            ...nonCurrentLiabilities,
        ].reduce((sum, acc) => sum + Math.abs(acc.balance), 0);
        const totalEquity = equityItems.reduce((sum, acc) => sum + Math.abs(acc.balance), 0);
    ...
    async generateIncomeStatement(organizationId, period) {
        ...
        const sumBalances = (items) => items.reduce((sum, acc) => sum + Math.abs(acc.balance), 0);
        const totalRevenue = sumBalances(operatingRevenue) + sumBalances(otherRevenue);
        const totalCostOfSales = sumBalances(costOfSales);
        const grossProfit = totalRevenue - totalCostOfSales;
        const totalOperatingExpenses = sumBalances(administrativeExpenses) + sumBalances(sellingExpenses);
        const operatingIncome = grossProfit - totalOperatingExpenses;
        const totalOtherIncomeExpense = sumBalances(financialIncome) - sumBalances(financialExpense);
    ...
    async generateGeneralLedger(organizationId, period, accountCodes) {
        const orgInfo = await this.organizationDataSource.getOrganizationInfo(organizationId);
        const accountData = await this.balanceDataSource.getAccountBalances(organizationId, period.startDate, period.endDate);
        ...
        for (const account of accountsToProcess) {
            const openingBalance = (await this.openingBalanceDataSource?.getOpeningBalance(organizationId, account.accountCode, period.startDate)) ?? 0;
            const entries = (await this.ledgerDataSource?.getLedgerEntries(organizationId, account.accountCode, period.startDate, period.endDate)) ?? [];
        ...
        return {
            ...
            organizationName: orgInfo.name,
            ruc: orgInfo.ruc,
            accounts: ledgerAccounts.sort((a, b) => a.accountCode.localeCompare(b.accountCode)),
        };
    }
}
