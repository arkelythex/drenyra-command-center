...
export interface IInvoiceOCRService {
    extractInvoiceData(options: OCRInvoiceExtractionOptions): Promise<OCRInvoiceExtractionResult>;
}
...
export interface IExpenseClassifier {
    classifyExpense(input: ExpenseClassificationInput): Promise<ExpenseClassificationResult | null>;
...
export interface IDocumentSyncProcessor {
    processDocumentSync(payload: DocumentProcessingPayload): Promise<DocumentProcessingResult>;
}
