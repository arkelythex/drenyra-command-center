function sameScope(entityScope, scope) {
    return (entityScope.companyId === scope.companyId &&
...
export class InMemoryDrenyraRepository {
    ...
    async listScopedAuditEvents(scope, filters = {}) {
        ...
        const events = byNewest([...this.auditEvents.values()].filter((event) => {
    ...
    async listCommandAuditEvents(scope, filter = {}) {
        return byNewest([...this.auditEvents.values()].filter((event) => sameScope(event.scope, scope) &&
