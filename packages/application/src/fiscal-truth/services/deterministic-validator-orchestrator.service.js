...
export class DeterministicValidatorOrchestratorService {
    ...
    async execute(input) {
        const results = await Promise.all(this.validators.map((validator) => validator.validate(input.validatorPayload)));
        const hasBlockingFailures = results.some((result) => !result.isValid || result.severity === "blocking");
        return {
            results,
