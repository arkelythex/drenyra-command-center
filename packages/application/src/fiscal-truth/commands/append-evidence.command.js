...
export class AppendEvidenceCommand {
    ...
    async execute(input) {
        if (!isFiscalTruthScope(input.expectedScope) ||
            !isFiscalTruthScope(input.evidence.scope)) {
            throw new Error("Invalid fiscal truth scope for evidence append.");
        }
        if (input.expectedScope.companyId !== input.evidence.scope.companyId ||
        ...
        if (input.policyDecision) {
            ...
            const policyNode = {
                ...
                metadata: {
                    ...
                    governanceBundleId: input.policyDecision.governance.governanceBundleId,
