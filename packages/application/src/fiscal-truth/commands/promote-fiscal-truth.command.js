...
export class PromoteFiscalTruthCommand {
    ...
    async execute(input) {
        ...
        if (typeof evidenceNodeId !== "string" ||
        ...
        if (this.deps.findEvidenceNodeById) {
            const persistedRoot = await this.deps.findEvidenceNodeById(input.event.evidenceRootNodeId, input.event.scope);
            if (!persistedRoot) {
                throw new Error("Evidence root was not persisted for authoritative promotion.");
            }
            const guardPasses = canPromoteAuthoritativeTruth({
                evidenceNodeKind: persistedRoot.nodeKind,
                hasDeterministicValidation: input.validatorResults.length > 0 &&
                    input.validatorResults.some((r) => r.validatorVersion === input.event.validatorSetVersion),
                ...
                hasApprovedGovernancePolicyDecision: input.policyDecision.outcome === POLICY_OUTCOME.PROMOTABLE,
                hasHumanMaterialApproval: !!input.event.approvalId,
            });
            if (!guardPasses) {
                throw new Error("Canonical promotion guard rejected authoritative truth promotion.");
            }
        }
        if (input.validatorResults.length === 0) {
            throw new Error("Missing validator results for authoritative promotion.");
        }
        const hasMatchingValidatorVersion = input.validatorResults.some((result) => result.validatorVersion === input.event.validatorSetVersion);
        ...
        if (input.hasRequiredApproval && !input.event.approvalId?.trim()) {
            throw new Error("Missing approval dependency for authoritative promotion.");
        }
        if (input.policyDecision.outcome !== POLICY_OUTCOME.PROMOTABLE ||
            !input.hasRequiredApproval) {
            throw new Error("Policy outcome does not allow authoritative promotion.");
        }
        const hasBlockingValidation = input.validatorResults.some((result) => !result.isValid || result.severity === "blocking");
        if (hasBlockingValidation) {
            throw new Error("Deterministic validation failed for authoritative promotion.");
        }
        await this.deps.append(input.event);
    }
}
//# sourceMappingURL=promote-fiscal-truth.command.js.map