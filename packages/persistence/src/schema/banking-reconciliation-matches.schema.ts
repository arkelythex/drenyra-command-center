/**
 * Transaction Reconciliation Matches Schema
 * Tracks auto-reconciliation matching history and decisions
 */

import { pgEnum, pgTable, uuid, varchar, integer, timestamp, boolean, index } from 'drizzle-orm/pg-core';
import { relations } from 'drizzle-orm';
import { bankTransactions } from './banking.schema';

/**
 * transactionReconciliationMatches const.
 *
 * @example
 * ```ts
 * console.log(transactionReconciliationMatches);
 * ```
 */
export const transactionReconciliationMatches = pgTable('transaction_reconciliation_matches', {
...
/**
 * partialPaymentTransactions const.
 *
 * @example
 * ```ts
 * console.log(partialPaymentTransactions);
 * ```
 */
export const partialPaymentTransactions = pgTable('partial_payment_transactions', {
  id: uuid('id').primaryKey().defaultRandom(),
...
/**
 * reconciliationShadowRunStatusEnum const.
 *
 * @example
 * ```ts
 * console.log(reconciliationShadowRunStatusEnum);
 * ```
 */
export const reconciliationShadowRunStatusEnum = pgEnum('reconciliation_shadow_run_status', [
  'SUCCESS',
  'FAILED',
]);

/**
 * reconciliationShadowRuns const.
 *
 * @example
 * ```ts
 * console.log(reconciliationShadowRuns);
 * ```
 */
export const reconciliationShadowRuns = pgTable(
  ...
  (table) => ({
    companyIdx: index('reconciliation_shadow_runs_company_idx').on(table.companyId),
    companyCreatedIdx: index('reconciliation_shadow_runs_company_created_idx').on(
      table.companyId,
      table.createdAt
    ),
  })
...
/**
 * transactionReconciliationMatchesRelations const.
 *
 * @example
 * ```ts
 * console.log(transactionReconciliationMatchesRelations);
 * ```
 */
...
/**
 * partialPaymentsRelations const.
 *
 * @example
 * ```ts
 * console.log(partialPaymentsRelations);
 * ```
 */
export const partialPaymentsRelations = relations(partialPayments, ({ many }) => ({
  transactions: many(partialPaymentTransactions),
}));

/**
 * partialPaymentTransactionsRelations const.
 *
 * @example
 * ```ts
 * console.log(partialPaymentTransactionsRelations);
 * ```
 */
