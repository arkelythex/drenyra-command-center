import { pgTable, uuid, varchar, decimal, integer, boolean, timestamp, text, index, foreignKey, } from 'drizzle-orm/pg-core';
...
export const interCompanyTransactions = pgTable('inter_company_transactions', {
    ...
    groupProfileIdx: index('inter_company_transactions_group_profile_idx').on(table.economicGroupId, table.detractionProfile),
    groupRuleIdx: index('inter_company_transactions_group_rule_idx').on(table.economicGroupId, table.detractionRuleCode),
    groupCreatedAtIdx: index('inter_company_transactions_group_created_at_idx').on(table.economicGroupId, table.createdAt),
}));
