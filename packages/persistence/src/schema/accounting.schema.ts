 ...
 * Accounting Schema
 *
 * Core accounting tables: PCGE chart of accounts, journal entries,
 * accounting periods, exchange rates, CPE logs, and detractions.
 *
 * Uses varchar().$type<>() pattern for enum-like columns instead of pgEnum
 * to avoid DB enum migration overhead.
 */
...
// --- EXCHANGE RATES ---
// Rates stored as integer with 4 decimal places (e.g., 37250 = 3.7250)
...
// --- CPE LOG (SUNAT Communication Log) ---
...
export const detractions = pgTable(
	"detractions",
	{
		...
		status: varchar("status", { length: 20 })
			.$type<"pendiente" | "depositado" | "usado" | "liberado">()
			.default("pendiente")
			.notNull(),
...
export const journalEntries = pgTable(
	"journal_entries",
	{
		...
		status: varchar("status", { length: 20 })
			.$type<"borrador" | "mayorizado" | "declarado">()
			.default("borrador")
			.notNull(),
