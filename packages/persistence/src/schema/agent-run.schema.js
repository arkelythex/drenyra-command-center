import { index, jsonb, pgTable, serial, text, timestamp, uniqueIndex, uuid } from "drizzle-orm/pg-core";
...
export const agentRunStates = pgTable("agent_run_states", {
    ...
    companyStatusIdx: index("agent_run_states_company_status_idx").on(table.companyId, table.status),
    companyCreatedIdx: index("agent_run_states_company_created_idx").on(table.companyId, table.createdAt),
...
export const agentRunEvents = pgTable("agent_run_events", {
    ...
    companyRunIdx: index("agent_run_events_company_run_idx").on(table.companyId, table.runId),
}));
