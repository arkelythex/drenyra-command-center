...
export const accountingJobRuns = pgTable("accounting_job_runs", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id")
    ...
    companyStatusIdx: index("accounting_job_runs_company_status_idx").on(table.companyId, table.status),
    jobLookupIdx: index("accounting_job_runs_job_lookup_idx").on(table.companyId, table.countryCode, table.jobId),
    createdAtIdx: index("accounting_job_runs_created_at_idx").on(table.createdAt),
}));
