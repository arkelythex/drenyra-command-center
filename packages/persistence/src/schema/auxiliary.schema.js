...
import { boolean, customType, date, decimal, index, integer, jsonb, pgEnum, pgTable, text, timestamp, varchar, } from "drizzle-orm/pg-core";
...
export const aiCostEvents = pgTable("ai_cost_events", {
...
export const sunatKnowledgeChunks = pgTable("sunat_knowledge_chunks", {
    id: text("id").primaryKey(),
    ...
    categoryDateIdx: index("sunat_knowledge_category_date_idx").on(table.category, table.effectiveDate.desc().nullsLast()),
    ftsGinIdx: index("sunat_knowledge_fts_gin_idx").using("gin", table.searchVector),
    sourceIdx: index("sunat_knowledge_source_idx").on(table.source),
}));
//# sourceMappingURL=auxiliary.schema.js.map