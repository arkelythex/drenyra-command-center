...
const transactionIdToUuid = (id) => toStableUuid(`legacy-bank-transaction:${id}`);
...
export class PostgresBankTransactionRepository {
    ...
    async getSumByType(bankAccountId, dateFrom, dateTo) {
        ...
        const results = await db
            .select({ type: bankTransactions.type, total: sql `SUM(CAST(${bankTransactions.amount} AS DECIMAL))` })
            .from(bankTransactions)
            .where(and(...conditions))
            .groupBy(bankTransactions.type);
        const sums = { DEPOSIT: 0, WITHDRAWAL: 0, TRANSFER_IN: 0, TRANSFER_OUT: 0, FEE: 0, INTEREST: 0, CHECK: 0, OTHER: 0 };
    ...
    buildConditions(accountId, filters) {
        ...
        if (filters?.type)
            conditions.push(eq(bankTransactions.type, mapDomainTypeToDb(filters.type)));
        if (filters?.dateFrom)
            conditions.push(gte(bankTransactions.transactionDate, toDateString(filters.dateFrom)));
        if (filters?.dateTo)
            conditions.push(lte(bankTransactions.transactionDate, toDateString(filters.dateTo)));
        if (filters?.importBatch)
            conditions.push(eq(bankTransactions.importedFrom, filters.importBatch));
        if (filters?.minAmount !== undefined)
            conditions.push(gte(bankTransactions.amount, toDecimal(filters.minAmount)));
        if (filters?.maxAmount !== undefined)
            conditions.push(lte(bankTransactions.amount, toDecimal(filters.maxAmount)));
        return conditions;
    }
    mapToDomain(raw) {
        const currency = "PEN";
