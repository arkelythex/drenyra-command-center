...
import { buildSyntheticEntries, mapDbStatusToDomain, mapDbToDomainType, mapDomainStatusToDb, mapDomainTypeToDb, mapDomainTypeToDocumentType, resolveReferenceParts, } from "./mappers";
export class PostgresTransactionRepository {
    ...
    async update(transaction, organizationId) {
        ...
        await db
            ...
            .where(and(eq(transactions.id, persistedId), eq(transactions.companyId, companyId)));
    }
    async delete(id, organizationId) {
        ...
        await db
            .delete(transactions)
            .where(and(eq(transactions.id, toStableUuid(id)), eq(transactions.companyId, companyId)));
    }
    ...
    async findByReferenceNumber(referenceNumber, organizationId) {
        ...
        const rows = await db
            .select()
            .from(transactions)
            .where(and(eq(transactions.companyId, companyId), eq(transactions.series, series), eq(transactions.number, number)))
            .limit(1);
        if (rows.length === 0)
            return null;
        return this.mapToDomain(rows[0]);
    ...
    async findByAccount(_accountCode, _organizationId, _dateFrom, _dateTo) {
        console.warn("findByAccount: Journal entries integration required for full implementation");
        return [];
    }
    ...
    async getNextReferenceNumber(organizationId, type) {
        ...
        const rows = await db
            ...
            .from(transactions)
            .where(and(eq(transactions.companyId, companyId), like(transactions.series, `${prefix}%`)))
    ...
    buildFilterConditions(companyId, filters) {
        ...
        if (filters?.referenceNumber) {
            ...
            if (seriesSearch && numberSearch) {
                conditions.push(and(like(transactions.series, `%${seriesSearch}%`), like(transactions.number, `%${numberSearch}%`)));
            }
            else {
                conditions.push(or(like(transactions.series, `%${filters.referenceNumber}%`), like(transactions.number, `%${filters.referenceNumber}%`)));
            }
        }
        if (filters?.minAmount !== undefined) {
            conditions.push(gte(transactions.totalAmount, filters.minAmount.toFixed(2)));
        }
        if (filters?.maxAmount !== undefined) {
            conditions.push(lte(transactions.totalAmount, filters.maxAmount.toFixed(2)));
        }
        return conditions;
    }
    mapToDomain(raw) {
        ...
        const referenceNumber = raw.series && raw.number ? `${raw.series}-${raw.number}` : undefined;
