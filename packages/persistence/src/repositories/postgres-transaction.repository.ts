 ...
 * - Persistence now uses the modular `transactions` table
...
const mapDomainTypeToDb = (domainType: TransactionType): DbTransactionType => {
	switch (domainType) {
		case "PURCHASE":
		case "PAYMENT":
			return "EXPENSE";
...
const mapDomainTypeToDocumentType = (
	...
	private mapToDomain(raw: typeof transactions.$inferSelect): Transaction {
		...
		return Transaction.create({
			...
			type: mapDbToDomainType(
				raw.type as DbTransactionType,
				raw.documentType as DbDocumentType,
			),
			date: raw.issueDate,
			description: raw.notes || referenceNumber || "Movimiento fiscal",
			referenceNumber,
			entries: buildSyntheticEntries(raw, totalAmount),
			status: mapDbStatusToDomain(raw.status as DbTransactionStatus | null),
