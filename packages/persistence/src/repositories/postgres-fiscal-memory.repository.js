...
export class PostgresFiscalMemoryRepository {
    ...
    async findByEvidenceRef(scope, evidenceRef) {
        return this.findMany(and(scopedWhere(scope), sql `${fiscalMemories.evidenceRefs} @> ${JSON.stringify([evidenceRef])}::jsonb`));
    }
    async findRelated(scope, memoryId) {
        return this.findMany(and(scopedWhere(scope), or(eq(fiscalMemories.id, memoryId), sql `${fiscalMemories.relatedMemoryIds} @> ${JSON.stringify([memoryId])}::jsonb`)));
    }
