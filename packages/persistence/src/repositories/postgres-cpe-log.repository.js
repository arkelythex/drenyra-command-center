...
export class PostgresCpeLogRepository {
    ...
    async findByCompanyAndPeriod(companyId, year, month) {
        ...
        const result = await db
            .select()
            .from(cpeLog)
            .where(and(eq(cpeLog.companyId, companyId), between(cpeLog.createdAt, startDate, endDate)))
            .orderBy(sql `${cpeLog.createdAt} DESC`);
    ...
    async findByStatus(companyId, status) {
        const result = await db
            .select()
            .from(cpeLog)
            .where(and(eq(cpeLog.companyId, companyId), eq(cpeLog.sunatStatus, status)))
            .orderBy(sql `${cpeLog.createdAt} DESC`);
        return result.map((row) => this.mapToDomain(row));
    }
    ...
    async verifyHash(id, xmlHash) {
        const result = await db
            ...
            .where(eq(cpeLog.id, id))
            .limit(1);
        if (!result[0] || !result[0].hashValue)
            return false;
