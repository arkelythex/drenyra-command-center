...
export class PostgresAccountRepository {
	...
	async findAll(organizationId) {
		...
		const rows = await db.query.pcgeAccounts.findMany({
			where: eq(pcgeAccounts.companyId, companyId),
	...
	async findWithFilters(filters) {
		...
		return Promise.all(
			rows.map((row) => this.mapToDomain(row, filters.organizationId)),
		);
	}
	...
	async findMovementAccounts(organizationId) {
		const accounts = await this.findWithFilters({
			organizationId,
			status: "active",
		});
	...
	buildConditions(companyId, filters) {
		...
		if (filters.status && filters.status !== "all")
			conditions.push(
				eq(pcgeAccounts.isActive, filters.status === "active" ? "S" : "N"),
			);
		...
		if (filters.searchTerm) {
			...
			const searchCondition = or(
				like(pcgeAccounts.code, search),
				like(pcgeAccounts.name, search),
			);
	...
	async mapToDomain(raw, organizationId) {
		...
		return Account.create({
			...
			name: raw.name,
			level:
				levelFromCode(raw.code) === level ? level : levelFromCode(raw.code),
			type: raw.type,
