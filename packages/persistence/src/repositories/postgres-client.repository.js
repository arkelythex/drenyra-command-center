...
export class PostgresClientRepository {
    async save(data) {
        const now = new Date();
        const companyId = await resolveCompanyIdFromOrganization(data.organizationId);
        const id = randomUUID();
    ...
    async update(id, data) {
        ...
        const updated = await db.transaction(async (tx) => {
            const [partner] = await tx
                ...
                partnerDocumentType: data.documentType ?? existing.partner.partnerDocumentType,
    ...
    async findAll(organizationId, filters) {
        ...
        if (filters?.documentType) {
            conditions.push(eq(businessPartners.partnerDocumentType, filters.documentType));
        }
        ...
        return rows.map((row) => this.mapToClient(row.partner, row.profile, organizationId));
    }
    ...
    async findByDocumentNumber(organizationId, documentNumber) {
        const companyId = await resolveCompanyIdFromOrganization(organizationId);
        const rows = await db
            .select({ partner: businessPartners, profile: customerProfiles })
            .from(customerProfiles)
            .innerJoin(businessPartners, eq(customerProfiles.id, businessPartners.id))
            .where(and(eq(businessPartners.companyId, companyId), eq(businessPartners.taxId, documentNumber)))
            .limit(1);
        if (rows.length === 0)
            return null;
        return this.mapToClient(rows[0].partner, rows[0].profile, organizationId);
    }
    async findClientContext(id) {
        const rows = await db
            .select({ partner: businessPartners, profile: customerProfiles })
            .from(customerProfiles)
            .innerJoin(businessPartners, eq(customerProfiles.id, businessPartners.id))
        ...
        if (rows.length === 0)
            return null;
        const organizationId = await resolveOrganizationIdFromCompany(rows[0].partner.companyId);
        return {
    ...
    mapToClient(partner, profile, organizationId) {
        return {
            id: partner.id,
            organizationId,
            name: partner.legalName,
            documentType: (partner.partnerDocumentType ?? "RUC"),
