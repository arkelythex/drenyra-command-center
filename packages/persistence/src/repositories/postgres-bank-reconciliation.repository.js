...
export class PostgresBankReconciliationRepository {
    ...
    buildConditions(companyId, filters) {
        ...
        if (filters?.bankAccountId)
            conditions.push(eq(bankReconciliations.accountId, accountIdToUuid(filters.bankAccountId)));
        if (filters?.status)
            conditions.push(eq(bankReconciliations.status, mapDomainStatusToDb(filters.status)));
        if (filters?.periodFrom)
            conditions.push(gte(bankReconciliations.startDate, toDateString(filters.periodFrom)));
        if (filters?.periodTo)
            conditions.push(lte(bankReconciliations.endDate, toDateString(filters.periodTo)));
        return conditions;
    }
