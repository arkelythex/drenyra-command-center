...
export class PostgresExchangeRateRepository {
    async save(rate, companyId) {
        ...
        await db.insert(exchangeRates).values({
            ...
            sunatReference: rate.sunatReference !== null
    ...
    async findByDateAndCurrency(companyId, date, currencyFrom, currencyTo) {
        ...
        const result = await db
            .select()
            .from(exchangeRates)
            .where(and(eq(exchangeRates.companyId, companyId), gte(exchangeRates.date, start), lte(exchangeRates.date, end), eq(exchangeRates.currencyFrom, currencyFrom), eq(exchangeRates.currencyTo, currencyTo)))
            .limit(1);
        if (!result[0])
            return null;
        return this.mapToDomain(result[0]);
    }
    async findByDateRange(companyId, startDate, endDate, currencyFrom, currencyTo) {
        const result = await db
            .select()
            .from(exchangeRates)
            .where(and(eq(exchangeRates.companyId, companyId), gte(exchangeRates.date, startDate), lte(exchangeRates.date, endDate), eq(exchangeRates.currencyFrom, currencyFrom), eq(exchangeRates.currencyTo, currencyTo)))
    ...
    async findLatestBefore(companyId, date, currencyFrom, currencyTo) {
        const result = await db
            .select()
            .from(exchangeRates)
            .where(and(eq(exchangeRates.companyId, companyId), lte(exchangeRates.date, date), eq(exchangeRates.currencyFrom, currencyFrom), eq(exchangeRates.currencyTo, currencyTo)))
            .orderBy(desc(exchangeRates.date))
            .limit(1);
        if (!result[0])
            return null;
    ...
    mapToDomain(raw) {
        return ExchangeRate.fromJSON({
            ...
            sunatReference: raw.sunatReference !== null
