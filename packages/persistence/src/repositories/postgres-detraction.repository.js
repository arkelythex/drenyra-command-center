...
export class PostgresDetractionRepository {
    ...
    async findByCompanyAndPeriod(companyId, year, month) {
        ...
        const result = await db
            .select()
            .from(detractions)
            .where(and(eq(detractions.companyId, companyId), between(detractions.createdAt, startDate, endDate)))
            .orderBy(sql `${detractions.createdAt} DESC`);
        return result.map((row) => this.mapToDomain(row));
    }
    async findByStatus(companyId, status) {
        const result = await db
            .select()
            .from(detractions)
            .where(and(eq(detractions.companyId, companyId), eq(detractions.status, status)))
            .orderBy(sql `${detractions.createdAt} DESC`);
        return result.map((row) => this.mapToDomain(row));
    }
    async findPendingByCompany(companyId) {
        const result = await db
            .select()
            .from(detractions)
            .where(and(eq(detractions.companyId, companyId), eq(detractions.status, "pendiente")))
