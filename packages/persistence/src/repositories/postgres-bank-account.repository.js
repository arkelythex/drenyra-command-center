...
export class PostgresBankAccountRepository {
    ...
    async getTotalBalanceByCurrency(organizationId) {
        ...
        const results = await db
            .select({ currency: bankAccounts.currency, total: sql `SUM(CAST(${bankAccounts.currentBalance} AS DECIMAL))` })
            .from(bankAccounts)
    ...
    buildConditions(companyId, filters) {
        ...
        if (filters?.accountType)
            conditions.push(eq(bankAccounts.accountType, mapDomainTypeToDb(filters.accountType)));
        if (filters?.bankName)
