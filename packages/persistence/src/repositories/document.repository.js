...
export class DocumentRepositoryImpl {
    ...
    async save(_document) {
        throw new DatabaseError("DocumentRepository.save requires tenant context. Use saveForCompany(document, companyId).");
    }
    async saveForCompany(document, companyId) {
        return this.persistDocument(document, companyId);
    }
    async update(_document) {
        throw new DatabaseError("DocumentRepository.update requires tenant context. Use updateForCompany(document, companyId).");
    }
    ...
    async persistDocument(document, companyId) {
        try {
            ...
            throw new DatabaseError(`Failed to save document: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    async findById(_id) {
        throw new DatabaseError("DocumentRepository.findById requires tenant context. Use findByIdForCompany(id, companyId).");
    }
    ...
    async findDocumentByIdForCompany(id, companyId) {
        try {
            ...
            throw new DatabaseError(`Failed to find document: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    async findAll(filters) {
        try {
            ...
            throw new DatabaseError(`Failed to find documents: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    async count(filters) {
        try {
            ...
            throw new DatabaseError(`Failed to count documents: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    ...
    async buildCompanyScopedConditions(companyId, filtersOrConditions) {
        const legacyOrganizationId = await this.resolveLegacyOrganizationId(companyId);
