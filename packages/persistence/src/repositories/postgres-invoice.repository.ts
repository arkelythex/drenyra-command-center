 ...
 *
 * Transitional note:
 * - The live database now exposes only the modular `invoices` schema.
 * - Tenant-aware writes (`saveForOrganization` / `updateForOrganization`) are the
 *   safe path because modular persistence requires tenant context.
 * - Legacy write methods fail fast to avoid issuing SQL against a schema shape
...
interface ModularInvoiceWithRelations {
	invoice: typeof invoices.$inferSelect;
	customer: typeof businessPartners.$inferSelect;
	items: Array<typeof invoiceItems.$inferSelect>;
}

type NormalizedInvoiceFilters = Omit<InvoiceFilters, "startDate" | "endDate"> & {
	startDate?: Date;
	endDate?: Date;
};
...
/**
 * PostgresInvoiceRepository class.
 *
 * @example
 * ```ts
 * const value = new PostgresInvoiceRepository();
 * console.log(value);
 * ```
 */
export class PostgresInvoiceRepository implements InvoiceRepository {
	async save(_invoice: Invoice): Promise<void> {
		throw new Error(
			"InvoiceRepository.save requires tenant context. Use saveForOrganization(invoice, organizationId).",
		);
	}
	...
	async update(_invoice: Invoice): Promise<void> {
		throw new Error(
			"InvoiceRepository.update requires tenant context. Use updateForOrganization(invoice, organizationId).",
		);
	}
			...
			const existingPartnerRows = await tx
				...
				.where(
					and(
						...
						eq(businessPartners.taxId, partnerIdentity.taxId),
					),
				)
				.limit(1);
	...
	private async findAllModular(
		filters: ReturnType<PostgresInvoiceRepository["normalizeFilters"]>,
	): Promise<Invoice[]> {
		...
		for (const row of rows) {
			...
			if (filters.clientSearch) {
				...
				const matchesSearch =
					row.customer.legalName.toLowerCase().includes(search) ||
	...
	private mapModularToDomain(raw: ModularInvoiceWithRelations): Invoice {
		...
		const items: InvoiceItem[] = raw.items.map((item) => ({
			id: item.id,
		...
		return Invoice.create({
			...
			clientRUC:
				customerDocumentType === "RUC" ? RUC.create(customerTaxId) : undefined,
			clientDNI:
				customerDocumentType === "DNI" ? DNI.create(customerTaxId) : undefined,
			...
			status: mapModularStatusToInvoiceStatus(
				normalizeInvoiceReadStatus(raw.invoice.status),
				normalizeSunatReadStatus(raw.invoice.sunatStatus),
			),
			items,
			notes: raw.invoice.notes ?? undefined,
			sentToSunatAt:
				raw.invoice.status === "SENT" ? raw.invoice.updatedAt : undefined,
