...
export class PostgresProviderRepository {
    async save(data) {
        ...
        return this.mapToProvider(created.partner, created.profile, data.organizationId);
    }
    async update(id, data) {
        ...
        const updated = await db.transaction(async (tx) => {
            ...
            const [profile] = await tx
                ...
                .set({
                paymentTermDays: data.paymentTerms ?? existing.profile.paymentTermDays,
                updatedAt: now,
            })
        ...
        return this.mapToProvider(updated.partner, updated.profile, existing.organizationId);
    }
    async delete(id) {
    ...
    async findById(id) {
        ...
        if (!context)
            return null;
        return this.mapToProvider(context.partner, context.profile, context.organizationId);
    }
    async findAll(organizationId, filters) {
        ...
        return rows.map((row) => this.mapToProvider(row.partner, row.profile, organizationId));
    }
    ...
    async findByRUC(organizationId, ruc) {
        ...
        const rows = await db
            ...
            .from(vendorProfiles)
            .innerJoin(businessPartners, eq(vendorProfiles.id, businessPartners.id))
            .where(and(eq(businessPartners.companyId, companyId), eq(businessPartners.taxId, ruc)))
            .limit(1);
        if (rows.length === 0)
            return null;
