/**
 * PR 3.2 — TransactionRepository cross-tenant integration tests
 *
 * Verifies that findById(scope, transactionId) enforces tenant isolation
 * at SQL level using the companyId column.
 *
 * Requires DATABASE_URL_TEST environment variable.
 *
 * @module h02-pr3.2-transaction-repository
 */
...
const TRANSACTION_IN_A1 = "50000000-0000-4000-8000-000000000001";
...
const repo = new PostgresTransactionRepository();

describe("TransactionRepository.findById — cross-tenant isolation", () => {
	it("finds a transaction in the selected company", async () => {
		const result = await repo.findById(scopeA1, TRANSACTION_IN_A1);
		expect(result).not.toBeNull();
		expect(result?.id).toBe(TRANSACTION_IN_A1);
	});

	it("returns null for another company in the same organization", async () => {
		const result = await repo.findById(scopeA2, TRANSACTION_IN_A1);
		expect(result).toBeNull();
	});

	it("returns null for another organization", async () => {
		const result = await repo.findById(scopeB1, TRANSACTION_IN_A1);
		expect(result).toBeNull();
	});
	...
	it("does not distinguish foreign transaction from nonexistent transaction", async () => {
		const foreignResult = await repo.findById(scopeA2, TRANSACTION_IN_A1);
