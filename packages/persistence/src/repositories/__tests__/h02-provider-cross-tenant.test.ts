/**
 * PR 3.4 — ProviderRepository cross-tenant integration tests
 *
 * Verifies that findById(scope, providerId) enforces tenant isolation
 * at SQL level using the companyId column on business_partners.
 *
 * Requires DATABASE_URL_TEST environment variable.
 *
 * @module h02-pr3.4-provider-repository
 */

import type { TenantScope } from "@drenyra/domain/scope";
import { describe, expect, it } from "vitest";
import { PostgresProviderRepository } from "../postgres-provider.repository";
...
const PROVIDER_IN_A1 = "70000000-0000-4000-8000-000000000001";
...
describe("ProviderRepository.findById — cross-tenant isolation", () => {
	it("finds a provider in the selected company", async () => {
		const result = await repo.findById(scopeA1, PROVIDER_IN_A1);
		expect(result).not.toBeNull();
		expect(result?.id).toBe(PROVIDER_IN_A1);
	});

	it("returns null for another company in the same organization", async () => {
		const result = await repo.findById(scopeA2, PROVIDER_IN_A1);
		expect(result).toBeNull();
	});

	it("returns null for another organization", async () => {
		const result = await repo.findById(scopeB1, PROVIDER_IN_A1);
		expect(result).toBeNull();
	});
	...
	it("does not distinguish foreign provider from nonexistent provider", async () => {
		const foreignResult = await repo.findById(scopeA2, PROVIDER_IN_A1);
