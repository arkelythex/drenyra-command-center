 ...
 *
 * These live in the persistence package because that's where the harness
 * is consumed during integration testing. The harness itself is in
 * @drenyra/test-utils, but module resolution in the worktree requires
 * tests to run from the consuming package.
 *
...
describe("DeterministicFailureHarness — crash", () => {
	...
	it("crash es distinguible de errores operativos normales", async () => {
		...
		// crash fires first (injected first)
		try {
			await harness.hit("outbox.after-queue-add");
			expect.unreachable("Expected crash error");
		} catch (err) {
			expect(err).toBeInstanceOf(SimulatedProcessCrash);
		}

		// After crash is exhausted, throw fires
		try {
			await harness.hit("outbox.after-queue-add");
			expect.unreachable("Expected throw error");
		} catch (err) {
			expect(err).not.toBeInstanceOf(SimulatedProcessCrash);
			expect((err as Error).message).toBe("DB_CONNECTION_LOST");
		}

		// Both exhausted → no-op
		harness.inject("just-throw", {
			kind: "throw",
			error: new Error("THIRD"),
		});
		await expect(harness.hit("recovery.after-claim")).rejects.toThrow("THIRD");
	});
});
...
describe("DeterministicFailureHarness — block", () => {
	it("block no continúa hasta liberar la barrera", async () => {
		...
		// Esperar que actor1 procese el hit y quede bloqueado en la barrera
...
describe("DeterministicFailureHarness — filter", () => {
	it("filter condicional no activa si no cumple", async () => {
		...
		const statsAfterMiss = harness.stats("filtered");
		expect(statsAfterMiss?.hits).toBe(1);
		expect(statsAfterMiss?.activations).toBe(0);
