/**
 * PR 4.1 — EvidenceRepository cross-tenant integration tests
 *
 * Verifies that findById(scope, evidenceId) enforces tenant isolation
 * at SQL level using the company_id column on evidence.
 *
...
const EVIDENCE_IN_A1 = "90000000-0000-4000-8000-000000000001";
...
const repo = new PostgresEvidenceRepository();

describe("EvidenceRepository.findById — cross-tenant isolation", () => {
	it("finds evidence in the selected company", async () => {
		const result = await repo.findById(scopeA1, EVIDENCE_IN_A1);
		expect(result).not.toBeNull();
		expect(result?.id).toBe(EVIDENCE_IN_A1);
	});

	it("returns null for another company in the same organization", async () => {
		const result = await repo.findById(scopeA2, EVIDENCE_IN_A1);
		expect(result).toBeNull();
	});

	it("returns null for another organization", async () => {
		const result = await repo.findById(scopeB1, EVIDENCE_IN_A1);
		expect(result).toBeNull();
	});
	...
	it("does not distinguish foreign evidence from nonexistent evidence", async () => {
		const foreignResult = await repo.findById(scopeA2, EVIDENCE_IN_A1);
