/**
 * PR 3.1 — ExchangeRateRepository cross-tenant integration tests
 *
 * Verifies that findById(scope, rateId) enforces tenant isolation
...
const EXCHANGE_RATE_IN_A1 = "40000000-0000-4000-8000-000000000001";
...
const repo = new PostgresExchangeRateRepository();

describe("ExchangeRateRepository.findById — cross-tenant isolation", () => {
	it("finds an exchange rate in the selected company", async () => {
		const result = await repo.findById(scopeA1, EXCHANGE_RATE_IN_A1);
		expect(result).not.toBeNull();
		expect(result?.buy).toBeDefined();
	});

	it("returns null for another company in the same organization", async () => {
		const result = await repo.findById(scopeA2, EXCHANGE_RATE_IN_A1);
		expect(result).toBeNull();
	});

	it("returns null for another organization", async () => {
		const result = await repo.findById(scopeB1, EXCHANGE_RATE_IN_A1);
		expect(result).toBeNull();
	});

	it("returns null for an unknown exchange rate id", async () => {
		const result = await repo.findById(scopeA1, "nonexistent-id");
		expect(result).toBeNull();
	});

	it("does not distinguish foreign rate from nonexistent rate", async () => {
		const foreignResult = await repo.findById(scopeA2, EXCHANGE_RATE_IN_A1);
