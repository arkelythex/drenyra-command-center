/**
 * PR 2.2 — CpeLogRepository cross-tenant integration tests
 *
 * Verifies that findById(scope, cpeLogId) enforces tenant isolation
...
const CPE_LOG_IN_A1 = "31000000-0000-4000-8000-000000000001";
...
const repo = new PostgresCpeLogRepository();

describe("CpeLogRepository.findById — cross-tenant isolation", () => {
	it("finds a CPE log in the selected company", async () => {
		const result = await repo.findById(scopeA1, CPE_LOG_IN_A1);
		expect(result).not.toBeNull();
		expect(result?.id).toBe(CPE_LOG_IN_A1);
	});

	it("returns null for another company in the same organization", async () => {
		const result = await repo.findById(scopeA2, CPE_LOG_IN_A1);
		expect(result).toBeNull();
	});

	it("returns null for another organization", async () => {
		const result = await repo.findById(scopeB1, CPE_LOG_IN_A1);
		expect(result).toBeNull();
	});

	it("returns null for an unknown CPE log id", async () => {
		const result = await repo.findById(scopeA1, "nonexistent-id");
		expect(result).toBeNull();
	});

	it("does not distinguish foreign CPE log from nonexistent CPE log", async () => {
		const foreignResult = await repo.findById(scopeA2, CPE_LOG_IN_A1);
