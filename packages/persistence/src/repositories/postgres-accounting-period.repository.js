...
export class PostgresAccountingPeriodRepository {
    ...
    async findByCompanyAndPeriod(companyId, year, month) {
        const result = await db
            .select()
            .from(accountingPeriods)
            .where(and(eq(accountingPeriods.companyId, companyId), eq(accountingPeriods.year, year), eq(accountingPeriods.month, month)))
            .limit(1);
        if (!result[0])
            return null;
        return this.mapToDomain(result[0]);
    ...
    async findByYear(companyId, year) {
        const result = await db
            .select()
            .from(accountingPeriods)
            .where(and(eq(accountingPeriods.companyId, companyId), eq(accountingPeriods.year, year)))
    ...
    async getCurrentPeriod(companyId) {
        const result = await db
            .select()
            .from(accountingPeriods)
            .where(and(eq(accountingPeriods.companyId, companyId), ne(accountingPeriods.status, "cerrado_final"), ne(accountingPeriods.status, "auditado")))
            .orderBy(desc(accountingPeriods.year), desc(accountingPeriods.month))
            .limit(1);
        if (!result[0])
            return null;
