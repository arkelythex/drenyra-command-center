		...
		OpeningBalanceDataSource
{
	async getAccountBalances(
			...
			db
				...
				.where(
					and(
						...
						eq(journalEntries.status, "mayorizado"),
					),
				)
				.groupBy(
		...
		return {
			accounts,
			totals: {
				totalDebit: centsToAmount(
					Number(totalsResult[0]?.totalDebitCents ?? 0),
				),
				totalCredit: centsToAmount(
					Number(totalsResult[0]?.totalCreditCents ?? 0),
				),
			},
		};
	}

	async getMaterializedAccountBalances(
	...
	async getOrganizationInfo(
		organizationId: number,
	): Promise<{ name: string; ruc: string }> {
	...
	async getOpeningBalance(
		...
		const [row] = await db
			.select({
				debitCents: sql<number>`COALESCE(SUM(${journalEntryLines.debitCents}), 0)`,
				creditCents: sql<number>`COALESCE(SUM(${journalEntryLines.creditCents}), 0)`,
			})
			.from(journalEntryLines)
			.innerJoin(
				journalEntries,
