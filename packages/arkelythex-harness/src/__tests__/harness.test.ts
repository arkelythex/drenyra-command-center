...
describe("resolveRootAgentId", () => {
	...
	it("routes HR tasks to hr orchestrator", () => {
		expect(resolveRootAgentId("Calcular payroll PLAME")).toBe("drenyra-hr-orchestrator");
	});
});
...
describe("ArkelythexHarness", () => {
	it("spawns nested fiscal chain sunat → payload", async () => {
		...
		const sunat = response.tree.children.find((c) => c.agentId === "fiscal-sunat-agent");
		expect(sunat).toBeDefined();
		expect(sunat?.children.some((c) => c.agentId === "fiscal-sunat-payload-agent")).toBe(
			true,
		);
	});
