...
export async function up() {
	...
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_invoices_company_id ON invoices(company_id)`,
	);
	await db.execute(
	...
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_invoices_issue_date ON invoices(issue_date DESC)`,
	);
	await db.execute(
	...
	// Composite indexes
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_invoices_company_status ON invoices(company_id, status)`,
	);
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_invoices_company_date ON invoices(company_id, issue_date DESC)`,
	);
  ...
  `);
	...
	// Customers indexes
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_customers_company_id ON customers(company_id)`,
	);
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_customers_ruc ON customers(ruc) WHERE ruc IS NOT NULL`,
	);
	await db.execute(sql`
  ...
  `);
	...
	// Products indexes
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_products_company_id ON products(company_id)`,
	);
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_products_sku ON products(company_id, sku) WHERE sku IS NOT NULL`,
	);
	...
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_invoice_items_product_id ON invoice_items(product_id)`,
	);

	// Bills indexes
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_bills_company_id ON bills(company_id)`,
	);
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_bills_vendor_id ON bills(vendor_id)`,
	);
	await db.execute(
	...
	// Payments indexes
	await db.execute(
		sql`CREATE INDEX IF NOT EXISTS idx_payments_company_id ON payments(company_id)`,
	);
	await db.execute(
