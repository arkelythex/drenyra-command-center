...
export class NATSEventBus implements EventBusPort {
      ...
      (async () => {
        for await (const msg of sub) {
          try {
            ...
            console.log(`📥 Received: ${eventType} [${event.metadata.eventId}]`);
  ...
  private async ensureDomainEventStream(): Promise<void> {
    ...
    try {
      ...
      const mergedSubjects = Array.from(new Set([...currentSubjects, ...desiredSubjects]));
      ...
      if (mergedSubjects.length !== currentSubjects.length) {
        ...
        console.log(`✅ Updated NATS stream ${streamName}: ${mergedSubjects.join(', ')}`);
      }
      return;
    } catch (error) {
    ...
    console.log(`✅ Created NATS stream ${streamName}: ${desiredSubjects.join(', ')}`);
  }
}

function resolveDesiredStreamSubjects(): string[] {
  const configured = process.env.NATS_STREAM_SUBJECTS
    ?.split(',')
...
function isStreamNotFoundError(error: unknown): boolean {
  ...
  return /stream/i.test(error.message) && /not found|no stream/i.test(error.message);
}
