...
const main = async () => {
	...
	const accountData = [
		...
		{
			name: "Interbank Dólares",
			type: "BANK",
			currency: "USD",
			bankName: "Interbank",
			accountNumber: "200-3000500090",
			balance: "5200.00",
		},
		{
			name: "AMEX Corporate",
	...
	const partners = [
		...
		},
		{
			...
			ruc: "20000000001",
			type: "EXPENSE",
			logo: "https://upload.wikimedia.org/wikipedia/commons/thumb/5/5a/Sunat_Logo.svg/1200px-Sunat_Logo.svg.png",
		},
		{
			name: "CLIENTE MINERA X",
			ruc: "20100000000",
			type: "INCOME",
			logo: null,
		},
	...
	for (let i = 0; i < 50; i++) {
		...
		const partner = partnerIds.find(
			(p) => p.type === (isIncome ? "INCOME" : "EXPENSE"),
		);
		...
		const category = categoryIds.find(
	...
	console.log(
		"✅ Seed completado: Cuentas, Categorías y Transacciones listas.",
	);
	process.exit(0);
};
