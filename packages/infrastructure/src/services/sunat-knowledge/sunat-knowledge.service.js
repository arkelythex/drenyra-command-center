...
export class SunatKnowledgeService {
    ...
    async getStats() {
      ...
      ORDER BY count DESC
    `);
        return Object.fromEntries(Array.from(rows).map((r) => [r.category, Number(r.count)]));
    }
    ...
    async hybridSearch(query, options = DEFAULT_HYBRID_OPTIONS) {
        ...
        const bm25Normalized = new Map(bm25Results.map((r) => [
            r.id,
            { chunk: r, score: (r.rank ?? 0) / bm25Max },
        ]));
        const denseMax = Math.max(...denseResults.map((r) => r.similarity), 0.01);
        const denseNormalized = new Map(denseResults.map((r) => [
            r.id,
        ...
        if (options.rerank && sortedResults.length > 0) {
            const rerankResults = await rerankerService.rerank(query.query, sortedResults.map((r) => ({ id: r.id, content: r.content })));
            const rerankedMap = new Map(rerankResults.map((r, i) => [r.chunkId, { ...r, newIndex: i }]));
