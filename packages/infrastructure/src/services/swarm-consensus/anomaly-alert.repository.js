...
export const anomalyAlertRepository = {
    ...
    async getRecentBySeverity(organizationId, severity, days = 7) {
        return db
            ...
            .from(anomalyAlerts)
            .where(and(eq(anomalyAlerts.organizationId, organizationId), eq(anomalyAlerts.severity, severity), sql `${anomalyAlerts.createdAt} > NOW() - INTERVAL '${days} days'`))
            .limit(50);
    },
    ...
    async getFalsePositiveRate(organizationId, severity, days = 30) {
        ...
        const rows = await db
            ...
            .from(anomalyAlerts)
            .where(and(eq(anomalyAlerts.organizationId, organizationId), eq(anomalyAlerts.severity, severity), gte(anomalyAlerts.createdAt, cutoff)));
