...
export function computeConsensusScore(confidences) {
    ...
    const variance = confidences.reduce((sum, c) => sum + (c.confidence - avgForVariance) ** 2, 0) / confidences.length;
...
export function computeDynamicThreshold(severity, falsePositiveRate) {
    ...
    const fpAdjustment = falsePositiveRate > 0.1
        ? Math.min((falsePositiveRate - 0.1) * 0.8, 0.08)
        : 0;
    return Math.min(base + fpAdjustment, MAX_DYNAMIC_THRESHOLD);
}
export function generateReasoning(confidences, consensusScore, threshold, dynamicAdjustment) {
    const parts = confidences.map((c) => `${c.agentRole}: ${(c.confidence * 100).toFixed(0)}%`);
    const status = consensusScore >= threshold ? "✓ Confirmado" : "✗ Insuficiente";
    const adjustmentNote = dynamicAdjustment !== undefined && dynamicAdjustment > 0
