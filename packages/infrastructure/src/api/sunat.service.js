export async function consultarRucSunat(ruc) {
    ...
    if (!isPersona && !isEmpresa) {
        throw new Error("RUC inválido: debe iniciar con 10 (persona) o 20 (empresa)");
    }
    try {
        ...
        throw new Error(`Error consultando SUNAT: ${error instanceof Error ? error.message : "Error desconocido"}`);
    }
}
...
export async function verificarComprobanteSunat(rucEmisor, tipo, serie, numero) {
    ...
    const seriePattern = tipo === "01" || tipo === "07" || tipo === "08" ? /^F\d{3}$/ : /^B\d{3}$/;
...
export async function consultarDni(dni) {
    ...
    try {
        ...
        throw new Error(`Error consultando DNI: ${error instanceof Error ? error.message : "Error desconocido"}`);
    }
}
