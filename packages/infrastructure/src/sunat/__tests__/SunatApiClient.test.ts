...
function setupFetchMock(handler: (url: string, _options?: RequestInit) => Promise<Response>) {
    ...
    it("fetches a new token on cache miss", async () => {
      ...
      setupFetchMock(async (url: string) => {
        if (url.includes("token")) {
          return new Response(
            JSON.stringify({ access_token: "test-token-123", token_type: "Bearer", expires_in: 3600 }),
            { status: 200 },
          );
        }
    ...
    it("builds correct URL and returns data", async () => {
      ...
      setupFetchMock(async (url: string) => {
        ...
        if (url.includes("contribuyente")) {
          return new Response(
            JSON.stringify({ ruc: "20123456789", razonSocial: "TEST SAC", estado: "ACTIVO", condicion: "HABIDO" }),
            { status: 200 },
          );
        }
        return new Response("Not found", { status: 404 });
