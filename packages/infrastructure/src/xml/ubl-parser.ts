/**
 * UBL 2.1 Parser for SUNAT Electronic Invoices
 *
 * Parses XML invoices in UBL 2.1 format used by SUNAT (Peru).
 * Split from 596 lines → facade + 2 modules (types, extractor).
 */
...
import type {
	...
	ParseResult,
	UBLDocument,
} from "./ubl-parser.types";

/**
 * UBL 2.1 Parser for SUNAT Electronic Invoices.
 *
 * Delegates extraction to ubl-extractor.ts for modularity.
 */
export class UBLParser {
	...
	parseInvoice(xmlContent: string): ParsedInvoice {
		...
		const doc = this.parser.parse(xmlContent) as UBLDocument;
...
/**
 * Performs a lightweight check for valid SUNAT UBL XML content.
 */
export function isValidSunatXML(content: string): boolean {
