...
export class UBLParser {
    ...
    parseInvoice(xmlContent) {
        ...
        if (validation !== true) {
            throw new Error(`XML inválido: ${validation.err?.msg || "Error de formato"}`);
        }
        const doc = this.parser.parse(xmlContent);
        const invoice = doc.Invoice ?? doc.CreditNote ?? doc.DebitNote;
        if (!invoice) {
            throw new Error("No se encontró elemento Invoice, CreditNote o DebitNote");
        }
        try {
            return {
            ...
            throw new Error(`Error al parsear factura: ${error instanceof Error ? error.message : "Error desconocido"}`);
        }
    }
...
//# sourceMappingURL=ubl-parser.js.map