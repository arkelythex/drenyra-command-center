...
export function extractSupplierRuc(invoice) {
    const partyNode = invoice["cac:AccountingSupplierParty"] || invoice.AccountingSupplierParty;
    if (!partyNode || typeof partyNode !== "object" || Array.isArray(partyNode))
        return "";
    const party = partyNode["cac:Party"] ||
        partyNode.Party;
    if (!party || typeof party !== "object" || Array.isArray(party))
        return "";
    const partyIdNode = party["cac:PartyIdentification"] ||
        party.PartyIdentification;
    if (!partyIdNode ||
        typeof partyIdNode !== "object" ||
        Array.isArray(partyIdNode))
        return "";
    const idNode = partyIdNode["cbc:ID"] ||
        partyIdNode.ID;
    return idNode
        ? String(typeof idNode === "object" && "#text" in idNode
            ? idNode["#text"]
            : idNode)
        : "";
}
export function extractSupplierName(invoice) {
    ...
    if (!party || typeof party !== "object" || Array.isArray(party))
        return "";
    const legalName = party["cac:PartyLegalEntity"] ||
        party.PartyLegalEntity;
    if (!legalName || typeof legalName !== "object" || Array.isArray(legalName))
        return "";
    const registrationName = legalName["cbc:RegistrationName"] ||
        legalName.RegistrationName;
    return registrationName
        ? String(typeof registrationName === "object" && "#text" in registrationName
            ? registrationName["#text"]
            : registrationName)
        : "";
}
export function extractCustomerRuc(invoice) {
    const partyNode = invoice["cac:AccountingCustomerParty"] || invoice.AccountingCustomerParty;
    if (!partyNode || typeof partyNode !== "object" || Array.isArray(partyNode))
        return "";
    const party = partyNode["cac:Party"] ||
        partyNode.Party;
...
export function extractSubtotal(invoice) {
    const monetaryTotal = invoice["cac:LegalMonetaryTotal"] || invoice.LegalMonetaryTotal;
    if (!monetaryTotal ||
        typeof monetaryTotal !== "object" ||
        Array.isArray(monetaryTotal))
        return 0;
    const lineExt = monetaryTotal["cbc:LineExtensionAmount"] ||
        monetaryTotal.LineExtensionAmount;
    return lineExt
        ? parseFloat(String(typeof lineExt === "object" && "#text" in lineExt
            ? lineExt["#text"]
            : lineExt)) || 0
        : 0;
}
export function extractIGV(invoice) {
    ...
    if (!taxTotal || typeof taxTotal !== "object" || Array.isArray(taxTotal))
        return 0;
    const taxAmt = taxTotal["cbc:TaxAmount"] ||
        taxTotal.TaxAmount;
    return taxAmt
        ? parseFloat(String(typeof taxAmt === "object" && "#text" in taxAmt
            ? taxAmt["#text"]
            : taxAmt)) || 0
        : 0;
}
export function extractTotal(invoice) {
    ...
    if (!monetaryTotal ||
        ...
        Array.isArray(monetaryTotal))
        return 0;
    const payableAmt = monetaryTotal["cbc:PayableAmount"] ||
        monetaryTotal.PayableAmount;
    return payableAmt
        ? parseFloat(String(typeof payableAmt === "object" && "#text" in payableAmt
            ? payableAmt["#text"]
            : payableAmt)) || 0
        : 0;
}
...
export function extractItems(invoice) {
    ...
    return lineArray.map((line) => {
        ...
        const lineObj = line;
        const description = extractText(lineObj, "Item.Name", "Item.cbc:Name", "cbc:Name");
        const quantity = extractNumber(lineObj, "InvoicedQuantity", "cbc:InvoicedQuantity");
        const unitPrice = extractNumber(lineObj, "Price.PriceAmount", "Price.cbc:PriceAmount", "cbc:PriceAmount");
        const lineTotal = extractNumber(lineObj, "LineExtensionAmount", "cbc:LineExtensionAmount");
        const unitCode = extractText(lineObj, "InvoicedQuantity.@_unitCode", "cbc:InvoicedQuantity.@_unitCode");
