...
async function processDocument(job) {
    ...
    const { companyId, documentId, fileUrl, fileType, fileName } = documentFetchSchema.parse(job.data);
    ...
    try {
        ...
        if (fileType === "XML") {
            ...
            extractedData = {
                ...
                documentNumber: data.series && data.number
        ...
        if (extractedData.providerName) {
            try {
                const classificationAmount = extractedData.totalAmount ??
        ...
        if (extractedData.providerRUC && !isValidRUC(extractedData.providerRUC)) {
            throw new Error(`Invalid supplier RUC extracted for document ${documentId}: ${extractedData.providerRUC}`);
        }
        const completedDoc = extractingDoc.completeExtraction(toExtractedDataPayload(extractedData));
