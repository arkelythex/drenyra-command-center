...
async function seed() {
	...
	try {
		...
		console.log(
			`✅ Operational dashboard demo seeded (${operationalDemo.monthsSeeded} months)`,
		);
		console.log(`   - ${operationalDemo.partnersSeeded} partners`);
		console.log(`   - ${operationalDemo.productsSeeded} products`);

		// === KILLER FEATURES DEMO DATA ===

		// 6. Create Economic Group (Multi-RUC Feature)
		...
		let economicGroupId: string;
		let economicGroup: typeof economicGroups.$inferSelect;
		...
		let insertedAdditionalCompanies: Array<typeof companies.$inferSelect> = [];

		if (!existingAdditional) {
			insertedAdditionalCompanies = await db
				.insert(companies)
				.values([
					{
						ownerId: adminUserId,
				...
				.returning();
			console.log(
				`✅ ${insertedAdditionalCompanies.length} additional companies created in group`,
			);
		}

		// Update main company to belong to group and be primary
		await db
			.update(companies)
			.set({
				economicGroupId: economicGroupId,
		...
		// 8. Create Firm Model (AI Learning Feature)
		...
		// 9. Create Inter-Company Transactions (Demo)
		const existingInterTx = await db.query.interCompanyTransactions.findFirst({
			where: eq(
				interCompanyTransactions.concept,
				"Alquiler Oficina - Mes Enero 2026",
			),
		});
		...
		console.log("\n🔐 Demo Auth Bootstrap:");
		console.log(
			"Run `bun scripts/create-admin-user.ts` or the root `bun run db:seed` command",
		);
		console.log(
			"to guarantee Better Auth login credentials are aligned with the demo company.",
		);
		...
		console.log(
			`   - Monthly Fee: S/ ${economicGroup.monthlyFee} (vs S/ 900 in CONCAR)`,
		);
 ...
 *
 * @returns Result of seedDatabase.
 * @example
 * ```ts
