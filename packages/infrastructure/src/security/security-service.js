...
export async function recordFailedLogin(email, ipAddress, userAgent, reason = "INVALID_CREDENTIALS") {
    const windowStart = new Date(Date.now() - WINDOW_MINUTES * 60 * 1000);
    const existing = await db
        .select()
        .from(failedLoginAttempts)
        .where(and(eq(failedLoginAttempts.email, email), gt(failedLoginAttempts.createdAt, windowStart)))
...
export async function recordSuccessfulLogin(userId, email, ipAddress, userAgent) {
    const windowStart = new Date(Date.now() - WINDOW_MINUTES * 60 * 1000);
    await db
...
export async function isAccountLocked(email) {
    const locked = await db
        .select()
        .from(failedLoginAttempts)
        .where(and(eq(failedLoginAttempts.email, email), eq(failedLoginAttempts.reason, "ACCOUNT_LOCKED"), isNotNull(failedLoginAttempts.lockedUntil), gt(failedLoginAttempts.lockedUntil, new Date())))
        .orderBy(desc(failedLoginAttempts.createdAt))
        .limit(1);
