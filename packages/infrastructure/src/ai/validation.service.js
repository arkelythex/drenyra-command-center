...
export async function validateInvoiceWithAI(invoice) {
    ...
    try {
        ...
        if (!zodResult.isValid && zodResult.errors) {
            const criticalErrors = zodResult.errors.filter((err) => err.code === "invalid_type" ||
    ...
    for (const suggestion of validationResult.suggestions) {
        if (suggestion.confidence > 0.9) {
            ...
            if (fieldPath.length === 1 && fieldPath[0]) {
                correctedInvoice[fieldPath[0]] =
                    suggestion.suggestion;
                appliedCorrections.push(`${suggestion.field}: ${suggestion.suggestion}`);
            }
        }
        else {
            pendingCorrections.push(`${suggestion.field}: ${suggestion.suggestion} (${(suggestion.confidence * 100).toFixed(0)}% confianza)`);
        }
    }
    return { correctedInvoice, appliedCorrections, pendingCorrections };
}
export async function batchValidateInvoices(invoices) {
    ...
    for (let i = 0; i < invoices.length; i += BATCH_SIZE) {
        ...
        const batchResults = await Promise.all(batch.map((inv) => validateInvoiceWithAI(inv)));
