/**
 * AI System Prompts - Domain-specific instructions
 *
 * These prompts contain critical domain knowledge about Peruvian accounting
 * @example
 * ```ts
 * console.log(PERUVIAN_ACCOUNTING_CONTEXT);
 * ```
 */
...
/**
 * OCR_EXTRACTION_PROMPT const.
 *
 * @example
 * ```ts
 * console.log(OCR_EXTRACTION_PROMPT);
 * ```
 */
...
/**
 * VALIDATION_PROMPT const.
 *
 * @example
 * ```ts
 * console.log(VALIDATION_PROMPT);
 * ```
 */
...
/**
 * ANTIGRAVITY_PROMPT const.
 *
 * @example
 * ```ts
 * console.log(ANTIGRAVITY_PROMPT);
 * ```
 */
...
/**
 * Generate OCR prompt for invoice extraction
 * @param imageType - Input for imageType.
 * @returns Result of getOCRPrompt.
 * @example
 * ```ts
 * const result = getOCRPrompt("pdf");
 * console.log(result);
 * ```
 */

export function getOCRPrompt(imageType: "pdf" | "image" = "pdf") {
	return `${OCR_EXTRACTION_PROMPT}\n\nTipo de documento: ${imageType === "pdf" ? "PDF" : "Imagen"}`;
}

/**
 * Generate validation prompt
 * @param invoiceData - Input for invoiceData.
 * @returns Result of getValidationPrompt.
 * @example
 * ```ts
 * const result = getValidationPrompt(undefined);
 * console.log(result);
 * ```
 */

export function getValidationPrompt(invoiceData: unknown) {
	return `${VALIDATION_PROMPT}\n\nDatos a validar:\n${JSON.stringify(invoiceData, null, 2)}`;
}

/**
 * Generate Antigravity real-time validation prompt
 * @param field - Input for field.
 * @param value - Input for value.
 * @param context - Input for context.
 * @returns Result of getAntigravityPrompt.
 * @example
 * ```ts
 * const result = getAntigravityPrompt("", undefined, undefined);
 * console.log(result);
 * ```
 */

export function getAntigravityPrompt(
	field: string,
	value: unknown,
	context?: unknown,
) {
