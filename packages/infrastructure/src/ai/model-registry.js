...
function instantiateModel(modelKey) {
    ...
    switch (def.provider) {
        ...
        case "openai":
            console.warn(`[ModelRegistry] OpenAI not available, using Google fallback`);
...
export function selectModelForTask(task) {
    ...
    const tierPriority = requiredTier === "opus"
    ...
    const sorted = candidates.sort((a, b) => {
        ...
        if (tierA !== tierB)
            return tierA - tierB;
        if (a.def.provider === currentConfig.preferredProvider &&
            b.def.provider !== currentConfig.preferredProvider) {
            return -1;
        }
        if (b.def.provider === currentConfig.preferredProvider &&
