...
import type { AgentRunState, AgentRunEvent, RunStateFilter } from "../../src/session/session.types";
...
describe("PostgresSessionStore", () => {
  ...
  describe("saveRunState", () => {
    ...
    it("should propagate DB error as SessionStoreError", async () => {
      ...
      mockDb.insert.mockReturnValue({
        values: vi.fn().mockReturnValue({
          onConflictDoUpdate: vi.fn().mockRejectedValue(new Error("Connection lost")),
        }),
      });
  ...
  describe("getRunState", () => {
    ...
    it("should propagate DB error as SessionStoreError", async () => {
      ...
      await expect(store.getRunState("run-error")).rejects.toThrow(SessionStoreError);
    });
  });
  ...
  describe("listRunStates", () => {
      ...
      mockDb.select.mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockReturnValue({
            orderBy: vi.fn().mockReturnValue({
              limit: vi.fn().mockReturnValue({
                offset: vi.fn().mockReturnValue(thenableResult([toDbRow(completedRunState)])),
              }),
            }),
          }),
        }),
    ...
    it("should propagate DB error as SessionStoreError", async () => {
      ...
      await expect(store.listRunStates(filter)).rejects.toThrow(SessionStoreError);
    });
  });
  ...
  describe("appendEvent", () => {
    ...
    it("should handle null payload", async () => {
      ...
      await expect(store.appendEvent("run-null-payload", event)).resolves.not.toThrow();
    });
    ...
    it("should propagate DB error as SessionStoreError", async () => {
      ...
      await expect(store.appendEvent("run-error", event)).rejects.toThrow(SessionStoreError);
    });
  });
  ...
  describe("getEvents", () => {
    it("should return events ordered ASC by createdAt", async () => {
      const event1 = toEventDbRow({ ...defaultEvent, id: 1, createdAt: new Date("2026-06-15T10:01:00Z") });
      const event2 = toEventDbRow({ ...defaultEvent, id: 2, createdAt: new Date("2026-06-15T10:02:00Z") });
    ...
    it("should propagate DB error as SessionStoreError", async () => {
      ...
      await expect(store.getEvents("run-error")).rejects.toThrow(SessionStoreError);
    });
  });
  ...
  describe("recoverRunState", () => {
      ...
      mockDb.select.mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockReturnValue({
            ...
            orderBy: vi.fn().mockReturnValue({
              limit: vi.fn().mockReturnValue(thenableResult([toEventDbRow(defaultEvent)])),
            }),
          }),
        }),
      });
    ...
    it("should propagate DB error as SessionStoreError", async () => {
      ...
      await expect(store.recoverRunState("run-error")).rejects.toThrow(SessionStoreError);
    });
  });
  ...
  describe("saveInput", () => {
    it("should save a new input record", async () => {
      await store.saveInput("run-input-1", "image", "base64data==", "abc123checksum");
  ...
  describe("getInput", () => {
    ...
    it("should propagate DB error as SessionStoreError", async () => {
      ...
      await expect(store.getInput("run-error")).rejects.toThrow(SessionStoreError);
    });
  });
  ...
  describe("tenant isolation", () => {
    it("should only return states for the specified companyId", async () => {
      ...
      const companyAResults = await store.listRunStates({ companyId: "company-a-uuid" });
      ...
      const leakedToA = companyAResults.filter((r) => r.companyId === "company-b-uuid");
