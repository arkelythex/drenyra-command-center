...
import type { SessionStore, AgentRunState } from "../../src/session";
...
function makeRun(overrides: Partial<AgentRunState> = {}): AgentRunState {
  return {
    ...
    context: {
      ...
      memorySummary: "Extracted invoice: EMPRESA SAC - RUC: 20123456789 - Amount: PEN 118.00 - Confidence: 95% - Flags: 0",
    },
    status: "completed",
    error: null,
...
describe("MemoryContextProvider", () => {
  describe("getContext", () => {
    ...
    it("respects custom maxRuns config", async () => {
      const runs = Array.from({ length: 10 }, (_, i) => makeRun({ runId: `run-${i}` }));
      const mockStore = createMockStore(runs);
      const provider = new MemoryContextProvider(mockStore, { maxRuns: 3, maxSummaryLength: 500 });
    ...
    it("truncates long summaries", async () => {
      ...
      const provider = new MemoryContextProvider(mockStore, { maxRuns: 5, maxSummaryLength: 100 });
    ...
    it("non-blocking when store throws", async () => {
      const mockStore = createMockStore();
      (mockStore.listRunStates as ReturnType<typeof vi.fn>).mockRejectedValue(new Error("DB connection lost"));
      const provider = new MemoryContextProvider(mockStore);

      await expect(provider.getContext("ruc-test")).rejects.toThrow("DB connection lost");
    });
    ...
    it("returns multiple runs in order", async () => {
      const runs = [
        makeRun({
          runId: "run-1",
          context: { memorySummary: "First extraction result", inputType: "invoice_image" },
        }),
        makeRun({
          runId: "run-2",
          context: { memorySummary: "Second extraction result", inputType: "invoice_xml" },
        }),
      ];
      const mockStore = createMockStore(runs);
      const provider = new MemoryContextProvider(mockStore);
