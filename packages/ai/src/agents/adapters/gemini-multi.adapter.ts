 ...
 * @example
 * ```ts
 * const value: GeminiConfig = {} as GeminiConfig;
 * console.log(value);
 * ```
 */
...
/**
 * GeminiMultimodalInput interface.
 *
 * @example
 * ```ts
 * const value: GeminiMultimodalInput = {} as GeminiMultimodalInput;
 * console.log(value);
 * ```
 */
...
/**
 * GeminiMultiAdapter class.
 *
 * @example
 * ```ts
 * const value = new GeminiMultiAdapter();
 * console.log(value);
 * ```
 */
export class GeminiMultiAdapter {
  ...
  // Pricing estimates per million tokens (2026)
  private readonly INPUT_PRICE_PER_M = 0.50;
  private readonly OUTPUT_PRICE_PER_M = 3.00;
  private readonly CACHED_INPUT_PRICE_PER_M = 0.05; // 90% discount
  ...
  /**
   * Generate content (text or multimodal)
   */
  async generate(input: GeminiMultimodalInput): Promise<AIResponse> {
    ...
    if (this.config.cacheEnabled && !input.images?.length) {
      ...
      if (cached) {
        loggers.ai.info('Gemini cache hit', { instanceId: this.config.instanceId });
        return {
          content: cached,
    ...
    try {
      ...
      // Estimate tokens and cost (Gemini API doesn't return token counts in all responses)
      const estimatedInputTokens = this.estimateTokens(input.text || '');
      const estimatedOutputTokens = this.estimateTokens(content);
      const cost = this.calculateCost(estimatedInputTokens, estimatedOutputTokens, false);

      // Cache the response (text-only)
      if (this.config.cacheEnabled && !input.images?.length) {
        const cacheKey = this.getCacheKey(input);
   ...
   */
  private parseBase64Image(imgData: string): { mimeType: string; data: string } {
  ...
  clearCache(): void {
    this.cache.clear();
    loggers.ai.info('Gemini cache cleared', { instanceId: this.config.instanceId });
  }
...
/**
 * Factory for creating multiple Gemini instances
 * @example
 * ```ts
 * const value = new GeminiInstanceFactory();
 * console.log(value);
 * ```
 */
...
export class GeminiInstanceFactory {
   ...
   */
  getInstance(instanceId: string, config?: Partial<GeminiConfig>): GeminiMultiAdapter {
  ...
  getStats(): Array<{ instanceId: string; size: number; ttl: number }> {
    return Array.from(this.instances.values()).map((instance) => instance.getCacheStats());
  }
}
