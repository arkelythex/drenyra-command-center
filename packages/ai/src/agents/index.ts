...
import { GeminiInstanceFactory, GrokAdapter, OpenRouterAdapter } from './adapters';
...
import { validateConfig, mergeWithDefaults } from './config/agent.config';
...
import { RetryEngine, PersistentCircuitBreaker } from '../services/error-recovery';
...
export class AgentSwarmFactory {
  ...
  constructor(config: AgentSwarmConfig) {
    ...
    if (this.config.enableErrorRecovery) {
      ...
      loggers.ai.info('Error recovery initialized with RetryEngine + PersistentCircuitBreaker');
    }
  ...
  async createOrchestrator(): Promise<WorkflowOrchestrator> {
    ...
    // Create agents
    const readerAgent = new ReaderAgent(readerGemini, this.memoryProvider);
    const parserAgent = new ParserAgent(parserGemini);
    ...
    if (this.config.openRouterApiKey && this.openRouterAdapter) {
      ...
      throw new Error('No valid adapter available for Validator Agent. Configure grokApiKey or openRouterApiKey.');
    }

    const validatorAgent = new ValidatorAgent(validatorAdapter);
    const arbitratorAgent = new ArbitratorAgent(arbitratorGemini);
    ...
    try {
      ...
      const { PostgresSessionStore } = await import('../session/postgres-store');
      sessionStore = new PostgresSessionStore(db);
      this.sessionStore = sessionStore;
      this.memoryProvider = new MemoryContextProvider(sessionStore);
      loggers.ai.info('PostgresSessionStore initialized for agent run persistence');
    } catch (err) {
      loggers.ai.warn('Session store unavailable — persistence disabled', { error: err });
    }
   ...
   */
  private async createOSEService(): Promise<import('./orchestrator/workflow-v2/types').OrchestratorConfig['oseService'] | undefined> {
    try {
      ...
      loggers.ai.warn('[OSE] Failed to load OSE service from @arkelythex/infrastructure/ose:', error);
      return undefined;
    }
  }
...
export async function createAgentSwarm(config: AgentSwarmConfig): Promise<WorkflowOrchestrator> {
