...
/**
 * ArbitratorAgent class.
 *
 * @example
 * ```ts
 * const value = new ArbitratorAgent();
 * console.log(value);
 * ```
 */
  ...
  "confidence": 0.95
	...
	async process(input: ArbitratorInput): Promise<ArbitrationDecision> {
		...
		try {
			...
			// Check if arbitration is even needed
	...
	/**
	 * Auto-approve when no conflicts
	 */
	private autoApprove(
		...
		// Prefer validator's data (most compliant)
	...
	/**
	 * Extract InvoiceData from ExtractedData
	 */
	private extractInvoiceData(extracted: InvoiceData): InvoiceData {
		// Already InvoiceData, just return
		return extracted;
	}
