...
export interface HistogramMetric {
  ...
  type: 'histogram';
  buckets: number[];
  values: Array<{ value: number; timestamp: number; labels: Record<string, string>; bucket: string }>;
}
...
export class AgentMetricsCollector {
   ...
   */
  recordProcessingTime(agentName: string, durationMs: number, status: 'success' | 'failure'): void {
    const metric = this.metrics.get('agent_processing_duration_ms') as HistogramMetric;
  ...
  recordSuccess(agentName: string): void {
    const metric = this.metrics.get('agent_processing_success_total') as CounterMetric;
    metric.values.push({
      value: 1,
      timestamp: Date.now(),
  ...
  recordFailure(agentName: string, errorType: string): void {
    const metric = this.metrics.get('agent_processing_failures_total') as CounterMetric;
    metric.values.push({
      value: 1,
      timestamp: Date.now(),
  ...
  recordConflict(agentNames: string[]): void {
    const metric = this.metrics.get('agent_conflicts_detected_total') as CounterMetric;
    metric.values.push({
      value: 1,
      timestamp: Date.now(),
  ...
  recordCircuitBreakerOpen(agentName: string): void {
    const metric = this.metrics.get('agent_circuit_breaker_opens_total') as CounterMetric;
    metric.values.push({
      value: 1,
      timestamp: Date.now(),
      labels: { agent: agentName },
  ...
  getSuccessRate(agentName: string, timeWindowMs: number = 3600000): number {
    ...
    const successMetric = this.metrics.get('agent_processing_success_total') as CounterMetric;
    const failureMetric = this.metrics.get('agent_processing_failures_total') as CounterMetric;
   ...
   */
  getAverageProcessingTime(agentName: string, timeWindowMs: number = 3600000): number {
    const now = Date.now();
    const cutoff = now - timeWindowMs;
   ...
   */
  getP95ProcessingTime(agentName: string, timeWindowMs: number = 3600000): number {
    const now = Date.now();
    const cutoff = now - timeWindowMs;
  ...
  private aggregateMetricValues(metric: CounterMetric | GaugeMetric): Map<string, number> {
...
export class DoraMetricsCollector {
  private deployments: Array<{ timestamp: number; success: boolean; leadTimeMinutes: number }> = [];
  ...
  getMetrics(): DoraMetrics {
    ...
    const recentDeployments = this.deployments.filter(d => d.timestamp >= now - weekWindow);
    const deploymentFrequency = recentDeployments.length / 7;

    // Lead time for changes (median of last week)
    const leadTimes = recentDeployments.map(d => d.leadTimeMinutes).sort((a, b) => a - b);
    ...
    const recentFailures = this.failures.filter(f => f.timestamp >= now - weekWindow);
    const meanTimeToRecovery = recentFailures.length > 0
      ? recentFailures.reduce((sum, f) => sum + f.recoveryTimeMinutes, 0) / recentFailures.length
      : 0;

    // Change failure rate
    const successfulDeployments = recentDeployments.filter(d => d.success).length;
    const changeFailureRate = recentDeployments.length > 0
      ? ((recentDeployments.length - successfulDeployments) / recentDeployments.length) * 100
      : 0;
