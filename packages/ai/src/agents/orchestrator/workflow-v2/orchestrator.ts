...
import { ReaderAgent, ParserAgent, ValidatorAgent, ArbitratorAgent } from '../../agents';
import { loggers } from '../../../logger';
import type { AgentRunStatus, AgentWorkflowState } from '../../../session/session.types';

import type { OrchestratorConfig, ParallelExecutionResult, AgentMetrics, PhaseSkipOptions } from './types';
...
import { CircuitBreaker, AgentMetricsCollector } from './steps';
import { SessionStoreError, SessionNotFoundError } from '../../../session/session.types';
...
export class WorkflowOrchestratorV2 {
  ...
  constructor(
    ...
    if (this.config.pruner) {
      const sub = this.eventBus.on('PRUNE_REQUESTED', (event) => {
        try {
          const pruneEvent = event as import('../../types/workflow.types').PruneRequestedEvent;
          loggers.ai.info('Orchestrator: PRUNE_REQUESTED received, triggering reactive pruning', {
  ...
  async processInvoice(
    ...
    this.appendSessionEvent(processId, 'PROCESS_STARTED', { inputType: input.type }, input.metadata?.ruc ?? 'unknown');

    try {
      // Stage 1: TRUE PARALLEL execution of all agents
      const executionResult = await this.executeAgentsInParallel(processId, input, phaseSkip);

      // Check if we have minimum required data
      if (!executionResult.reader?.result) {
        throw new Error('Reader agent failed - cannot proceed without extracted data');
      }
      ...
      if (conflicts.length > 0) {
        ...
        await this.persistRunState(processId, { workflowState: 'ARBITRATING' as AgentWorkflowState });

        // Stage 3: Arbitrate conflicts
        const arbitration = await this.arbitrateConflicts(processId, executionResult, conflicts);
      ...
      if (this.config.oseService && xmlContent && finalData?.invoiceNumber) {
        try {
          ...
          if (oseResult.success) {
            ...
            oseStageLog = this.createStageLog('ose', oseStartTime, new Date(), 'success');
            ...
            oseStageLog = this.createStageLog('ose', oseStartTime, new Date(), 'failed', undefined, oseResult.error);
          ...
          oseStageLog = this.createStageLog('ose', oseStartTime, new Date(), 'failed', undefined, oseError instanceof Error ? oseError.message : 'Unknown OSE error');

          this.eventBus.emit({
            type: 'OSE_FAILED',
            timestamp: new Date(),
            processId,
            error: oseError instanceof Error ? oseError.message : 'Unknown OSE error',
            retryCount: 0,
          });
        }
      } else {
        oseStageLog = this.createStageLog('ose', oseStartTime, oseStartTime, 'skipped');
      }
      ...
      this.appendSessionEvent(processId, 'PROCESS_COMPLETED', { duration: executionResult.totalDuration }, input.metadata?.ruc ?? 'unknown');
      ...
      this.eventBus.emit({
        ...
        metrics: this.config.enableMetrics ? this.getMetricsSnapshot() : undefined,
      });

      return this.createSuccessResult(processId, context, finalData, xmlContent, oseStageLog, cdrResponse);
      ...
      this.appendSessionEvent(processId, 'PROCESS_FAILED', { error: (error as Error).message }, input.metadata?.ruc ?? 'unknown');
  ...
  private async executeAgentsInParallel(
    ...
    loggers.ai.info('Executing agents in parallel', { processId, skipPhases: phaseSkip?.skipPhases });
    ...
    const extractedData = readerSettlement.status === 'fulfilled' ? readerSettlement.value.result : null;
    const parsedData = parserSettlement.status === 'fulfilled' ? parserSettlement.value.result : null;
    const bestSource = extractedData || parsedData;

    let validatorSettlement: PromiseSettledResult<{ result: ValidationResult; metrics: AgentMetrics }>;

    if (bestSource || skipSet.has('validator')) {
      validatorSettlement = await this.executeValidatorAgent(processId, bestSource, skipSet, phaseSkip);
    } else {
      validatorSettlement = {
        status: 'rejected',
        reason: new Error('No data available for validation (both Reader and Parser failed)')
      };
    }
    ...
    if (readerSettlement.status === 'fulfilled') {
      ...
      result.reader = {
        ...
        log: this.createStageLog('reader', new Date(metrics.startTime), new Date(metrics.endTime || Date.now()), 'success', data),
        metrics,
      };
      ...
      result.errors.push({ agent: 'reader', error: readerSettlement.reason as Error });
    }
    ...
    if (parserSettlement.status === 'fulfilled') {
      ...
      result.parser = {
        ...
        log: this.createStageLog('parser', new Date(metrics.startTime), new Date(metrics.endTime || Date.now()), 'success', data),
        metrics,
      };
      ...
      result.errors.push({ agent: 'parser', error: parserSettlement.reason as Error });
    }
    ...
    if (validatorSettlement.status === 'fulfilled') {
      ...
      result.validator = {
        ...
        log: this.createStageLog('validator', new Date(metrics.startTime), new Date(metrics.endTime || Date.now()), 'success', data),
        metrics,
      };
      // Persist VALIDATING state with all data for recovery
      const persistContext: Record<string, unknown> = {};
      if (result.reader?.result) persistContext.extractedData = result.reader.result;
      if (result.parser?.result) persistContext.parsedData = result.parser.result;
      ...
      result.errors.push({ agent: 'validator', error: validatorSettlement.reason as Error });
    }
  ...
  private async executeReaderAgent(
    ...
    if (skipSet.has('reader') && phaseSkip?.prebuiltExtractedData) {
      loggers.ai.info('Skipping reader phase — using prebuilt extracted data', { processId });
      const now = Date.now();
      return {
  ...
  private async executeParserAgent(
    ...
    if (skipSet.has('parser') && phaseSkip?.prebuiltParsedData) {
      loggers.ai.info('Skipping parser phase — using prebuilt parsed data', { processId });
      const now = Date.now();
      return {
  ...
  private async executeValidatorAgent(
    ...
    skipSet: Set<string>,
    phaseSkip?: PhaseSkipOptions,
  ): Promise<PromiseSettledResult<{ result: ValidationResult; metrics: AgentMetrics }>> {
    ...
    if (skipSet.has('validator') && phaseSkip?.prebuiltValidationResult) {
      loggers.ai.info('Skipping validator phase — using prebuilt validation result', { processId });
      const now = Date.now();
      return {
    ...
    try {
      const res = await this.executeAgentWithRetry(
        'validator',
        async () => {
          this.eventBus.emit({
    ...
    const executeWithTimeout = async (): Promise<T> => {
      return new Promise((resolve, reject) => {
        const timeoutId = setTimeout(() => {
          reject(new Error(`Agent ${agentName} timed out after ${this.config.agentTimeoutMs}ms`));
   ...
   */
  private detectConflictsFromResults(results: ParallelExecutionResult): Conflict[] {
    ...
    if (results.parser?.result?.parsedData) {
      ...
      const criticalFields: Array<keyof InvoiceData> = ['total', 'subtotal', 'igv', 'issuerRuc', 'customerRuc', 'invoiceNumber'];
      ...
      for (const field of criticalFields) {
        ...
        if (readerVal !== parserVal && parserVal !== undefined) {
          conflicts.push({
            ...
            severity: ['total', 'issuerRuc', 'customerRuc'].includes(field) ? 'high' : 'medium',
          });
        }
      }
    }
   ...
   */
  getMetricsSnapshot(): Record<string, { successRate: number; avgProcessingTime: number }> {
    return {
      reader: {
        successRate: this.metricsCollector.getSuccessRate('reader'),
        avgProcessingTime: this.metricsCollector.getAverageProcessingTime('reader'),
      },
      parser: {
        successRate: this.metricsCollector.getSuccessRate('parser'),
        avgProcessingTime: this.metricsCollector.getAverageProcessingTime('parser'),
      },
      validator: {
        successRate: this.metricsCollector.getSuccessRate('validator'),
        avgProcessingTime: this.metricsCollector.getAverageProcessingTime('validator'),
      },
    };
  }
  ...
  async healthCheck(): Promise<{
    ...
    const overallStatus = Object.values(agentHealth).every(h => h.status === 'healthy')
      ? 'healthy'
   ...
   */
  async recoverRun(runId: string, input: ReaderInput): Promise<WorkflowContext> {
    // Validate sessionStore is configured
    if (!this.config.sessionStore) {
      throw new SessionStoreError('SessionStore not configured — recovery unavailable');
    }

    loggers.ai.info('Starting recovery for run', { runId, inputType: input.type });
    ...
    if (runState.status === 'running') {
      ...
      throw new SessionStoreError(`Run ${runId} is still active — cannot recover`);
    }

    if (runState.status === 'completed') {
      loggers.ai.warn('Recovery blocked: run already completed', { runId });
      throw new SessionStoreError(`Run ${runId} already completed — no recovery needed`);
    }

    const workflowState = runState.workflowState;
    if (!workflowState || !this.isIntermediateState(workflowState)) {
      loggers.ai.warn('Recovery blocked: no intermediate workflow state', { runId, workflowState });
      throw new SessionStoreError(
        `Run ${runId} has no intermediate workflow state (${workflowState ?? 'none'}) — cannot determine recovery point`,
      );
    }

    const recoveredContext = (runState.context ?? {}) as Record<string, unknown>;
    ...
    this.appendSessionEvent(runId, 'RECOVERY_STARTED', { workflowState, recoveredAt: new Date().toISOString() }, runState.companyId);
    ...
    try {
      ...
      switch (workflowState) {
        ...
        case 'PARSING': {
          ...
          loggers.ai.info('Recovery: skip reader, resume from parser', { runId });
          const prebuiltExtractedData = recoveredContext.extractedData as ExtractedData | undefined;
        ...
        case 'VALIDATING': {
          ...
          loggers.ai.info('Recovery: skip reader + parser, resume from validator', { runId });
          const prebuiltExtractedData = recoveredContext.extractedData as ExtractedData | undefined;
          const prebuiltParsedData = recoveredContext.parsedData as ParsedInvoice | undefined;
        ...
        case 'ARBITRATING': {
          ...
          loggers.ai.info('Recovery: all phases already completed for run', { runId });
          const prebuiltExtractedData = recoveredContext.extractedData as ExtractedData | undefined;
          const prebuiltParsedData = recoveredContext.parsedData as ParsedInvoice | undefined;
          const prebuiltValidationResult = recoveredContext.validationResult as ValidationResult | undefined;
          if (!prebuiltExtractedData || !prebuiltParsedData || !prebuiltValidationResult) {
            throw new SessionStoreError(
      ...
      const context = this.buildRecoveryContext(runId, input, result, workflowState, recoveryStartTime);
      ...
      loggers.ai.info('Recovery completed successfully', { runId, duration, status: result.status });
      return context;
  ...
  private buildRecoveryContext(
    ...
    const stateMap: Record<string, 'IDLE' | 'EXTRACTING' | 'PARSING' | 'VALIDATING' | 'ARBITRATING' | 'COMPLETED' | 'FAILED' | 'MANUAL_REVIEW'> = {
      success: 'COMPLETED',
      failed: 'FAILED',
    ...
    return {
      ...
      extractedData: result.processingLog.stages.reading.status === 'success'
        ? (result.processingLog.stages.reading.output as ExtractedData)
        : undefined,
      parsedData: result.processingLog.stages.parsing.status === 'success'
        ? (result.processingLog.stages.parsing.output as ParsedData)
        : undefined,
      validationResult: result.processingLog.stages.validation.status === 'success'
  ...
  private isIntermediateState(state: string): boolean {
    return ['IDLE', 'EXTRACTING', 'PARSING', 'VALIDATING', 'ARBITRATING', 'OSE_SUBMITTING'].includes(state);
  }
  ...
  private async persistInput(runId: string, input: ReaderInput): Promise<void> {
    ...
    try {
      ...
      await this.config.sessionStore.saveInput(runId, input.type, input.data, checksum);
    } catch (err) {
      loggers.ai.error('Failed to persist input for recovery', { runId, error: err });
    }
  }
  ...
  private appendSessionEvent(
    ...
    this.config.sessionStore
      ...
      .catch((err) => {
        loggers.ai.warn('Failed to persist session event', { runId, eventType, error: err });
      });
  }
  ...
  private createManualReviewResult(processId: string, context: WorkflowContext): ProcessedInvoice {
    this.eventBus.emit({
    ...
    return {
      ...
      invoiceData: context.arbitrationDecision?.finalData || ({} as InvoiceData),
      processingLog: this.createProcessingLog(context),
    };
  }

  private createProcessingLog(context: WorkflowContext, oseStageLog?: StageLog): ProcessingLog {
  ...
  private checkContextThreshold(processId: string): void {
    ...
    try {
        ...
        if (usage) {
          this.eventBus.emit({
            ...
            usage: {
              ...
              usagePercent: (usage.totalTokens / usage.modelContextWindow) * 100,
            },
      ...
      loggers.ai.warn('Context threshold check failed', { processId, error: err });
    }
  }
}
