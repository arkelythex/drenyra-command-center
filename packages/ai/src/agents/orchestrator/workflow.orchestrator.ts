...
export class WorkflowOrchestrator {
  ...
  async processInvoice(input: ReaderInput): Promise<ProcessedInvoice> {
    ...
    try {
      ...
      const { reader, parser, validator } = await this.executeAgentsInParallel(processId, input);
      ...
      const conflicts = this.detectConflicts(reader.result, parser.result, validator.result);
      ...
      if (conflicts.length > 0) {
        ...
        // Stage 3: Arbitration
        loggers.ai.info('Invoking arbitrator for conflicts', { processId, conflicts: conflicts.length });
        ...
        if (decision.decision === 'REJECTED') {
          return this.createRejectedResult(processId, context, 'Rejected by arbitrator');
        }
      ...
      const finalData = context.arbitrationDecision?.finalData || reader.result.extractedData;
      ...
      const xmlContent = validator.result.generatedXML || this.generateFallbackXML(finalData);
      ...
      return this.createSuccessResult(processId, context, finalData, xmlContent);
    } catch (error) {
  ...
  private async executeAgentsInParallel(processId: string, input: ReaderInput) {
    ...
    const [readerResult, validatorResultPromise] = await Promise.all([
      ...
      (async () => {
        ...
        return {
          ...
          result,
          log: this.createStageLog('reader', stageStart, new Date(), 'success', result),
        };
      })(),
    ...
    return {
      ...
      parser: {
        ...
        result: parserResult,
        log: this.createStageLog('parser', parserStageStart, new Date(), 'success', parserResult),
      },
      validator: {
   ...
   */
  private getConflictSeverity(field: keyof InvoiceData): 'low' | 'medium' | 'high' {
    const highSeverity: Array<keyof InvoiceData> = ['total', 'issuerRuc', 'customerRuc'];
